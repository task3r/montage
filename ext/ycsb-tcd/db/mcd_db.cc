...
void TCDDB::Init(int tid) {
    ...
    if (parent == NULL) {
        ...
        //      memcached_behavior_set(parent,
        //      MEMCACHED_BEHAVIOR_BINARY_PROTOCOL, 1);
        memcached_return_t rc =
            memcached_server_add(parent, "localhost", 11211);
...
int TCDDB::Read(const std::string &table, const std::string &key,
    ...
    char *cstr = memcached_get(structs[tid], key.c_str(), key.size(), &v_len,
                               &flags, &err);
    if (err == MEMCACHED_SUCCESS && cstr != NULL) {
        result.push_back(
            std::make_pair(std::string("field0"), std::string(cstr, v_len)));
    } else {
...
int TCDDB::Scan(const std::string &table, const std::string &key, int len,
                const std::vector<std::string> *fields,
...
int TCDDB::Update(const std::string &table, const std::string &key,
    ...
    uint32_t flags;
    memcached_return_t err =
        memcached_set(structs[tid], key.c_str(), key.size(), value.c_str(),
                      value.size(), 0, 0);
    printf("success [%d]\n", err);
