...
class Foo {
   public:
    ...
    int x;
    int y;
    void *operator new(std::size_t size) { return pm_malloc(size); }

    void *operator new[](std::size_t size) { return pm_malloc(size); }

    void operator delete(void *ptr) noexcept { pm_free(ptr); }
...
int main(int argc, char *argv[]) {
    ...
    printf(
        "Running threadtest for %d threads, %d iterations, %d objects, %d work "
        "and %d sz...\n",
