...
struct _12atomic {
    T value;
    uint32_t counter;
    _12atomic(T b = 0, uint32_t c = 0) noexcept : value(b), counter(c){};
};
...
int main() {
    ...
    cout << "sizeof 12atomic<uint64_t>: " << sizeof(_12atomic<uint64_t>)
         << endl;
    cout << "sizeof 12atomic<uint32_t>: " << sizeof(_12atomic<uint32_t>)
         << endl;
    cout << "new val: " << new_val.value << endl;
    cout << "new cnt: " << new_val.counter << endl;
    for (int i = 0; i < 10; i++) {
        auto old_val = t[i].load();
        cout << "previous t[" << i << "]: " << (uint64_t)t[i].load().value
             << endl;
    fuck:
        t[i].compare_exchange_strong(old_val, new_val);
        for (int j = 0; j <= i; j++) {
