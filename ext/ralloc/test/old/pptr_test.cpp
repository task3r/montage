...
struct Node2 {
    int val;
    pptr<Node2> next;
    Node2(int v = 0, Node2* n = nullptr) : val(v), next(n){};
};
...
// 	function<vector<gc_ptr_base*>(gc_ptr_base*,GarbageCollection*)>
// filters[3]; 	template<class T> 	void set_root(gc_ptr<T>* ptr, uint64_t i){
// 		roots[i] = ptr;
...
// 		mgr = new RegionManager("/dev/shm/test");
// 		bool res =
// mgr->__nvm_region_allocator((void**)&ctr,PAGESIZE,sizeof(container)+3*sizeof(Node)+3*sizeof(gc_ptr<Node>));
...
// 		gc_ptr<Node>* pn1 =
// (gc_ptr<Node>*)(((uint64_t)nodes)+3*sizeof(Node)); 		new (pn1)
// gc_ptr<Node>(n1); 		gc_ptr<Node2>* pn2 =
// (gc_ptr<Node2>*)(((uint64_t)nodes)+3*sizeof(Node)+sizeof(gc_ptr<Node>)); 		new
// (pn2) gc_ptr<Node2>(n2);
...
void test_pptr() {
    ...
    if (RegionManager::exists_test("/dev/shm/test")) {
        ...
        pptr<Node2>* p2 = (pptr<Node2>*)(((uint64_t)p1) + 2 * sizeof(Node) +
                                         sizeof(pptr<Node>));
        pptr<Node>* p3 = (pptr<Node>*)(((uint64_t)p1) + 2 * sizeof(Node) +
                                       2 * sizeof(pptr<Node>));
        Node* tp1 = (Node*)(*p1);
        ...
        *p3 = *p1;
        ...
        mgr = new RegionManager("/dev/shm/test");
        char* start;
        bool res = mgr->__nvm_region_allocator(
            (void**)&start, PAGESIZE,
            2 * sizeof(Node) + 3 * sizeof(pptr<Node>));
        if (!res) assert(0 && "mgr allocation fails!");
        ...
        */
        ...
        new (start + sizeof(pptr<Node>) + 2 * sizeof(Node))
            pptr<Node2>((Node2*)(start + sizeof(pptr<Node>) + sizeof(Node)));
        new (start + 2 * sizeof(pptr<Node>) + 2 * sizeof(Node))
            pptr<Node>(*(pptr<Node>*)start);
    }
    delete mgr;
    cout << "done!\n";
}
