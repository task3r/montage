...
#define PAGE_CEILING(s) (((s) + (PAGESIZE - 1)) & ~(PAGESIZE - 1))
...
/* SHM_SIMULATING switches to compatible mode for machines without real
 * persistent memory. */
...
const uint64_t MIN_SB_REGION_SIZE =
    1 * 1024 * 1024 * 1024ULL;  // min sb region size
...
const uint64_t MAX_DESC_AMOUNT =
    1ULL << MAX_DESC_AMOUNT_BITS;  // maximum of superblocks in region
const uint64_t MAX_DESC_OFFSET_BITS =
    MAX_DESC_AMOUNT_BITS + 6;  // plus 2^6(64)Byte descriptor
const uint64_t MAX_DESC_OFFSET_MASK = (1ULL << MAX_DESC_OFFSET_BITS) - 1;
const uint64_t MAX_SB_AMOUNT = MAX_DESC_AMOUNT;
const int64_t MAX_SB_REGION_SIZE =
    SBSIZE * MAX_SB_AMOUNT;  // max possible sb region size to call RP_init.
                             // Currently it's 1TB which must be sufficient
