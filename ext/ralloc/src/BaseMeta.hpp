...
class CrossPtr {
   public:
    ...
    inline bool is_null() const { return off == nullptr; }
};
...
template <class T, RegionIndex idx>
inline bool operator==(const CrossPtr<T, idx>& lhs,
                       const CrossPtr<T, idx>& rhs) {
...
class ptr_cnt {
   public:
    T* ptr;  // ptr
    uint64_t cnt;
    ptr_cnt(T* p = nullptr, uint64_t c = 0) noexcept : ptr(p), cnt(c){};
    void set(T* p, uint64_t c) {
        ptr = p;
        cnt = c;
    }
    T* get_ptr() const { return ptr; }
    uint64_t get_counter() const { return cnt; }
};
...
class AtomicCrossPtrCnt {
   public:
    ...
    AtomicCrossPtrCnt(Regions* _rgs = nullptr, T* real_ptr = nullptr,
...
struct ProcHeap {
   public:
    ...
    // std::mutex lk;
    ProcHeap() noexcept : partial_list(){};
} __attribute__((aligned(CACHELINE_SIZE)));
...
class GarbageCollection {
   public:
    ...
    std::stack<std::function<void(char*, GarbageCollection& gc)>>
        to_filter_func;
    ...
    class iterator
       ...
       private:
        ...
        inline int update_status() {
            if (dirty)
                return update_status_dirty();
            else
                return update_status_clean();
        }
        ...
        inline bool action_at_new_sb() {
            if (dirty)
                return action_at_new_sb_dirty();
            else
                return action_at_new_sb_clean();
        }
        bool action_at_new_sb_dirty();
        bool action_at_new_sb_clean();

       public:
        explicit iterator(BaseMeta* b, bool d, size_t begin_sb_idx = 0,
                          size_t end_sb_idx = 0);
        iterator& operator++();
        inline bool operator==(iterator other) const {
            return curr_blk == other.curr_blk;
        }
        inline bool operator!=(iterator other) const {
            return !(*this == other);
        }
        ...
        inline size_t size() const { return curr_desc->block_size; }
...
class BaseMeta {
   public:
    ...
    // fake_dirty is set only in RP_simulate_crash and is transient. Don't call
    // RP_simulate_crash if there may be real crash
    ...
    void* set_root(void* ptr, uint64_t i) {
        ...
        void* res = nullptr;
        assert(i < MAX_ROOTS);
        if (!roots[i].is_null()) res = roots[i].to_addr(_rgs);
    ...
    inline T* get_root(uint64_t i) {
        ...
        assert(i < MAX_ROOTS);
        roots_filter_func[i] = [this](const CrossPtr<char, SB_IDX>& cptr,
                                      GarbageCollection& gc) {
            // this new statement is intentionally designed to use transient
            // allocator since it's offline
   ...
   public:
    ...
    inline Descriptor* desc_lookup(const void* ptr) {
        return desc_lookup(reinterpret_cast<const char*>(ptr));
    }
   ...
   private:
    ...
    void malloc_from_partial(size_t sc_idx, TCacheBin* cache,
                             size_t& block_num);
