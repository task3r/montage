...
void RegionManager::__map_persistent_region() {
    ...
    // | heap_start |
    // |     size   |
    new (((atomic_pptr<char>*)base_addr))
        atomic_pptr<char>((char*)((size_t)addr + PAGESIZE));
...
void RegionManager::__remap_persistent_region() {
    ...
    base_addr = (char*)addr;
    curr_addr_ptr = (atomic_pptr<char>*)base_addr;
    assert(*(uint64_t*)((size_t)base_addr + 2 * sizeof(atomic_pptr<char>)) ==
           FILESIZE);
    DBG_PRINT("Addr: %p\n", addr);
    DBG_PRINT("Base_addr: %p\n", base_addr);
...
void RegionManager::__map_transient_region() {
    ...
    void* addr = mmap(0, FILESIZE, PROT_READ | PROT_WRITE,
...
void RegionManager::__close_persistent_region() {
    ...
    unsigned long space_used =
        ((unsigned long)curr_addr_ptr->load() - (unsigned long)base_addr);
...
void RegionManager::__close_transient_region() {
    ...
    unsigned long space_used =
        ((unsigned long)curr_addr - (unsigned long)base_addr);
    unsigned long remaining_space =
        ((unsigned long)FILESIZE - space_used) / (1024 * 1024);
    DBG_PRINT("Space Used(rounded down to MiB): %ld, Remaining(MiB): %ld\n",
              space_used / (1024 * 1024), remaining_space);
...
int RegionManager::__nvm_region_allocator(void** memptr, size_t alignment,
                                          size_t size) {
    char* next;
    char* res;
    if (size <= 0) return -1;
...
int RegionManager::__try_nvm_region_allocator(void** memptr, size_t alignment,
                                              size_t size) {
    char* next;
    char* res;
    if (size <= 0) return -1;
