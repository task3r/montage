...
class RegionManager {
   public:
    ...
    atomic_pptr<char>*
        curr_addr_ptr;  // this always points to the place of base_addr
    bool persist;

    RegionManager(const std::string& file_path, uint64_t size, bool p = true,
                  bool imm_expand = true)
        : FILESIZE(((size / PAGESIZE) + 2) *
                   PAGESIZE),  // size should align to page
          HEAPFILE(file_path),
          curr_addr_ptr(nullptr),
          persist(p) {
        assert(size % CACHELINE_SIZE ==
               0);  // size should be multiple of cache line size
        if (persist) {
    ...
    /* return 1 if succeeds, 0 if CAS failed, -1 if expansion is illegal (e.g.,
     * space runs out) ret should be flushed after the call as a return value if
     * needed
     */
     ...
     */
    int __try_nvm_region_allocator(void** /*ret */, size_t /* alignment */,
                                   size_t /*size */);
...
class Regions {
   public:
    ...
    void create(const std::string& file_path, uint64_t size, bool p = true,
                bool imm_expand = true) {
        bool restart = exists_test(file_path);
        RegionManager* new_mgr =
            new RegionManager(file_path, size, p, imm_expand);
    ...
    inline char* lookup(int index) const { return regions_address[index]; }
    ...
    inline char* untranslate(int index, char* absolute_address) const {
        return (char*)((uint64_t)absolute_address -
                       (uint64_t)regions_address[index]);
    }
