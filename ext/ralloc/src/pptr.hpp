...
inline bool is_null_pptr(uint64_t off) { return off == PPTR_PATTERN_POS; }
 ...
 *
 *  TODO: implement pptr as RIV which allows cross-region references, while
 * still keeping the same interface.
 */
template <class T>
class pptr;
/*
...
class pptr {
   public:
    ...
    bool is_null() const { return off == PPTR_PATTERN_POS; }
