...
class Ralloc {
   ...
   public:
    Ralloc(int thd_num_, const char* id_,
           uint64_t size_ = 5 * 1024 * 1024 * 1024ULL);
    ~Ralloc();
    ...
    inline bool is_restart() { return restart; }
    ...
    /* return 1 if the query is invalid, otherwise 0 and write start and end
     * addr to the parameter. */
    ...
    static void set_tid(int tid_) {
        ...
        // assert(tid_<thd_num && "tid exceeds total thread number passed to
        // Ralloc constructor!");
        tid = tid_;
    }
};
...
struct RallocHolder {
    ...
    ~RallocHolder() { delete ralloc_instance; }
};

extern RallocHolder _holder;

/* return 1 if it's a restart, otherwise 0. */
extern "C" int RP_init(const char* _id,
                       uint64_t size = 5 * 1024 * 1024 * 1024ULL,
                       int thd_num = 100);
...
/* return 1 if the query is invalid, otherwise 0 and write start and end addr to
 * the parameter. */
...
// #define RP_pthread_create(thd, attr, f, arg) pm_thread_create(thd, attr, f,
// arg)
/*
 ************class ralloc************
 ...
 * 			maps to. If the file exists, it tries to restart;
 otherwise,
 ...
 * 			Currently it only supports small allocation
 (<=MAX_SMALLSIZE).
 ...
 * It's from paper:
 * 		LRMalloc: A Modern and Competitive Lock-Free Dynamic Memory
 Allocator
 * by
