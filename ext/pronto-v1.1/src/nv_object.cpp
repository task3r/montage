 ...
 */
PersistentObject::PersistentObject(uuid_t id) { constructor(id); }
...
inline bool operator<(const CommitRecord &lhs, const CommitRecord &rhs) {
    return lhs.getCommitId() >
           rhs.getCommitId();  // Force ASC order for priority queue
}

/*
 * [General rules]
 * NVM manager is responsible for recovering all persistent objects through
 * calling their Recover() method at startup. Each object is assigned to a
 * recovery thread. Look for the constructor method of NVManager for more
 * details. Also, all allocations are handled by the NVM manager object, which
 * either finds the object or creates an new persistent object using the
 * object's factory method.
 * ----------------------------------------------------------------------------------------------------
 * [Single object recovery]
 * If there are no dependencies (nested transactions), the object will go
 * through the log and plays log entries one by one based on the commit order.
 * ----------------------------------------------------------------------------------------------------
 * [Normal recovery]
 * In presence of dependant objects, here is how the synchronization between
 * persistent objects works to ensure the right replay order for log entries: >
 * Parent: there is no change in the recovery code except the code snippet which
 * child objects run to make sure the method on child object is called (and
 * executed) at the right order with respect to other operations. In other
 * words, the child object will stall the calls until it's ready. > Child
 * (non-parent): once a child object reads a log entry that belongs to a nested
 * transaction, it will wait for the parent object to pass the point specified
 * in the nested transaction log entry. For example, if the parent log shows
 * commit order 12, the child object waits for the parent to finish executing
 * the corresponding log entry and update 'last_played_commit_order' to 12.
 * ----------------------------------------------------------------------------------------------------
 * [Partial commits]
 * These are committed log entries for nested transactions where the system
 * fails before marking the outer-most transaction as committed. For example,
 * assume A calls B and the program terminates after B's log entry is marked as
 * committed but before A's entry is marked as committed. In this situation, B
 * must avoid waiting for A and should end the recovery process. If there are
 * other objects trying to play a log entry that involves the child or parent
 * object, the recovery process for those objects should stop as well. If an
 * unclean shutdown is detected, the NVM manager will initiate a process to fix
 * the log so that uncommitted transactions are not played.
 * ----------------------------------------------------------------------------------------------------
 */
void PersistentObject::Recover() {
    ...
    uint64_t logHead =
        RecoveryContext::getInstance().queryLogHeadOffset(uuid_str);
    ...
    while (ptr < limit) {
        ...
        while (!commit_queue.empty() &&
            ...
            PRINT("[%s] Playing record with commit order = %zu\n", uuid_prefix,
                  record.getCommitId());
            if (record.getMethodTag() &
                NESTED_TX_TAG) {  // dependant (nested) transaction
                off_t parent_offset =
                    (off_t)(record.getMethodTag() & (~NESTED_TX_TAG));
                ...
                assert(parent != NULL);
                uint64_t expected_commit_id =
                    *((uint64_t *)((char *)parent->log + parent_offset));
                PRINT(
                    "[%s] Nested transaction, waiting for object %s to execute "
                    "commit %zu\n",
            ...
            PRINT(
                "[%s] Finished playing commit order %zu, last played commit "
                "updated to %zu\n",
