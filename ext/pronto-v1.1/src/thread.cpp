    ...
    for (uint8_t c = 0; c < (available_cores >> 1); c++) {
        for (uint8_t ht = 0; ht < 2; ht++) {  // number of hyper-threads
            ...
            for (int i = 1; i < available_cores; i++) {
                if (core_info[i] < core_info[min_offset]) min_offset = i;
            }
...
void Savitar_core_alloc(int *core_ids) {
    ...
    // #else // SYNC_SL
    //     int total_cores = available_cores >> 1;// force to reuse cores within
    //     socket
    // #endif // SYNC_SL
...
static void *routine_wrapper(void *arg) {
    ...
    if (cfg->routine == Savitar_persister_worker) {
        ...
        cfg->buffer[0].method_tag =
            UINT64_MAX;  // Signals logger thread to terminate
                         // #ifndef SYNC_SL
...
int Savitar_thread_create(pthread_t *thread, const pthread_attr_t *attr,
    ...
    NvMethodCall *buffer =
        (NvMethodCall *)calloc(MAX_ACTIVE_TXS, sizeof(NvMethodCall));
    assert(buffer != NULL);
    memset(buffer, 0, sizeof(NvMethodCall) * MAX_ACTIVE_TXS);

    // Allocate transaction buffer
    uint64_t *tx_buffer =
        (uint64_t *)calloc(MAX_ACTIVE_TXS + 1, sizeof(uint64_t));
...
void Savitar_thread_wait(PersistentObject *object, SavitarLog *log) {
    ...
    fprintf(stdout, "%zu,%zu,%zu,%zu\n", cycles[1] - cycles[0],
            cycles[2] - cycles[1],
            sync_buffer[tx_buffer[0]].arg_ptrs[1] -
                sync_buffer[tx_buffer[0]].arg_ptrs[0],
            cycles[3] - cycles[2]);
#endif
}
