/*
 This list is based on functional list from
 https://github.com/BartoszMilewski/Okasaki/blob/master/queue/queue.hpp
*/
...
class queue {
   ...
   public:
    ...
    void operator delete(void* data) { heap::deallocate(sizeof(queue), data); }
    ...
    queue push_back(T v) const { return check(_front, list<T>(v, _rear)); }
    queue* pop_front_ptr() const {
        assert(!isEmpty());
        queue* new_queue =
            new queue(std::move(check(_front.popped_front(), _rear)));
        NVM_PERSIST_NOW(new_queue, sizeof(_front) + sizeof(_rear));
        return new_queue;
    }
    queue* push_back_ptr(T v) const {
        queue* new_queue =
            new queue(std::move(check(_front, list<T>(std::move(v), _rear))));
        NVM_PERSIST_NOW(new_queue, sizeof(_front) + sizeof(_rear));
        return new_queue;
    }
    void delete_front() {
   ...
   private:
    static queue check(list<T> const& f, list<T> const& r) {
        if (f.isEmpty()) {
            if (!r.isEmpty()) return queue(reversed(r), list<T>());
        }
        return queue(f, r);
    }
    list<T> _front;
