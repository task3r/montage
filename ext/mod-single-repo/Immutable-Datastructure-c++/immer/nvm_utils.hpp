...
struct FlushOps {
    static void persist_range(const void *ptr, uint64_t len, bool finalize) {
        ...
        if (finalize == false) {
            //    std::cout << "Flushing addr:" << ptr << ", size:" << len <<
            //    std::endl;
            uintptr_t start = (uintptr_t)ptr & ~(CACHE_LINE_SIZE - 1);
            bool unique = true;
            for (; (char *)start < (char *)ptr + len;
    ...
    static void persist_fence() { _mm_sfence(); }
};

#ifndef IMMER_DISABLE_FLUSHING
#define NVM_PERSIST(object, size) \
    immer::FlushOps::persist_range(object, size, false);
#define NVM_PERSIST_NOW(object, size) \
    immer::FlushOps::persist_range_now(object, size);
