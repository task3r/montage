...
struct node : enable_intrusive_ptr<node<T, B, MP>, typename MP::refcount>,
    ...
    enum { leaf_kind, inner_kind } kind;
    ...
    node(leaf_node<T, B> n) : kind{leaf_kind}, data{std::move(n)} {}

    node(inner_node<T, B, MP> n) : kind{inner_kind}, data{std::move(n)} {}
...
struct ref {
    ...
    void goto_pos_writable_from_clean(std::size_t old_index, std::size_t index,
                                      std::size_t xr) {
        assert(depth);
        auto d = depth - 1;
        if (d == 0) {
    ...
    void goto_pos_writable_from_dirty(std::size_t old_index,
                                      std::size_t new_index, std::size_t xr) {
        assert(depth);
        if (xr < (1 << B)) {
            display[0] = copy_of_leaf(display[0]);
        } else {
...
struct impl {
    ...
    void goto_pos_writable(std::size_t old_index, std::size_t new_index,
                           std::size_t xr) {
        if (dirty) {
    ...
    void goto_fresh_pos_writable(std::size_t old_index, std::size_t new_index,
                                 std::size_t xr) {
        if (dirty) {
    ...
    impl push_back(T value) const {
        if (size) {
            ...
            if (size != block_index) {
                ...
                auto s = impl{size + 1, block_index, dirty, p};
                s.goto_fresh_pos_writable(focus, block_index,
                                          focus ^ block_index);
                s.p.display[0]->leaf()[lo] = std::move(value);
                return s;
            }
...
template <typename T, int B, typename MP>
struct iterator
    : boost::iterator_facade<iterator<T, B, MP>, T,
                             boost::random_access_traversal_tag, const T&> {
    struct end_t {};

    iterator() = default;

    iterator(const impl<T, B, MP>& v) : p_{v.p}, i_{0}, base_{0} {
        if (v.dirty) p_.stabilize(v.focus);
        p_.goto_pos(0, 0 ^ v.focus);
        curr_ = p_.display[0]->leaf().begin();
    }

    iterator(const impl<T, B, MP>& v, end_t)
        : p_{v.p}, i_{v.size}, base_{(v.size - 1) & ~mask<B>} {
        if (v.dirty) p_.stabilize(v.focus);
