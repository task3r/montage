...
struct for_each_chunk_left_visitor {
    ...
    template <typename Pos, typename Fn>
    friend void visit_inner(this_t, Pos&& pos, size_t last, Fn&& fn) {
    ...
    template <typename Pos, typename Fn>
    friend void visit_leaf(this_t, Pos&& pos, size_t last, Fn&& fn) {
        auto data = pos.node()->leaf();
        auto l = pos.index(last);
        fn(data, data + l + 1);
    }
...
struct for_each_chunk_right_visitor {
    ...
    template <typename Pos, typename Fn>
    friend void visit_inner(this_t, Pos&& pos, size_t first, Fn&& fn) {
    ...
    template <typename Pos, typename Fn>
    friend void visit_leaf(this_t, Pos&& pos, size_t first, Fn&& fn) {
        auto data = pos.node()->leaf();
        auto f = pos.index(first);
...
struct for_each_chunk_i_visitor {
    ...
    template <typename Pos, typename Fn>
    friend void visit_relaxed(this_t, Pos&& pos, size_t first, size_t last,
                              Fn&& fn) {
    ...
    template <typename Pos, typename Fn>
    friend void visit_regular(this_t, Pos&& pos, size_t first, size_t last,
                              Fn&& fn) {
        if (first < last) {
            auto f = pos.index(first);
            auto l = pos.index(last - 1);
    ...
    template <typename Pos, typename Fn>
    friend void visit_leaf(this_t, Pos&& pos, size_t first, size_t last,
                           Fn&& fn) {
        auto data = pos.node()->leaf();
        if (first < last) {
            auto f = pos.index(first);
...
struct for_each_chunk_p_left_visitor {
    ...
    template <typename Pos, typename Fn>
    friend bool visit_inner(this_t, Pos&& pos, size_t last, Fn&& fn) {
        auto l = pos.index(last);
    ...
    template <typename Pos, typename Fn>
    friend bool visit_leaf(this_t, Pos&& pos, size_t last, Fn&& fn) {
        auto data = pos.node()->leaf();
        auto l = pos.index(last);
...
struct for_each_chunk_p_right_visitor {
    ...
    template <typename Pos, typename Fn>
    friend bool visit_inner(this_t, Pos&& pos, size_t first, Fn&& fn) {
        auto f = pos.index(first);
    ...
    template <typename Pos, typename Fn>
    friend bool visit_leaf(this_t, Pos&& pos, size_t first, Fn&& fn) {
        auto data = pos.node()->leaf();
        auto f = pos.index(first);
...
struct for_each_chunk_p_i_visitor {
    ...
    template <typename Pos, typename Fn>
    friend bool visit_relaxed(this_t, Pos&& pos, size_t first, size_t last,
                              Fn&& fn) {
        // we are going towards *two* indices, so we need to do the
        // relaxed as a special case to correct the second index
        if (first < last) {
            ...
            if (f == l) {
                auto sbh = pos.size_before(f);
                return pos.towards_oh_sbh(this_t{}, first, f, sbh, last - sbh,
                                          fn);
            } else {
                assert(f < l);
                return pos.towards_oh(for_each_chunk_p_right_visitor{}, first,
                                      f, fn) &&
                       pos.each_pred_i(for_each_chunk_p_visitor{}, f + 1, l,
                                       fn) &&
                       pos.towards_oh(for_each_chunk_p_left_visitor{}, last - 1,
                                      l, fn);
            }
        }
        return true;
    ...
    template <typename Pos, typename Fn>
    friend bool visit_regular(this_t, Pos&& pos, size_t first, size_t last,
                              Fn&& fn) {
        if (first < last) {
            auto f = pos.index(first);
            auto l = pos.index(last - 1);
    ...
    template <typename Pos, typename Fn>
    friend bool visit_leaf(this_t, Pos&& pos, size_t first, size_t last,
                           Fn&& fn) {
        auto data = pos.node()->leaf();
        if (first < last) {
            auto f = pos.index(first);
...
struct equals_visitor {
    ...
    struct this_aux_t {
        ...
        friend bool visit_inner(this_aux_t, PosR&& posr, count_t i, PosL&& posl,
                                Iter&& first, size_t idx) {
            return posl.nth_sub(i, this_t{}, posr, first, idx);
        }

        template <typename PosR, typename PosL, typename Iter>
        friend bool visit_leaf(this_aux_t, PosR&& posr, count_t i, PosL&& posl,
                               Iter&& first, size_t idx) {
    ...
    struct rrb {
        ...
        friend bool visit_node(rrb, PosR&& posr, Iter&& first, Node* rootl,
                               shift_t shiftl, size_t sizel) {
            assert(shiftl <= posr.shift());
            return shiftl == posr.shift()
                       ? visit_maybe_relaxed_sub(rootl, shiftl, sizel, this_t{},
                                                 posr, first, size_t{})
                       : posr.first_sub_inner(rrb{}, first, rootl, shiftl,
                                              sizel);
        }
    };
    ...
    friend bool visit_relaxed(this_t, PosL&& posl, PosR&& posr, Iter&& first,
                              size_t idx) {
        auto nl = posl.node();
        auto nr = posr.node();
    ...
    template <typename PosL, typename PosR, typename Iter>
    friend std::enable_if_t<is_relaxed_v<PosR>, bool> visit_regular(
        this_t, PosL&& posl, PosR&& posr, Iter&& first, size_t idx) {
    ...
    template <typename PosL, typename PosR, typename Iter>
    friend bool visit_leaf(this_t, PosL&& posl, PosR&& posr, Iter&& first,
                           size_t idx) {
        if (posl.node() == posr.node()) return true;
        auto cl = posl.count();
        auto cr = posr.count();
        auto mp = std::min(cl, cr);
        return std::equal(posl.node()->leaf(), posl.node()->leaf() + mp,
                          posr.node()->leaf()) &&
...
struct get_mut_visitor {
    ...
    template <typename Pos>
    friend value_t& visit_relaxed(this_t, Pos&& pos, size_t idx, edit_t e,
                                  node_t** location) {
        auto offset = pos.index(idx);
        auto count = pos.count();
        auto node = pos.node();
        if (node->can_mutate(e)) {
            return pos.towards_oh(this_t{}, idx, offset, e,
                                  &node->inner()[offset]);
        } else {
            auto new_node = node_t::copy_inner_sr_e(e, node, count);
            try {
                auto& res = pos.towards_oh(this_t{}, idx, offset, e,
    ...
    template <typename Pos>
    friend value_t& visit_regular(this_t, Pos&& pos, size_t idx, edit_t e,
                                  node_t** location) {
        ...
        auto node = pos.node();
        if (node->can_mutate(e)) {
            return pos.towards_oh_ch(this_t{}, idx, offset, count, e,
                                     &node->inner()[offset]);
        } else {
            auto new_node = node_t::copy_inner_e(e, node, count);
            try {
                auto& res = pos.towards_oh_ch(this_t{}, idx, offset, count, e,
                                              &new_node->inner()[offset]);
                pos.visit(dec_visitor{});
                *location = new_node;
                return res;
    ...
    template <typename Pos>
    friend value_t& visit_leaf(this_t, Pos&& pos, size_t idx, edit_t e,
                               node_t** location) {
        assert(pos.node() == *location);
        auto node = pos.node();
        if (node->can_mutate(e)) {
...
struct push_tail_mut_visitor {
    ...
    template <typename Pos>
    friend node_t* visit_relaxed(this_t, Pos&& pos, edit_t e, node_t* tail,
                                 count_t ts) {
        auto node = pos.node();
        ...
        if (new_idx >= branches<B>)
            ...
            new_child =
                mutate ? pos.last_oh_csh(this_t{}, idx, children, e, tail, ts)
                       : pos.last_oh_csh(this_no_mut_t{}, idx, children, e,
                                         tail, ts);
            if (!new_child) {
                if (++new_idx < branches<B>)
    ...
    friend node_t* visit_regular(this_t, Pos&& pos, edit_t e, node_t* tail,
                                 Args&&...) {
    ...
    template <typename Pos, typename... Args>
    friend node_t* visit_leaf(this_t, Pos&& pos, edit_t e, node_t* tail,
                              Args&&...) {
        IMMER_UNREACHABLE;
    }
};
...
struct push_tail_visitor {
    ...
    friend node_t* visit_regular(this_t, Pos&& pos, node_t* tail, Args&&...) {
        ...
        NOTE("push_tail_visitor: new_parent:%p\n", new_parent);
        try {
            NOTE("new_parent->inner:%p\n", &(new_parent->inner()[new_idx]));
            new_parent->inner()[new_idx] =
                idx == new_idx ? pos.last_oh(this_t{}, idx, tail)
                               /* otherwise */
                               : node_t::make_path(pos.shift() - B, tail);
            NVM_PERSIST_NOW(&(new_parent->inner()[new_idx]), 1);
        } catch (...) {
            ...
            try {
                auto subs =
                    mutate ? pos.towards_sub_oh(no_collapse_t{}, first, idx, e)
                           : pos.towards_sub_oh(no_collapse_no_mut_t{}, first,
                                                idx, e);
                if (mutate) pos.each_left(dec_visitor{}, idx);
                pos.copy_sizes(idx + 1, newcount - 1, new_child_size,
                               newr->d.sizes + 1);
    ...
    friend result_t visit_regular(this_t, PosT&& pos, size_t first, edit_t e) {
        ...
        if (Collapse && pos.shift() > BL && idx == pos.count() - 1) {
            ...
            try {
                ...
                pos.copy_sizes(idx + 1, newcount - 1, newr->d.sizes[0],
                               newr->d.sizes + 1);
                newr->d.count = newcount;
                newn->inner()[0] = get<1>(subs);
                std::uninitialized_copy(node->inner() + idx + 1,
...
struct concat_merger {
    ...
    concat_merger(shift_t shift, count_t* counts, count_t n)
        : curr_{counts},
          n_{n},
          result_{shift + B, node_t::make_inner_r_n(std::min(n_, branches<B>)),
                  0} {}
    ...
    void merge_inner(Pos&& p) {
        ...
        if (!to_ && *curr_ == from_count) {
            ...
            do {
                ...
                p.copy_sizes(from_offset, to_copy, to_size_,
                             sizes + to_offset_);
                to_offset_ += to_copy;
                from_offset += to_copy;
...
#pragma GCC diagnostic pop
    ...
    template <typename LPos, typename CPos, typename RPos>
    concat_center_pos<node_type<CPos>> merge(LPos&& lpos, CPos&& cpos,
                                             RPos&& rpos) {
...
concat_center_pos<Node> concat_rebalance(LPos&& lpos, CPos&& cpos,
                                         RPos&& rpos) {
...
concat_center_pos<Node> concat_leafs(LPos&& lpos, TPos&& tpos, RPos&& rpos) {
    ...
    if (tpos.count() > 0)
        return {
            Node::bits_leaf,    lpos.node()->inc(), lpos.count(),
            tpos.node()->inc(), tpos.count(),       rpos.node()->inc(),
            rpos.count(),
        };
    else
        return {
...
struct concat_left_visitor {
    ...
    friend concat_center_pos<Node> visit_inner(this_t, LPos&& lpos, TPos&& tpos,
                                               RPos&& rpos) {
        return concat_inners<Node>(lpos, tpos, rpos);
    }

    template <typename LPos, typename TPos, typename RPos>
    friend concat_center_pos<Node> visit_leaf(this_t, LPos&& lpos, TPos&& tpos,
                                              RPos&& rpos) {
        IMMER_UNREACHABLE;
    }
};
...
struct concat_right_visitor {
    ...
    friend concat_center_pos<Node> visit_inner(this_t, RPos&& rpos, LPos&& lpos,
                                               TPos&& tpos) {
        return concat_inners<Node>(lpos, tpos, rpos);
    }

    template <typename RPos, typename LPos, typename TPos>
    friend concat_center_pos<Node> visit_leaf(this_t, RPos&& rpos, LPos&& lpos,
                                              TPos&& tpos) {
...
struct concat_trees_right_visitor {
    ...
    template <typename RPos, typename LPos, typename TPos>
    friend concat_center_pos<Node> visit_node(this_t, RPos&& rpos, LPos&& lpos,
                                              TPos&& tpos) {
        return concat_inners<Node>(lpos, tpos, rpos);
    }
};
...
struct concat_trees_left_visitor {
    ...
    friend concat_center_pos<Node> visit_node(this_t, LPos&& lpos, TPos&& tpos,
                                              Args&&... args) {
...
template <typename Node>
relaxed_pos<Node> concat_trees(Node* lroot, shift_t lshift, size_t lsize,
                               Node* ltail, count_t ltcount, Node* rroot,
                               shift_t rshift, size_t rsize) {
    return visit_maybe_relaxed_sub(
               lroot, lshift, lsize, concat_trees_left_visitor<Node>{},
               make_leaf_pos(ltail, ltcount), rroot, rshift, rsize)
        .realize();
}

template <typename Node>
relaxed_pos<Node> concat_trees(Node* ltail, count_t ltcount, Node* rroot,
                               shift_t rshift, size_t rsize) {
    return make_singleton_regular_sub_pos(ltail, ltcount)
        .visit(concat_trees_left_visitor<Node>{}, empty_leaf_pos<Node>{}, rroot,
               rshift, rsize)
        .realize();
}
...
struct concat_merger_mut {
    ...
    concat_merger_mut(edit_t ec, shift_t shift, count_t* counts, count_t n,
                      edit_t candidate_e, node_t* candidate)
        : ec_{ec}, curr_{counts}, n_{n}, result_{shift + B, nullptr, 0} {
        if (candidate) {
...
struct concat_merger_mut_visitor {
    ...
    template <typename Pos, typename Merger>
    friend void visit_inner(this_t, Pos&& p, Merger& merger, edit_type<Pos> e) {
        merger.merge_inner(p, e);
    }

    template <typename Pos, typename Merger>
    friend void visit_leaf(this_t, Pos&& p, Merger& merger, edit_type<Pos> e) {
...
struct concat_rebalance_plan_mut : concat_rebalance_plan<B, BL> {
    ...
    concat_center_mut_pos<node_type<CPos>> merge(edit_type<CPos> ec,
        ...
        auto merger = merger_t{ec,      cpos.shift(), this->counts,
                               this->n, el,           lmut2 ? lnode : nullptr};
        try {
...
concat_center_pos<Node> concat_rebalance_mut(edit_type<Node> ec,
                                             edit_type<Node> el, LPos&& lpos,
                                             CPos&& cpos, edit_type<Node> er,
                                             RPos&& rpos) {
...
concat_center_mut_pos<Node> concat_leafs_mut(edit_type<Node> ec,
    ...
    if (tpos.count() > 0)
        return {
            Node::bits_leaf, lpos.node(), lpos.count(), tpos.node(),
            tpos.count(),    rpos.node(), rpos.count(),
        };
    else
        return {
            Node::bits_leaf, lpos.node(),  lpos.count(),
...
concat_center_mut_pos<Node> concat_inners_mut(edit_type<Node> ec,
    ...
    if (lshift > rshift) {
        auto cpos = lpos.last_sub(concat_left_mut_visitor<Node>{}, ec, el, tpos,
                                  er, rpos);
        return concat_rebalance_mut<Node>(ec, el, lpos, cpos, er,
                                          null_sub_pos{});
    } else if (lshift < rshift) {
        auto cpos = rpos.first_sub(concat_right_mut_visitor<Node>{}, ec, el,
                                   lpos, tpos, er);
        return concat_rebalance_mut<Node>(ec, el, null_sub_pos{}, cpos, er,
                                          rpos);
    } else {
        assert(lshift == rshift);
        assert(Node::bits_leaf == 0u || lshift > 0);
        auto cpos = lpos.last_sub(concat_both_mut_visitor<Node>{}, ec, el, tpos,
                                  er, rpos);
...
struct concat_left_mut_visitor {
    ...
    template <typename LPos, typename TPos, typename RPos>
    friend concat_center_mut_pos<Node> visit_inner(this_t, LPos&& lpos,
                                                   edit_t ec, edit_t el,
                                                   TPos&& tpos, edit_t er,
                                                   RPos&& rpos) {
    ...
    template <typename LPos, typename TPos, typename RPos>
    friend concat_center_mut_pos<Node> visit_leaf(this_t, LPos&& lpos,
                                                  edit_t ec, edit_t el,
                                                  TPos&& tpos, edit_t er,
                                                  RPos&& rpos) {
        IMMER_UNREACHABLE;
...
struct concat_right_mut_visitor {
    ...
    template <typename RPos, typename LPos, typename TPos>
    friend concat_center_mut_pos<Node> visit_inner(this_t, RPos&& rpos,
                                                   edit_t ec, edit_t el,
                                                   LPos&& lpos, TPos&& tpos,
                                                   edit_t er) {
        return concat_inners_mut<Node>(ec, el, lpos, tpos, er, rpos);
    ...
    template <typename RPos, typename LPos, typename TPos>
    friend concat_center_mut_pos<Node> visit_leaf(this_t, RPos&& rpos,
                                                  edit_t ec, edit_t el,
                                                  LPos&& lpos, TPos&& tpos,
                                                  edit_t er) {
...
struct concat_both_mut_visitor {
    ...
    friend concat_center_mut_pos<Node> visit_inner(this_t, LPos&& lpos,
                                                   ...
                                                   TPos&& tpos, edit_t er,
                                                   RPos&& rpos) {
        return rpos.first_sub(concat_right_mut_visitor<Node>{}, ec, el, lpos,
                              tpos, er);
    }
    ...
    friend concat_center_mut_pos<Node> visit_leaf(this_t, LPos&& lpos,
                                                  ...
                                                  TPos&& tpos, edit_t er,
                                                  RPos&& rpos) {
        return rpos.first_sub_leaf(concat_right_mut_visitor<Node>{}, ec, el,
                                   lpos, tpos, er);
    }
};
...
struct concat_trees_right_mut_visitor {
    ...
    template <typename RPos, typename LPos, typename TPos>
    friend concat_center_mut_pos<Node> visit_node(this_t, RPos&& rpos,
                                                  edit_t ec, edit_t el,
                                                  LPos&& lpos, TPos&& tpos,
                                                  edit_t er) {
        return concat_inners_mut<Node>(ec, el, lpos, tpos, er, rpos);
...
struct concat_trees_left_mut_visitor {
    ...
    template <typename LPos, typename TPos, typename... Args>
    friend concat_center_mut_pos<Node> visit_node(this_t, LPos&& lpos,
                                                  edit_t ec, edit_t el,
                                                  TPos&& tpos, edit_t er,
                                                  Args&&... args) {
...
template <typename Node>
relaxed_pos<Node> concat_trees_mut(edit_type<Node> ec, edit_type<Node> el,
                                   Node* lroot, shift_t lshift, size_t lsize,
                                   Node* ltail, count_t ltcount,
                                   edit_type<Node> er, Node* rroot,
                                   shift_t rshift, size_t rsize) {
    return visit_maybe_relaxed_sub(
               lroot, lshift, lsize, concat_trees_left_mut_visitor<Node>{}, ec,
               el, make_leaf_pos(ltail, ltcount), er, rroot, rshift, rsize)
        .realize_e(ec);
}
...
relaxed_pos<Node> concat_trees_mut(edit_type<Node> ec, edit_type<Node> el,
                                   ...
                                   shift_t rshift, size_t rsize) {
    return make_singleton_regular_sub_pos(ltail, ltcount)
        .visit(concat_trees_left_mut_visitor<Node>{}, ec, el,
               empty_leaf_pos<Node>{}, er, rroot, rshift, rsize)
        .realize_e(ec);
}
