...
struct regular_pos {
    ...
    decltype(auto) towards(Visitor v, size_t idx, Args&&... args) {
        return towards_oh_ch_regular(*this, v, idx, index(idx), count(),
                                     args...);
    }

    template <typename Visitor, typename... Args>
    decltype(auto) towards_oh(Visitor v, size_t idx, count_t offset_hint,
                              Args&&... args) {
        return towards_oh_ch_regular(*this, v, idx, offset_hint, count(),
                                     args...);
    }

    template <typename Visitor, typename... Args>
    decltype(auto) towards_oh_ch(Visitor v, size_t idx, count_t offset_hint,
                                 count_t count_hint, Args&&... args) {
        return towards_oh_ch_regular(*this, v, idx, offset_hint, count(),
                                     args...);
    }

    template <typename Visitor, typename... Args>
    decltype(auto) towards_sub_oh(Visitor v, size_t idx, count_t offset_hint,
                                  Args&&... args) {
...
void each_regular(Pos&& p, Visitor v, Args&&... args) {
    ...
    if (p.shift() == BL) {
        ...
        auto ss = p.shift() - B;
        for (; n != e; ++n) make_full_pos(*n, ss).visit(v, args...);
...
bool each_pred_regular(Pos&& p, Visitor v, Args&&... args) {
    ...
    if (p.shift() == BL) {
        for (; n != e; ++n) {
            IMMER_PREFETCH(n + 1);
            if (!make_full_leaf_pos(*n).visit(v, args...)) return false;
        }
        return make_leaf_pos(*n, p.size()).visit(v, args...);
    } else {
        auto ss = p.shift() - B;
        for (; n != e; ++n)
            if (!make_full_pos(*n, ss).visit(v, args...)) return false;
        return make_regular_pos(*n, ss, p.size()).visit(v, args...);
    }
}

template <typename Pos, typename Visitor, typename... Args>
bool each_pred_zip_regular(Pos&& p, Visitor v, node_type<Pos>* other,
                           Args&&... args) {
    constexpr auto B = bits<Pos>;
    constexpr auto BL = bits_leaf<Pos>;
    ...
    if (p.shift() == BL) {
        for (; n != e; ++n, ++n2) {
            IMMER_PREFETCH(n + 1);
            IMMER_PREFETCH(n2 + 1);
            if (!make_full_leaf_pos(*n).visit(v, *n2, args...)) return false;
        }
        return make_leaf_pos(*n, p.size()).visit(v, *n2, args...);
    } else {
        auto ss = p.shift() - B;
        for (; n != e; ++n, ++n2)
            if (!make_full_pos(*n, ss).visit(v, *n2, args...)) return false;
        return make_regular_pos(*n, ss, p.size()).visit(v, *n2, args...);
    }
}

template <typename Pos, typename Visitor, typename... Args>
bool each_pred_i_regular(Pos&& p, Visitor v, count_t f, count_t l,
                         Args&&... args) {
    constexpr auto B = bits<Pos>;
    constexpr auto BL = bits_leaf<Pos>;
...
template <typename Pos, typename Visitor, typename... Args>
bool each_pred_right_regular(Pos&& p, Visitor v, count_t start,
                             Args&&... args) {
    constexpr auto B = bits<Pos>;
    constexpr auto BL = bits_leaf<Pos>;
    ...
    if (p.shift() == BL) {
        ...
        if (n <= e) {
            ...
            if (!make_leaf_pos(*n, p.size()).visit(v, args...)) return false;
        }
    } else {
        auto n = p.node()->inner() + start;
        auto last = p.count() - 1;
...
decltype(auto) towards_oh_ch_regular(Pos&& p, Visitor v, size_t idx,
                                     count_t offset_hint, count_t count_hint,
                                     Args&&... args) {
    constexpr auto B = bits<Pos>;
    constexpr auto BL = bits_leaf<Pos>;
...
template <typename Pos, typename Visitor, typename... Args>
decltype(auto) last_oh_regular(Pos&& p, Visitor v, count_t offset_hint,
                               Args&&... args) {
...
struct regular_sub_pos {
    ...
    void copy_sizes(count_t offset, count_t n, size_t init, size_t* sizes) {
        if (n) {
            ...
            auto e = sizes + n - 1;
            for (; sizes != e; ++sizes) init = *sizes = init + (1 << shift_);
...
template <typename NodeT>
regular_sub_pos<NodeT> make_regular_sub_pos(NodeT* node, shift_t shift,
                                            size_t size) {
    assert(node);
    assert(shift >= NodeT::bits_leaf);
    assert(size > 0);
...
struct full_pos {
    ...
    void each(Visitor v, Args&&... args) {
        ...
        if (shift_ == BL) {
            ...
            auto ss = shift_ - B;
            for (; p != e; ++p) make_full_pos(*p, ss).visit(v, args...);
        }
    }
...
struct relaxed_pos {
    ...
    void each_i(Visitor v, count_t i, count_t n, Args&&... args) {
        if (shift_ == BL) {
            ...
            auto ss = shift_ - B;
            for (; i < n; ++i) {
                visit_maybe_relaxed_sub(p[i], ss, relaxed_->d.sizes[i] - s, v,
                                        args...);
                s = relaxed_->d.sizes[i];
            }
        }
    ...
    template <typename Visitor, typename... Args>
    decltype(auto) towards_oh_sbh(Visitor v, size_t idx, count_t offset_hint,
                                  size_t left_size_hint, Args&&... args) {
    ...
    decltype(auto) towards_sub_oh_sbh(Visitor v, size_t idx,
        ...
        return is_leaf ? make_leaf_sub_pos(child, next_size)
    ...
    template <typename Visitor, typename... Args>
    decltype(auto) last_oh_csh(Visitor v, count_t offset_hint,
                               size_t child_size_hint, Args&&... args) {
    ...
    decltype(auto) last_sub(Visitor v, Args&&... args) {
        ...
        return is_leaf ? make_leaf_sub_pos(child, child_size).visit(v, args...)
                       : visit_maybe_relaxed_sub(child, shift_ - B, child_size,
                                                 v, args...);
    }
    ...
    decltype(auto) first_sub_inner(Visitor v, Args&&... args) {
        ...
        auto child = node_->inner()[0];
        auto child_size = relaxed_->d.sizes[0];
        return visit_maybe_relaxed_sub(child, shift_ - B, child_size, v,
                                       args...);
    }
...
template <typename NodeT>
relaxed_pos<NodeT> make_relaxed_pos(NodeT* node, shift_t shift,
...
decltype(auto) visit_maybe_relaxed_descent(NodeT* node, shift_t shift,
    ...
    if (r) {
        switch (shift) {
            case BL + B * 0:
                return relaxed_descent_pos<NodeT, BL + B * 0>{node, r}.visit(
                    v, idx);
            case BL + B * 1:
                return relaxed_descent_pos<NodeT, BL + B * 1>{node, r}.visit(
                    v, idx);
            case BL + B * 2:
                return relaxed_descent_pos<NodeT, BL + B * 2>{node, r}.visit(
                    v, idx);
            case BL + B * 3:
                return relaxed_descent_pos<NodeT, BL + B * 3>{node, r}.visit(
                    v, idx);
            case BL + B * 4:
                return relaxed_descent_pos<NodeT, BL + B * 4>{node, r}.visit(
                    v, idx);
            case BL + B * 5:
                return relaxed_descent_pos<NodeT, BL + B * 5>{node, r}.visit(
                    v, idx);
#if IMMER_DESCENT_DEEP
            default:
                for (auto level = shift; level != endshift<B, BL>; level -= B) {
