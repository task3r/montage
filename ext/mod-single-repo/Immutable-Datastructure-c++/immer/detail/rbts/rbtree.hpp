...
struct rbtree {
    ...
    static const rbtree& empty() {
        static const rbtree empty_{0, BL, node_t::make_inner_n(0u),
    ...
    rbtree(rbtree&& other) : rbtree{empty()} { swap(*this, other); }
    ...
    bool traverse_p(Visitor v, size_t first, size_t last,
        ...
        return (first < tail_off
                    ? make_regular_sub_pos(root, shift, tail_off)
                          .visit(v, first, last < tail_off ? last : tail_off,
                                 args...)
                    : true) &&
               (last > tail_off
                    ? make_leaf_sub_pos(tail, tail_size)
    ...
    rbtree push_back(T value) const {
        ...
        if (ts < branches<BL>) {
            ...
            NOTE("1 new_tail:%p\n", (void*)new_tail);
            NVM_PERSIST_NOW(new_tail, node_t::sizeof_packed_leaf_n(ts + 1));
            return {size + 1, shift, root->inc(), new_tail};
        } else {
            auto new_tail = node_t::make_leaf_n(1, std::move(value));
            NOTE("2 new_tail:%p\n", (void*)new_tail);
            NVM_PERSIST_NOW(new_tail, node_t::sizeof_packed_leaf_n(1));
            try {
                if (tail_off == size_t{branches<B>} << shift) {
                    ...
                    try {
                        ...
                        NOTE("2 root:%p\n", (void*)root);
                        NOTE("2 tail:%p\n", (void*)tail);
                        NVM_PERSIST_NOW(new_root,
                                        node_t::sizeof_packed_inner_n(2));
                        // Swapnil: path is flushed within make_path.
                    ...
                    NOTE("3 new_root:%p\n", (void*)new_root);
                    tail->inc();
                    // NVM_PERSIST_NOW(new_root, node_t::max_sizeof_inner);
                    return {size + 1, shift, new_root, new_tail};
                } else {
                    auto new_root = node_t::make_path(shift, tail);
                    NOTE("4 new_root:%p\n", (void*)new_root);
                    tail->inc();
                    // Swapnil: new_root is flushed within make_path.
                    // New_tail is flushed before if-else.
                    return {size + 1, shift, new_root, new_tail};
                }
            } catch (...) {
                node_t::delete_leaf(new_tail, 1);
    ...
    rbtree update(size_t idx, FnT&& fn) const {
        ...
        if (idx >= tail_off) {
            ...
            // TODO: Figure out how to reduce size of flush.
            NOTE("update: new_tail:%p\n", new_tail);
            // NVM_PERSIST_NOW(new_tail, 1);
            ...
            // TODO:: Figure out how to reduce size of flush.
            NOTE("update: new_root:%p\n", new_root);
            // NVM_PERSIST_NOW(new_root, 1);
            // Looking at valgrind log, new_root/new_tail involve 1 cacheline
            // and the impl created below is flushed in vector.hpp
...
#if IMMER_DEBUG_DEEP_CHECK
        if (tail_size() > 0) assert(tail->check(0, tail_size()));
#endif
        return true;
    }
