...
struct rrbtree {
    ...
    static const rrbtree& empty() {
        static const rrbtree empty_{0, BL, node_t::make_inner_n(0u),
    ...
    static auto from_initializer_list(std::initializer_list<U> values) {
        ...
        for (auto&& v : values) result.push_back_mut(e, v);
        return result;
    }
    ...
    static auto from_range(Iter first, Sent last) {
        auto e = owner_t{};
        auto result = rrbtree{empty()};
        for (; first != last; ++first) result.push_back_mut(e, *first);
        return result;
    }
    ...
    static auto from_fill(size_t n, T v) {
        auto e = owner_t{};
        auto result = rrbtree{empty()};
        while (n-- > 0) result.push_back_mut(e, v);
        return result;
    }
    ...
    rrbtree(rrbtree&& other) : rrbtree{empty()} { swap(*this, other); }
    ...
    void traverse(Visitor v, size_t first, size_t last, Args&&... args) const {
        ...
        if (first < tail_off)
            visit_maybe_relaxed_sub(root, shift, tail_off, v, first,
                                    last < tail_off ? last : tail_off, args...);
        if (last > tail_off)
            make_leaf_sub_pos(tail, tail_size)
                .visit(v, first > tail_off ? first - tail_off : 0,
                       last - tail_off, args...);
    }
    ...
    template <typename Visitor, typename... Args>
    bool traverse_p(Visitor v, size_t first, size_t last,
                    Args&&... args) const {
        auto tail_off = tail_offset();
        auto tail_size = size - tail_off;
    ...
    bool for_each_chunk_p(size_t first, size_t last, Fn&& fn) const {
        return traverse_p(for_each_chunk_p_i_visitor{}, first, last,
                          std::forward<Fn>(fn));
    }
        ...
        if (tail_off > 0 && tail_off_other > 0) {
            ...
            if (other.shift >= shift) {
                if (!visit_maybe_relaxed_sub(
                        ...
                        equals_visitor::rrb{}, iter_t{other}, root, shift,
                        tail_off))
                    return false;
            } else {
                if (!visit_maybe_relaxed_sub(
                        root, shift, tail_off, equals_visitor::rrb{},
                        iter_t{*this}, other.root, other.shift, tail_off_other))
                    return false;
            }
        }
    ...
    std::tuple<shift_t, node_t*> push_tail(node_t* root, shift_t shift,
                                           size_t size, node_t* tail,
    ...
    void push_tail_mut(edit_t e, size_t tail_off, node_t* tail,
                       count_t tail_size) {
        if (auto r = root->relaxed()) {
            auto new_root =
                make_relaxed_pos(root, shift, r)
    ...
    std::tuple<const T*, size_t, size_t> region_for(size_t idx) const {
        ...
        if (idx >= tail_off) {
            ...
            auto subs = visit_maybe_relaxed_sub(root, shift, tail_off,
    ...
    T& get_mut(edit_t e, size_t idx) {
        ...
        if (idx >= tail_off) {
            ...
            return visit_maybe_relaxed_sub(root, shift, tail_off,
                                           get_mut_visitor<node_t>{}, idx, e,
                                           &root);
        }
    }
    ...
    rrbtree concat(const rrbtree& r) const {
        ...
        if (size == 0)
            ...
            if (tail_size == branches<BL>) {
                ...
                try {
                    ...
                    try {
                        ...
                        return {size + r.size, get<0>(new_root),
                                get<1>(new_root), new_tail};
                    } catch (...) {
            ...
            auto tail_size = size - tail_offst;
            auto concated =
                concat_trees(root, shift, tail_offst, tail, tail_size, r.root,
                             r.shift, r.tail_offset());
            auto new_shift = concated.shift();
            auto new_root = concated.node();
            assert(new_shift == new_root->compute_shift());
            assert(new_root->check(new_shift, size + r.tail_offset()));
    ...
    friend void concat_mut_l(rrbtree& l, edit_t el, const rrbtree& r) {
        ...
        if (l.size == 0)
            ...
            if (tail_size == branches<BL>) {
                ...
                std::uninitialized_copy(r.tail->leaf(), r.tail->leaf() + r.size,
            ...
            if (supports_transient_concat) {
                ...
                auto tail_size = l.size - tail_offst;
                auto concated =
                    concat_trees_mut(el, el, l.tail, tail_size,
                                     MemoryPolicy::transience_t::noone, r.root,
                                     r.shift, r.tail_offset());
                ...
                auto tail_offst = l.tail_offset();
                auto tail_size = l.size - tail_offst;
                auto concated = concat_trees(l.tail, tail_size, r.root, r.shift,
                                             r.tail_offset());
                l = {l.size + r.size, concated.shift(), concated.node(),
                     r.tail->inc()};
                return;
            }
        } else {
            if (supports_transient_concat) {
                ...
                auto concated = concat_trees_mut(
                    el, el, l.root, l.shift, tail_offst, l.tail, tail_size,
                    MemoryPolicy::transience_t::noone, r.root, r.shift,
                    r.tail_offset());
                assert(concated.shift() == concated.node()->compute_shift());
                assert(concated.node()->check(concated.shift(),
                                              l.size + r.tail_offset()));
                ...
                auto tail_size = l.size - tail_offst;
                auto concated =
                    concat_trees(l.root, l.shift, tail_offst, l.tail, tail_size,
    ...
    friend void concat_mut_r(const rrbtree& l, rrbtree& r, edit_t er) {
        ...
        if (r.size == 0)
            ...
            if (tail_size == branches<BL>) {
                ...
                auto remaining = branches<BL> - tail_size;
                auto add_tail = node_t::copy_leaf_e(er, l.tail, tail_size,
                                                    r.tail, remaining);
                try {
                    auto new_tail =
                    ...
                    try {
                        ...
                        auto new_root = l.push_tail(l.root, l.shift, tail_offst,
                                                    add_tail, branches<BL>);
                        r = {l.size + r.size, get<0>(new_root),
                             get<1>(new_root), new_tail};
                        return;
                    } catch (...) {
                        node_t::delete_leaf(new_tail, r.size - remaining);
            ...
            if (supports_transient_concat) {
                ...
                auto tail_size = l.size - tail_offst;
                auto concated = concat_trees_mut(
                    er, MemoryPolicy::transience_t::noone, l.tail, tail_size,
                ...
                auto concated = concat_trees(l.tail, tail_size, r.root, r.shift,
                                             r.tail_offset());
                r = {l.size + r.size, concated.shift(), concated.node(),
                     r.tail->inc()};
                return;
            }
        } else {
            if (supports_transient_concat) {
                ...
                auto tail_size = l.size - tail_offst;
                auto concated = concat_trees_mut(
                    er, MemoryPolicy::transience_t::noone, l.root, l.shift,
                    tail_offst, l.tail, tail_size, er, r.root, r.shift,
                    r.tail_offset());
                assert(concated.shift() == concated.node()->compute_shift());
                assert(concated.node()->check(concated.shift(),
                                              l.size + r.tail_offset()));
    ...
    friend void concat_mut_lr_l(rrbtree& l, edit_t el, rrbtree& r, edit_t er) {
        ...
        if (l.size == 0)
            ...
            if (supports_transient_concat) {
                ...
                auto tail_size = l.size - tail_offst;
                auto concated =
                    concat_trees_mut(el, el, l.tail, tail_size, er, r.root,
                                     r.shift, r.tail_offset());
                assert(concated.shift() == concated.node()->compute_shift());
                assert(concated.node()->check(concated.shift(),
                                              l.size + r.tail_offset()));
            ...
            if (supports_transient_concat) {
                ...
                auto tail_size = l.size - tail_offst;
                auto concated = concat_trees_mut(
                    el, el, l.root, l.shift, tail_offst, l.tail, tail_size, er,
                    r.root, r.shift, r.tail_offset());
                assert(concated.shift() == concated.node()->compute_shift());
                assert(concated.node()->check(concated.shift(),
                                              l.size + r.tail_offset()));
    ...
    friend void concat_mut_lr_r(rrbtree& l, edit_t el, rrbtree& r, edit_t er) {
        ...
        if (r.size == 0)
            ...
            if (supports_transient_concat) {
                ...
                auto tail_size = l.size - tail_offst;
                auto concated =
                    concat_trees_mut(er, el, l.tail, tail_size, er, r.root,
                                     r.shift, r.tail_offset());
                assert(concated.shift() == concated.node()->compute_shift());
                assert(concated.node()->check(concated.shift(),
                                              l.size + r.tail_offset()));
            ...
            if (supports_transient_concat) {
                ...
                auto tail_size = l.size - tail_offst;
                auto concated = concat_trees_mut(
                    er, el, l.root, l.shift, tail_offst, l.tail, tail_size, er,
                    r.root, r.shift, r.tail_offset());
                assert(concated.shift() == concated.node()->compute_shift());
                assert(concated.node()->check(concated.shift(),
                                              l.size + r.tail_offset()));
...
#if IMMER_DEBUG_DEEP_CHECK
        if (tail_size() > 0) assert(tail->check(endshift<B, BL>, tail_size()));
#endif
        return true;
    }
    ...
    void debug_print_node(std::ostream& out, node_t* node, shift_t shift,
                          size_t size, unsigned indent = 8) const {
        ...
        if (shift == endshift<B, BL>) {
            ...
            for (auto i = count_t{}; i < count; ++i) {
                debug_print_node(out, node->inner()[i], shift - B,
            ...
            if (count) {
                for (auto i = count_t{}; i < count - 1; ++i)
                    debug_print_node(out, node->inner()[i], shift - B,
                                     1 << shift, indent + indent_step);
                debug_print_node(out, node->inner()[count - 1], shift - B,
                                 size - ((count - 1) << shift),
                                 indent + indent_step);
            }
        }
