...
//
// Flushing policy: Flush node_t and relaxed_t separately.
// This is directly done in rbtree.hpp
...
struct node {
    ...
    using relaxed_t = std::conditional_t<embed_relaxed, relaxed_data_no_meta_t,
    ...
    static node_t* make_inner_n(count_t n) {
        ...
        NOTE("make_inner_n: from:%p to:%p\n", m, (char*)m + sizeof_inner_n(n));
        auto p = new (m) node_t;
        p->impl.d.data.inner.relaxed = nullptr;
#if IMMER_RBTS_TAGGED_NODE
        p->impl.d.kind = node_t::kind_t::inner;
#endif
        // TODO: flush p->impl in all callers
        return p;
    }

    static node_t* make_inner_e(edit_t e) {
        auto m = heap::allocate(max_sizeof_inner);
        NOTE("make_inner_e: from:%p to:%p\n", m, (char*)m + max_sizeof_inner);
        auto p = new (m) node_t;
        ownee(p) = e;
        p->impl.d.data.inner.relaxed = nullptr;
#if IMMER_RBTS_TAGGED_NODE
    ...
    static node_t* make_inner_r_n(count_t n) {
        ...
        NOTE("make_inner_r_n: from:%p to:%p\n", mp,
             (char*)mp + sizeof_inner_r_n(n));
    ...
    static node_t* make_inner_sr_n(count_t n, relaxed_t* r) {
        return static_if<embed_relaxed, node_t*>(
            ...
            [&](auto) {
                ...
                NOTE("make_inner_sr_n: from:%p to:%p\n", p,
                     (char*)p + node_t::sizeof_inner_r_n(n));
                assert(r->d.count >= n);
                node_t::refs(r).inc();
                p->impl.d.data.inner.relaxed = r;
#if IMMER_RBTS_TAGGED_NODE
    ...
    static node_t* make_inner_r_e(edit_t e) {
        ...
        NOTE("make_inner_r_e: from:%p to:%p\n", mp,
             (char*)mp + max_sizeof_inner_r);
        auto mr = static_cast<void*>(nullptr);
        if (embed_relaxed) {
    ...
    static node_t* make_inner_sr_e(edit_t e, relaxed_t* r) {
        return static_if<embed_relaxed, node_t*>(
            ...
            [&](auto) {
                ...
                NOTE("make_inner_sr_e: from:%p to:%p\n", p,
                     (char*)p + node_t::max_sizeof_inner_r);
                node_t::refs(r).inc();
                p->impl.d.data.inner.relaxed = r;
                node_t::ownee(p) = e;
#if IMMER_RBTS_TAGGED_NODE
    ...
    static node_t* make_leaf_n(count_t n) {
        ...
        NOTE("make_leaf_n: from:%p to:%p\n", p, (char*)p + sizeof_leaf_n(n));
#if IMMER_RBTS_TAGGED_NODE
    ...
    static node_t* make_leaf_e(edit_t e) {
        ...
        NOTE("make_leaf_e: from:%p to:%p\n", p, (char*)p + max_sizeof_leaf);
        ownee(p) = e;
#if IMMER_RBTS_TAGGED_NODE
        p->impl.d.kind = node_t::kind_t::leaf;
#endif
    ...
    static node_t* make_path(shift_t shift, node_t* node) {
        ...
        if (shift == endshift<B, BL>)
            ...
            try {
                ...
                NOTE("make_path: n->inner:%p\n", n->inner());
            } catch (...) {
                heap::deallocate(node_t::sizeof_inner_n(1), n);
                throw;
            }
            // Swapnil: We need to flush this here as nodes are
            // created recursively.
            NOTE("make_path: n:%p\n", n);
            NVM_PERSIST_NOW(n, node_t::sizeof_packed_inner_n(1));
            return n;
        }
    }
    ...
    static node_t* do_copy_inner(node_t* dst, node_t* src, count_t n) {
        ...
        NOTE("do_copy_inner: dst->inner:%p to :%p\n", dst->inner(),
             dst->inner() + n - 1);
        // FLUSH below has an extra flush sometimes, not sure why..
        NVM_PERSIST(dst->inner(), (char*)(p + n - 1) - (char*)p);
        return dst;
    }
    ...
    static node_t* do_copy_inner_sr(node_t* dst, node_t* src, count_t n) {
        if (embed_relaxed)
            ...
            inc_nodes(src->inner(), n);
            std::copy(src->inner(), src->inner() + n, dst->inner());
            NOTE("do_copy_inner_sr: dst->inner:%p\n", dst->inner());
            NVM_PERSIST(dst->inner(),
                        (char*)(dst->inner() + n - 1) - (char*)dst->inner());
            return dst;
        }
    }
    ...
    static node_t* copy_leaf(node_t* src, count_t n) {
        ...
        try {
            ...
            NOTE("copy_leaf: dst->leaf:%p to :%p\n", dst->leaf(),
                 (char*)dst->leaf() + n);
            NVM_PERSIST(dst->leaf(),
                        (char*)(dst->leaf() + n - 1) - (char*)dst->leaf());
        } catch (...) {
    ...
    static node_t* copy_leaf_e(edit_t e, node_t* src, count_t n) {
        ...
        try {
            std::uninitialized_copy(src->leaf(), src->leaf() + n, dst->leaf());
            NOTE("copy_leaf_e: dst->leaf:%p to :%p\n", dst->leaf(),
                 (char*)dst->leaf() + n);
            NVM_PERSIST(dst->leaf(),
                        (char*)(dst->leaf() + n - 1) - (char*)dst->leaf());
        } catch (...) {
    ...
    static node_t* copy_leaf_n(count_t allocn, node_t* src, count_t n) {
        ...
        try {
            std::uninitialized_copy(src->leaf(), src->leaf() + n, dst->leaf());
            NOTE("copy_leaf_n: dst->leaf:%p to :%p\n", dst->leaf(),
                 (char*)dst->leaf() + n);
            NVM_PERSIST(dst->leaf(),
                        (char*)(dst->leaf() + n - 1) - (char*)dst->leaf());
        } catch (...) {
    ...
    static node_t* copy_leaf(node_t* src1, count_t n1, node_t* src2,
                             count_t n2) {
        assert(src1->kind() == kind_t::leaf);
        assert(src2->kind() == kind_t::leaf);
        auto dst = make_leaf_n(n1 + n2);
        try {
            std::uninitialized_copy(src1->leaf(), src1->leaf() + n1,
                                    dst->leaf());
        } catch (...) {
        ...
        try {
            std::uninitialized_copy(src2->leaf(), src2->leaf() + n2,
                                    dst->leaf() + n1);
        } catch (...) {
        ...
        NOTE("copy_leaf_n1_n2: dst->leaf:%p to :%p\n", dst->leaf(),
             (char*)dst->leaf() + n1 + n2);
        NVM_PERSIST(dst->leaf(),
                    (char*)(dst->leaf() + n1 + n2 - 1) - (char*)dst->leaf());
        return dst;
    }

    static node_t* copy_leaf_e(edit_t e, node_t* src1, count_t n1, node_t* src2,
                               count_t n2) {
        assert(src1->kind() == kind_t::leaf);
        assert(src2->kind() == kind_t::leaf);
        auto dst = make_leaf_e(e);
    ...
    static node_t* copy_leaf_e(edit_t e, node_t* src, count_t idx,
                               count_t last) {
        assert(src->kind() == kind_t::leaf);
        auto dst = make_leaf_e(e);
        try {
            std::uninitialized_copy(src->leaf() + idx, src->leaf() + last,
                                    dst->leaf());
        } catch (...) {
            heap::deallocate(max_sizeof_leaf, dst);
            throw;
        ...
        NOTE("copy_leaf_e: dst->leaf:%p to :%p\n", dst->leaf(),
             (char*)dst->leaf() + last - idx);
        NVM_PERSIST(dst->leaf(),
                    (char*)(dst->leaf() + last - idx - 1) - (char*)dst->leaf());
        return dst;
    }
        ...
        return static_if<embed_relaxed, relaxed_t*>(
            ...
            [&](auto) {
                if (node_t::refs(src_r).unique() ||
                    ...
                    if (src_r) node_t::refs(src_r).dec_unsafe();
        ...
        return static_if<embed_relaxed, relaxed_t*>(
            ...
            [&](auto) {
                if (node_t::refs(src_r).unique() ||
                    ...
                    auto dst_r =
                        new (heap::allocate(max_sizeof_relaxed)) relaxed_t;
                    std::copy(src_r->d.sizes, src_r->d.sizes + n,
                              dst_r->d.sizes);
                    node_t::ownee(dst_r) = e;
