/*
 ...
 https://github.com/BartoszMilewski/Okasaki/blob/master/Queue/List.h
*/
...
class list {
   ...
   public:
    ...
    void operator delete(void* data) { heap::deallocate(sizeof(list), data); }
    ...
    list prepended(T v) const { return list(v, *this); }
    ...
    list* insertedAtPtr(int i, T v) const {
        ...
        if (i == 0)
            ...
            assert(!isEmpty());
            return_list = new list(std::move(
                list<T>(front(), popped_front().insertedAt(i - 1, v))));
        }
        NVM_PERSIST_NOW(return_list, sizeof(list));
        return return_list;
    }

    void delete_head() { delete (_head); }
...
list<T> reversed(list<T> const& lst) {
    return foldl([](list<T> const& acc, T v) { return list<T>(v, acc); },
                 list<T>(), lst);
}
