...
auto uninitialized_move(Iter1 in1, Iter1 in2, Iter2 out) {
    return std::uninitialized_copy(std::make_move_iterator(in1),
                                   std::make_move_iterator(in2), out);
}
...
T* make(Args&&... args) {
    // SWAPNIL: This is where inner objects get created.
...
inline constexpr auto log2(T x)
    -> std::enable_if_t<!std::is_same<decltype(clz_(x)), not_supported_t>{},
                        T> {
...
template <typename Iterator, typename Sentinel, typename SinkIter,
          std::enable_if_t<detail::std_uninitialized_copy_supports_v<
                               Iterator, Sentinel, SinkIter>,
                           bool> = true>
...
template <typename SourceIter, typename Sent, typename SinkIter,
          std::enable_if_t<(!detail::std_uninitialized_copy_supports_v<
                               SourceIter, Sent, SinkIter>)&&detail::
