...
struct no_capacity {
    ...
    no_capacity(node_t* p, size_t s) : ptr{p}, size{s} {}

    no_capacity(const no_capacity& other) : no_capacity{other.ptr, other.size} {
        inc();
    }

    no_capacity(no_capacity&& other) : no_capacity{empty()} {
        swap(*this, other);
    }
    ...
    void dec() {
        using immer::detail::get;
        if (ptr->refs().dec()) node_t::delete_n(ptr, size, size);
    }
    ...
    no_capacity assoc(std::size_t idx, T value) const {
        ...
        try {
            ...
            NVM_PERSIST(&(p->data()[idx]), sizeof(T));
            return {p, size};
        } catch (...) {
