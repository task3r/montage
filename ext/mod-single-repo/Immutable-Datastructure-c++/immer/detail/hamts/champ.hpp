...
struct champ {
    ...
    static const champ& empty() {
        ...
        // NVM_PERSIST(&empty_, node_t::sizeof_inner_n(0));
        return empty_;
    }

    champ(node_t* r, size_t sz) : root{r}, size{sz} {
        NOTE("Champ ctor:%p\n", this);
    }

    champ(const champ& other) : champ{other.root, other.size} { inc(); }

    champ(champ&& other) : champ{empty()} { swap(*this, other); }
    ...
    void dec() const {
        if (root->dec()) node_t::delete_deep(root, 0);
    }
    ...
    void for_each_chunk_traversal(node_t* node, count_t depth, Fn&& fn) const {
        if (depth < max_depth<B>) {
            ...
            if (datamap) fn(node->values(), node->values() + popcount(datamap));
    ...
    decltype(auto) get(const K& k) const {
        ...
        for (; fst != lst; ++fst)
            if (Equal{}(*fst, k)) return Project{}(*fst);
        return Default{}();
    }

    std::pair<node_t*, bool> do_add(node_t* node, T v, hash_t hash,
                                    shift_t shift) const {
        if (shift == max_shift<B>) {
            ...
            if (node->nodemap() & bit) {
                auto offset = popcount(node->nodemap() & (bit - 1));
                auto result = do_add(node->children()[offset], std::move(v),
                                     hash, shift + B);
                NOTE("1. result:%p\n", result);
                try {
                    result.first =
                        node_t::copy_inner_replace(node, offset, result.first);
                    NOTE("1. result.first:%p\n", result.first);
                    return result;
                } catch (...) {
                ...
                auto val = node->values() + offset;
                if (Equal{}(*val, v))
                    return {node_t::copy_inner_replace_value(node, offset,
                                                             std::move(v)),
                            false};
                else {
                    auto child = node_t::make_merged(shift + B, std::move(v),
                                                     hash, *val, Hash{}(*val));
                    try {
    ...
    /*
        #if IMMER_USE_NVM
        champ *add_ptr(T v) const
        {
            auto hash = Hash{}(v);
            auto res = do_add(root, std::move(v), hash, 0);
            auto new_size = size + (res.second ? 1 : 0);
            champ *mem = (champ*) nvm_reserve(sizeof(champ));
            return new (mem) champ res.first, new_size );
        }
        #endif
    */
    template <typename Project, typename Default, typename Combine, typename K,
              typename Fn>
    std::pair<node_t*, bool> do_update(node_t* node, K&& k, Fn&& fn,
        ...
        if (shift == max_shift<B>) {
            ...
            if (node->nodemap() & bit) {
                ...
                auto result = do_update<Project, Default, Combine>(
                    node->children()[offset], k, std::forward<Fn>(fn), hash,
                    shift + B);
                try {
                    result.first =
                        node_t::copy_inner_replace(node, offset, result.first);
    ...
    template <typename K>
    sub_result do_sub(node_t* node, const K& k, hash_t hash,
                      shift_t shift) const {
        if (shift == max_shift<B>) {
            auto fst = node->collisions();
            auto lst = fst + node->collision_count();
            ...
            if (node->nodemap() & bit) {
                ...
                if (Equal{}(*val, k)) {
                    ...
                    if (node->nodemap() || nv > 2)
                        return node_t::copy_inner_remove_value(node, bit,
                                                               offset);
                    else if (nv == 2) {
    ...
    static bool equals_tree(const node_t* a, const node_t* b, count_t depth) {
        if (a == b)
            ...
            for (auto i = count_t{}; i < n; ++i)
                if (!equals_tree<Eq>(a->children()[i], b->children()[i],
                                     depth + 1))
                    return false;
