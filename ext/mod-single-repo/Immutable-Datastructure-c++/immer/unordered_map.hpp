...
class kv_list {
    ...
    struct Item {
        ...
        Item(std::string k, std::string v, const Item* const& tail)
            : _next(tail) {
   ...
   public:
    ...
    kv_list(const kv_list& list) {
        ...
        if (_head != nullptr) NVM_PERSIST_NOW(_head, sizeof(Item));
    }
    ...
    kv_list* putPtr(std::string k, std::string v) const {
        ...
        if (isEmpty() || (cmp = strncmp(_head->_key, k.data(), k.size())) > 0)
            ...
            return_list =
                new kv_list(_head->_key, _head->_val, popped_front().put(k, v));
    ...
    std::string find(const std::string& k) {
        ...
        while (true) {
            int cmp;
            if (i == nullptr ||
                (cmp = strncmp(_head->_key, k.data(), k.size())) > 0)
                return std::string();
            else if (cmp == 0)
    ...
    void delete_item_until(std::string k) {
        ...
        while (true) {
            int cmp;
            if (i == nullptr ||
                (cmp = strncmp(i->_key, k.data(), k.size())) > 0)
                break;
            old = i;
            i = i->_next;
            delete old;
    ...
    kv_list remove(std::string k) const {
        // remove k from list in ascending order, all items smaller than k are
        // duplicated
        int cmp;
        if (isEmpty() || (cmp = strncmp(_head->_key, k.data(), k.size())) > 0)
            return kv_list(*this);
...
template <size_t BucketSize = 1000000, size_t KEY_SIZE = 32,
...
class unordered_map {
   ...
   public:
    ...
    void set(std::string k, std::string v) {
        ...
        _buckets[hash_idx].head = _buckets[hash_idx].head->putPtr(k, v);
    ...
    void remove(std::string k) {
        ...
        std::lock_guard<std::mutex> lk(_buckets[hash_idx].lock);
        auto* old_list = _buckets[hash_idx].head;
        _buckets[hash_idx].head = _buckets[hash_idx].head->removePtr(k);
        NVM_PERSIST_NOW(&_buckets[hash_idx], sizeof(Bucket));
        _mm_sfence();
        old_list->delete_item_until(k);
        delete old_list;
   ...
   private:
    // static unordered_map check(kv_list<K,T> const & f, kv_list<K,T> const &
    // r)
    // {
    //     if (f.isEmpty())
    //     {
