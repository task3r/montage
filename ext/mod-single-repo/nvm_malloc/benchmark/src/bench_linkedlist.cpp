...
void worker(int id) {
    ...
    std::uniform_int_distribution<uint64_t> distribution(payload_size_min,
                                                         payload_size_max);
    ...
    for (idx = 0; idx < 100000; ++idx) {
        ...
        nvb::activate((void *)payload, (void **)&new_node->data,
                      (void *)payload);
...
int main(int argc, char **argv) {
    if (argc < 3 || argc > 4) {
        std::cout << "usage: " << argv[0]
                  << " <num_threads> <payload_size_min> [<payload_size_max>]"
                  << std::endl;
        return -1;
    }
    ...
    if (payload_size_min < 64) {
        std::cout << "WARNING: specified allocation size was less than "
