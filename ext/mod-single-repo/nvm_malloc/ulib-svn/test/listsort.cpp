...
int comp_list_node(void *, const void *x, const void *y) {
    return generic_compare(((const list_node *)x)->data,
                           ((const list_node *)y)->data);
}

int comp_list_node_forward(void *, const void *x, const void *y) {
    return generic_compare(((const list_node_forward *)x)->data,
                           ((const list_node_forward *)y)->data);
}
...
int main() {
    ...
    list_sort_forward(NULL, (list_head_forward *)&fhead,
                      comp_list_node_forward);

    // verify and sorted list
    struct list_node *tmp;
    list_for_each_entry_safe(node, tmp, &head, link) {
        if (&tmp->link != &head) assert(comp_list_node(NULL, node, tmp) <= 0);
        delete node;
    }
