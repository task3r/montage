...
class delay_inc_mcs_cas : public ulib::thread {
   public:
    ...
    ~delay_inc_mcs_cas() { stop_and_join(); }

    int run() {
        ULIB_DEBUG("thread %ld lock entry address: %016lx", pthread_self(),
                   (unsigned long)&_lockent);
        ...
        int cur = *_counter;
        ULIB_DEBUG("thread %ld before sleep counter is %d", pthread_self(),
                   cur);
...
class delay_inc_mcs_xchg : public ulib::thread {
   public:
    ...
    ~delay_inc_mcs_xchg() { stop_and_join(); }
...
class delay_inc_ticket : public ulib::thread {
   public:
    ...
    ~delay_inc_ticket() { stop_and_join(); }
...
int main() {
    ...
    ULIB_DEBUG("w=%u, r=%u, u=%u", tkrw.tickets.write, tkrw.tickets.read,
               tkrw.tickets.user);
