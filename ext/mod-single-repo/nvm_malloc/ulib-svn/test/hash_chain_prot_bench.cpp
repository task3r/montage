...
const char *usage = "%s [ins] [get]\n";

volatile long counter = 0;

DEFINE_CHAINHASH(myhash, uint64_t, uint64_t, 1, chainhash_hashfn,
                 chainhash_equalfn, chainhash_cmpfn)
...
void constant_insert(long ins, long get) {
    ...
    for (t = 0; t < ins; t++) {
        chainhash_itr_t(myhash) itr = chainhash_set(myhash, my, myrand());
        if (!chainhash_end(itr)) chainhash_value(myhash, itr) = t;
        counter++;
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) ins = atol(argv[1]);
    if (argc > 2) get = atol(argv[2]);
