...
const unsigned char test_key1[16] = {0x80};
const unsigned char test_vec1[16] = {0x0e, 0xdd, 0x33, 0xd3, 0xc6, 0x21,
                                     0xe5, 0x46, 0x45, 0x5b, 0xd8, 0xba,
                                     0x14, 0x18, 0xbe, 0xc8};

const unsigned char test_key2[16] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                     0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                     0xff, 0xff, 0xff, 0xff};
const unsigned char test_vec2[16] = {0xa1, 0xf6, 0x25, 0x8c, 0x87, 0x7d,
                                     0x5f, 0xcd, 0x89, 0x64, 0x48, 0x45,
                                     0x38, 0xbf, 0xc9, 0x2c};

// for cbc mode with 192-bit key
// initial vector
const unsigned char test_vec3[16] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
...
// plaintext
const unsigned char test_vec4[16] = {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40,
                                     0x9f, 0x96, 0xe9, 0x3d, 0x7e, 0x11,
                                     0x73, 0x93, 0x17, 0x2a};
// ciphertext
const unsigned char test_vec5[16] = {0x4f, 0x02, 0x1d, 0xb2, 0x43, 0xbc,
                                     0x63, 0x3d, 0x71, 0x78, 0x18, 0x3a,
                                     0x9f, 0xa0, 0x71, 0xe8};
...
int main() {
    ...
    print_hex_dump_bytes("KEY\t", DUMP_PREFIX_OFFSET, test_key1,
                         sizeof(test_key1));
    ...
    print_hex_dump_bytes("KEY\t", DUMP_PREFIX_OFFSET, test_key2,
                         sizeof(test_key2));
    ...
    print_hex_dump_bytes("KEY\t", DUMP_PREFIX_OFFSET, test_key3,
                         sizeof(test_key3));
    unsigned char ivec1[16];
    memcpy(ivec1, test_vec3, sizeof(ivec1));
    aes_cbc_encrypt(test_vec4, output5, ivec1, 1, &key);
    print_hex_dump_bytes("VEC\t", DUMP_PREFIX_OFFSET, test_vec3,
                         sizeof(test_vec3));
    print_hex_dump_bytes("VEC\t", DUMP_PREFIX_OFFSET, test_vec4,
                         sizeof(test_vec4));
    ...
    const unsigned char str[128] =
        "this is a long string for AES cbc mode testing. "
        "Padding..................END";
    aes_setks_encrypt(test_key3, 192, &key);
    ...
    aes_cbc_decrypt(output7, output8, ivec1, sizeof(output7) / AES_BLOCK_SIZE,
                    &key);
