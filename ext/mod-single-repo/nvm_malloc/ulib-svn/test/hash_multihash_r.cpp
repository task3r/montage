...
struct combiner : public do_nothing_combiner<uint32_t> {
    virtual void operator()(uint32_t &sum, const uint32_t &v) const {
        sum += v;
    }
};
...
int main() {
    ...
    for (multi_hash_map<uint64_t, uint32_t, ulib_except, combiner>::iterator
             it = wc.begin();
         it != wc.end(); ++it) {
        printf("key:%lu, value: %u, hid=%zu, size=%zu, bucket=%zu\n",
               (unsigned long)it.key(), it.value(), it._hid,
               it._ht[it._hid].size(), it._nht);
        ++size;
    }
