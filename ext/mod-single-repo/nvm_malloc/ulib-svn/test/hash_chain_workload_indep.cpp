...
class writer : public thread {
   public:
    ...
    writer() : _cnt(0) {
        ...
        _seed += time(NULL);
        map = new chain_hash_map<int, int>(0x1000000);
    }
    ...
    uint64_t count() const { return _cnt; }
...
class reader : public thread {
   public:
    int run() {
        while (is_running()) {
            uint64_t r = _seed++;
            map->find(RAND_INT4_MIX64(r) & MASK);
            ++_cnt;
        }
        return 0;
    }
...
int main() {
    ...
    for (int i = 0; i < R_TH_NUM; ++i) r[i].start();
    for (int i = 0; i < W_TH_NUM; ++i) w[i].start();
    ...
    for (int i = 0; i < R_TH_NUM; ++i) r[i].stop_and_join();
    for (int i = 0; i < W_TH_NUM; ++i) w[i].stop_and_join();

    float elapsed = timer_stop(&timer);

    uint64_t r_cnt = 0;
    uint64_t w_cnt = 0;
    for (int i = 0; i < R_TH_NUM; ++i) r_cnt += r[i].count();
    for (int i = 0; i < W_TH_NUM; ++i) w_cnt += w[i].count();
