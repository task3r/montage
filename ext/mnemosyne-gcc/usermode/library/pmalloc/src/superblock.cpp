...
superblock::superblock(
    int numBlocks,              // The number of blocks in the sb.
    ...
    computeFullness();
    assert((uintptr_t)b <=
           hoardHeap::align(sizeof(superblock) + blksize * _numBlocks) +
               (uintptr_t)this);
...
superblock *superblock::makeSuperblock(int sizeclass,
    ...
    moreMemory = hoardHeap::align(
        sizeof(superblock) + (hoardHeap::align(sizeof(block)) * numBlocks));
    ...
    superblock *sb =
        new (buf) superblock(numBlocks, sizeclass, NULL, pSuperblock);
