...
void hoardCreateThread(hoardThreadType& t, void* (*function)(void*),
                       void* arg) {
    pthread_attr_t attr;
...
void hoardJoinThread(hoardThreadType& t) { pthread_join(t, NULL); }
...
void hoardSetConcurrency(int n) { pthread_setconcurrency(n); }

int hoardGetThreadID(void) {
    // Consecutive thread id's in Linux are 1024 apart.
    return (int)pthread_self() / 1024;
    // Why is this by-1024 ? If pthread_self < 1024, the return will always be 0
    // and all threads will map to the same heap ! Further, pthread_self returns
    // pthread_t which must be considered as opaque data and not int !
}
...
#else
unsigned long InterlockedExchange(unsigned long* oldval, unsigned long newval) {
#if defined(sparc)
...
void hoardYield(void) { sched_yield(); }
...
int hoardGetPageSize(void) { return (int)sysconf(_SC_PAGESIZE); }
