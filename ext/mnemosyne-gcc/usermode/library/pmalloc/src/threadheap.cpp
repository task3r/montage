...
threadHeap::threadHeap(void) : _pHeap(0), _persistentHeap(0) {}
...
void *threadHeap::malloc(const size_t size) {
    ...
    // std::cout << "threadHeap::malloc[1]: size = " << size << ", sb =  " <<
    // sb; if (sb) { std::cout << ", owner = " << sb->getOwner();	}
    ...
    if (sb == NULL) {
        ...
        // assert(sb->getBlockRegion(0) >=
        // _persistentHeap->getPersistentSegmentBase());
