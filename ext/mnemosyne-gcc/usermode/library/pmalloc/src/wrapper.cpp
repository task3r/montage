...
extern "C" void *mtm_pmalloc(size_t sz) {
    void *addr;
...
extern "C" void *mtm_pcalloc(size_t nelem, size_t elsize) {
    void *addr;
    static persistentHeap *persistentheap = getPersistentAllocator();
    static processHeap *pHeap = getAllocator(persistentheap);
...
extern "C" void mtm_pfree(void *ptr) {
    static persistentHeap *persistentheap = getPersistentAllocator();
...
extern "C" size_t mtm_get_obj_size(void *ptr) {
    static persistentHeap *persistentheap = getPersistentAllocator();
    size_t objSize = -1;
    if ((uintptr_t)ptr >=
            ((uintptr_t)persistentheap->getPersistentSegmentBase()) &&
