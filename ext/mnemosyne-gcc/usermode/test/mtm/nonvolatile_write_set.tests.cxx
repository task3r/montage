...
SUITE(NonvolatileWriteSet) {
    TEST_FIXTURE(AllWriteSetsAvailable, FirstAllocationDoesNotFail) {
        nonvolatile_write_set_t* first_set =
    ...
    TEST_FIXTURE(AllWriteSetsAvailable, SecondAllocationDoesNotFail) {
        nonvolatile_write_set_t* first_set =
            nonvolatile_write_set_next_available();
        nonvolatile_write_set_t* second_set =
            nonvolatile_write_set_next_available();
        CHECK(second_set != NULL);
        CHECK(second_set != first_set);
    }

    TEST(InitializationSucceeds) { nonvolatile_write_set_force_initialize(); }

    TEST_FIXTURE(AllWriteSetsAreBusy,
                 NextAvailableRespectsLimitsOnWriteSetBodySize) {
        CHECK(nonvolatile_write_set_next_available() == NULL);
    }

    TEST_FIXTURE(AllWriteSetsAreBusy,
                 InitializationRespectsPreviousInitialization) {
