...
/*!
 * A singleton class that wires the nonvolatile write set structure underlying
 * the nvws interface to a temporary set, owned by this object. This allows
 * tests to proceed independently of actual production or example code.
 */
struct UseTestWriteSets {
    /*! Connects the global nonvolatile-write-set structure to a temporary
     * structure. */
    UseTestWriteSets();
...
UseTestWriteSets::UseTestWriteSets()
    : theNewWriteSets(
          new nonvolatile_write_set_t[NUMBER_OF_NONVOLATILE_WRITE_SET_BLOCKS]),
      theOldWriteSets(the_nonvolatile_write_sets) {
    nonvolatile_write_set_t* sets =
        reinterpret_cast<nonvolatile_write_set_t*>(the_nonvolatile_write_sets);
