    ...
    */
    struct segment {
        ...
        size_type segment_insert(const key_type &key, size_type key_len,
            ...
            for (size_type i = 0; i < kNumPairPerCacheLine * kNumCacheLine;
                 i++) {
                auto slot_idx = (loc + i) % KNumSlot;
                scoped_t lock_bucket(
                    bucket_lock[slot_idx],
                    true);  // true：write lock，false: read lock
                if (slots[slot_idx] != nullptr) {
                    ...
                    if ((hv >> (8 * sizeof(key_hash) - local_depth)) ==
                        ...
                        transaction::run(pop, [&] {
                            ...
                            slots[slot_idx]->key =
                                make_persistent<uint8_t[]>(key_len);
                            slots[slot_idx]->value =
                                make_persistent<uint8_t[]>(value_len);
                            slots[slot_idx]->value_len = value_len;
                            memcpy(slots[slot_idx]->value.get(), value,
                                   value_len);
                            slots[slot_idx]->key_len = key_len;
                            memcpy(slots[slot_idx]->key.get() + 8, key + 8,
                                   key_len - 8);
                            pop.drain();
        ...
        description:
                For efficient migration, we only copy the pointers of inserted
        items without allocating memory again.
        */
        void Insert4split(persistent_ptr<KV_entry> &old_slot, size_t loc,
                          pool_base &pop) {
            for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine;
                ...
                if (slots[slot_idx] == nullptr) {
                    scoped_t lock_bucket(
                        bucket_lock[slot_idx],
                        true);  // true：write lock，false: read lock
                    transaction::run(pop, [&] { slots[slot_idx] = old_slot; });
                    lock_bucket.release();
                    break;
                }
            }
        ...
        persistent_ptr<persistent_ptr<segment>[]> Split(pool_base &pop,
                                                        hv_type key_hash) {
            using namespace std;
            scoped_t lock_segment;
            if (!lock_segment.try_acquire(segment_lock, true))
                return nullptr;  // try to acquire segment write lock
            ...
            for (size_type i = 0; i < KNumSlot; i++) {
                if (slots[i] != nullptr) {
                    hv_type hv =
                        hasher{}(slots[i]->key.get(), slots[i]->key_len);
                    if (hv & ((hv_type)1
                              << (sizeof(hv_type) * 8 - local_depth - 1))) {
    ...
    struct directory {
        ...
        ~directory() {
            if (segments) {
                delete_persistent_atomic<persistent_ptr<segment>[]>(segments,
                                                                    capacity);
            }
        }
    };
    ...
    RETRY:
        ...
        if (rc == 1)  // insertion failure: hash collision, need segment split
        {
            ...
            sp_segment[1]->pattern =
                ((key_hash >>
                  (8 * sizeof(key_hash) - sp_segment[0]->local_depth + 1))
                 << 1) +
                1;
            ...
            if ((dir->segments[x]->local_depth - 1) <
                dir->depth)  // segment split without directory doubling
            {
                size_type depth_diff = dir->depth - sp_segment[0]->local_depth;
                if (depth_diff == 0) {
                    if (x % 2 == 0) {
                        dir->segments[x + 1] = sp_segment[1];
                        pop.persist(&dir->segments[x + 1],
                                    sizeof(persistent_ptr<segment>));
                    } else {
                        dir->segments[x] = sp_segment[1];
                        pop.persist(&dir->segments[x],
                                    sizeof(persistent_ptr<segment>));
                    }
                } else {
            ...
            } else  // directory doubling for segment split
            {
                ...
                persistent_ptr<persistent_ptr<segment>[]> dir_old_segments =
                    dir->segments;
                transaction::run(pop, [&] {
                    persistent_ptr<directory> new_dir =
                        make_persistent<directory>(dir->depth + 1, pop);
            ...
            goto RETRY;
        } else if (rc == 2)  // insertion failure: the pattern of the item does
    ...
    ret get(const key_type &key, size_type key_len) {
        hv_type key_hash = hasher{}(key, key_len);
        size_type y = (key_hash & kMask) * kNumPairPerCacheLine;
        /* The directory reader locks protect readers from accessing a reclaimed
           directory, which guarantees the thread safety for directory. */
        ...
        if (target_segment->local_depth < dir->depth) {
            x = (key_hash >>
                 (8 * sizeof(key_hash) - target_segment->local_depth));
        }
        ...
        for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine; ++i) {
            ...
            if (target_segment->slots[location] != nullptr &&
                ...
                key_equal{}(key, target_segment->slots[location]->key.get(),
                            key_len)) {
                lock_dir.release();
