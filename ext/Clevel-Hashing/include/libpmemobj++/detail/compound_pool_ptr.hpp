...
class compound_pool_ptr {
   ...
   public:
    ...
    compound_pool_ptr() : off(0) { verify_type(); }

    /**
     *  Default null constructor, zeroes the off.
     */
    compound_pool_ptr(std::nullptr_t) noexcept : off(0) { verify_type(); }
     ...
     */
    compound_pool_ptr(PMEMoid oid) noexcept : off(oid.off) { verify_type(); }
     ...
     */
    compound_pool_ptr(uint64_t _off) noexcept : off(_off) { verify_type(); }
     ...
     * Move constructor.
     */
    compound_pool_ptr(compound_pool_ptr &&r) noexcept : off(std::move(r.off)) {
        verify_type();
    }
     ...
     */
    compound_pool_ptr &operator=(compound_pool_ptr &&r) {
    ...
    compound_pool_ptr &operator=(const compound_pool_ptr &r) {
        this->off = r.off;
    ...
    PMEMoid raw_ptr(uint64_t pool_uuid) const noexcept {
     ...
     */
    element_type *get_address(uint64_t pool_uuid) const noexcept {
        uint64_t ptr = (this->off & 0x0000FFFFFFFFFFFC);
        PMEMoid oid = {pool_uuid, ptr};
     ...
     */
    void swap(compound_pool_ptr &other) noexcept {
     ...
     */
    uint8_t get_marker() const { return (uint8_t)(this->off & 0x3); }
    ...
    bool is_next_state_of(const compound_pool_ptr &r) {
     ...
     */
    uint64_t get_offset() { return (this->off & 0x0000FFFFFFFFFFFC); }
...
inline bool operator==(const compound_pool_ptr<T> &lhs,
...
inline bool operator!=(std::nullptr_t,
                       const compound_pool_ptr<T> &lhs) noexcept {
    return lhs.raw() != 0;
}
