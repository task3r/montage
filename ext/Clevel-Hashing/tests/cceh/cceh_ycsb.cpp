...
int main(int argc, char *argv[]) {
    ...
    auto proot = pop.root();
    {
        ...
        proot->cons =
            nvobj::make_persistent<persistent_map_type>(INITIAL_DEPTH);
    ...
    while (getline(&pbuf, &len, ycsb_read) != -1) {
        if (strncmp(buf, "INSERT", 6) == 0) {
            memcpy(run_queue[operation_num % thread_num]
                            [move[operation_num % thread_num]]
                                .key,
                   buf + 7, KEY_LEN - 1);
            run_queue[operation_num % thread_num]
                     [move[operation_num % thread_num]]
                         .operation = cceh_op::INSERT;
            move[operation_num % thread_num]++;
            ...
            [&](size_t thread_id) {
                ...
                for (size_t j = 0; j < READ_WRITE_NUM / thread_num; j++) {
                    if (THREADS[thread_id].run_queue[j].operation ==
                        cceh_op::INSERT) {
                        auto ret =
                            map->insert(THREADS[thread_id].run_queue[j].key,
                        ...
                        if (ret.found) {
                            THREADS[thread_id].inserted++;
                               ...
                               cceh_op::READ) {
                        auto ret = map->get(THREADS[thread_id].run_queue[j].key,
                                            KEY_LEN);
                        if (ret.found) {
