...
cceh_op parse_cceh_op(const char *str) {
...
void insert_item(nvobj::pool<root> &pop, int i) {
    ...
    if (r.found) {
        UT_OUT("[SUCCESS] inserted %d in segment[%d] buckets[%ld]", i,
               r.segment_idx, r.bucket_idx);
    } else {
...
void search_item(nvobj::pool<root> &pop, int i) {
    ...
    if (r.found) {
        UT_OUT("[SUCCESS] found %d in segment[%d] buckets[%ld]", i,
               r.segment_idx, r.bucket_idx);
    } else {
...
void print_usage(char *exe) {
    ...
    UT_OUT(
        "    cmd: a query for a key, including \"print\" (search) and "
        "\"alloc\" (insert)");
