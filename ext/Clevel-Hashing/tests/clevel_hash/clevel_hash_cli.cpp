...
clevel_op parse_clevel_op(const char *str) {
...
void insert_item(nvobj::pool<root> &pop, int i) {
    ...
    auto r = map->insert(persistent_map_type::value_type(i, i), 1,
...
void search_item(nvobj::pool<root> &pop, int i) {
    ...
    if (r.found) {
        UT_OUT("[SUCCESS] found %d in levels[%d] buckets[%ld] slots[%d]", i,
...
void simple_test(nvobj::pool<root> &pop) {
    auto map = pop.root()->cons;
