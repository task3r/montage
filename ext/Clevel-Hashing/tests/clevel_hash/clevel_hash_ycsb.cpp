...
int main(int argc, char *argv[]) {
    ...
    while (getline(&pbuf, &len, ycsb) != -1) {
        if (strncmp(buf, "INSERT", 6) == 0) {
            ...
            auto ret = map->insert(persistent_map_type::value_type(key, key), 1,
                                   loaded);
            if (!ret.found) {
                loaded++;
            } else {
