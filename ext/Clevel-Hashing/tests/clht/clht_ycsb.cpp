...
int main(int argc, char *argv[]) {
    ...
    while (getline(&pbuf, &len, ycsb) != -1) {
        if (strncmp(buf, "INSERT", 6) == 0) {
            ...
            auto ret =
                map->put(persistent_map_type::value_type(key, key), loaded);
            if (!ret.found) {
                loaded++;
            } else {
                break;
            ...
            [&](size_t thread_id) {
                ...
                for (size_t j = 0; j < READ_WRITE_NUM / thread_num; j++) {
                    if (THREADS[thread_id].run_queue[j].operation ==
                        clht_op::INSERT) {
                        auto ret =
                            map->put(persistent_map_type::value_type(
