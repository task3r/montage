...
clht_op parse_clht_op(const char *str) {
...
void insert_item(nvobj::pool<root> &pop, int i) {
    ...
    auto r =
        map->put(persistent_map_type::value_type(i, i), static_cast<size_t>(i));
...
void search_item(nvobj::pool<root> &pop, int i) {
    ...
    if (r.found) {
        UT_OUT("[SUCCESS] found %d in buckets[%ld] step[%d] slots[%d]", i,
               r.bucket_idx, r.step, r.slot_idx);
    } else {
...
void delete_item(nvobj::pool<root> &pop, int i) {
    ...
    if (r.found) {
        UT_OUT("[SUCCESS] delete %d in buckets[%ld] step[%d] slots[%d]", i,
               r.bucket_idx, r.step, r.slot_idx);
    } else {
