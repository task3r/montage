...
int main(int argc, char *argv[]) {
    ...
    try {
        nvobj::transaction::run(pop, [&] {
            ...
            {
                vector_type::const_iterator i = r->v_pptr->begin();
                vector_type::const_iterator j = r->v_pptr->end();
                UT_ASSERT(std::distance(i, j) == 0);
                UT_ASSERT(i == j);
            }
            {
                vector_type::const_iterator i = r->v_pptr->cbegin();
                vector_type::const_iterator j = r->v_pptr->cend();
                UT_ASSERT(std::distance(i, j) == 0);
                UT_ASSERT(i == j);
            ...
            r->v_pptr =
                nvobj::make_persistent<vector_type>(std::begin(a), std::end(a));

            {
                ...
                *i = 10;
                UT_ASSERT(*i == 10);
                UT_ASSERT(std::distance(r->v_pptr->begin(), r->v_pptr->end()) ==
                          10);
            }
            {
