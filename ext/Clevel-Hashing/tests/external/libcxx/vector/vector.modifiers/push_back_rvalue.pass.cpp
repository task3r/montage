...
int main(int argc, char *argv[]) {
    ...
    auto path = argv[1];
    auto pop = nvobj::pool<root>::create(path, "VectorTest: push_back_rvalue",
    ...
    try {
        nvobj::transaction::run(pop,
                                [&] { r->c = nvobj::make_persistent<C>(); });
        r->c->push_back(move_only(0));
        UT_ASSERT(r->c->size() == 1);
        for (std::size_t j = 0; static_cast<std::size_t>(j) < r->c->size(); ++j)
