...
int main(int argc, char *argv[]) {
    ...
    auto path = argv[1];
    auto pop = nvobj::pool<root>::create(path, "VectorTest: erase_iter_iter",
    ...
    auto r = pop.root();
    {
        int a1[] = {1, 2, 3};
        {
            try {
                ...
                C::iterator i = r->l1->erase(r->l1->cbegin(), r->l1->cbegin());
                UT_ASSERT(r->l1->size() == 3);
                UT_ASSERT(distance(r->l1->cbegin(), r->l1->cend()) == 3);
                UT_ASSERT(i == r->l1->begin());
                nvobj::transaction::run(
                    pop, [&] { nvobj::delete_persistent<C>(r->l1); });
        ...
        }
        {
            try {
                ...
                C::iterator i =
                    r->l1->erase(r->l1->cbegin(), next(r->l1->cbegin()));
                UT_ASSERTeq(r->l1->size(), 2);
                UT_ASSERT(distance(r->l1->cbegin(), r->l1->cend()) == 2);
                UT_ASSERT(i == r->l1->begin());

                nvobj::transaction::run(pop, [&] {
                    nvobj::persistent_ptr<C> tmp =
                        nvobj::make_persistent<C>(a1 + 1, a1 + 3);
                    UT_ASSERT(*r->l1 == *tmp);
        ...
        }
        {
            try {
                ...
                C::iterator i =
                    r->l1->erase(r->l1->cbegin(), next(r->l1->cbegin(), 2));
                UT_ASSERT(r->l1->size() == 1);
                UT_ASSERT(distance(r->l1->cbegin(), r->l1->cend()) == 1);
                UT_ASSERT(i == r->l1->begin());

                nvobj::transaction::run(pop, [&] {
                    nvobj::persistent_ptr<C> tmp =
                        nvobj::make_persistent<C>(a1 + 2, a1 + 3);
                    UT_ASSERT(*r->l1 == *tmp);
                    nvobj::delete_persistent<C>(tmp);
                });
                nvobj::transaction::run(
        ...
        }
        {
            try {
                ...
                C::iterator i =
                    r->l1->erase(r->l1->cbegin(), next(r->l1->cbegin(), 3));
                UT_ASSERT(r->l1->size() == 0);
                UT_ASSERT(distance(r->l1->cbegin(), r->l1->cend()) == 0);
                UT_ASSERT(i == r->l1->begin());
                nvobj::transaction::run(
                    pop, [&] { nvobj::delete_persistent<C>(r->l1); });
            } catch (std::exception &e) {
        ...
        }
        {
            try {
                ...
                nvobj::transaction::run(pop, [&] {
                    ...
                    r->outer = nvobj::make_persistent<C3>(2U, *tmp);
                });
                r->outer->erase(r->outer->begin(), r->outer->begin());
    ...
    }
    {
        ...
        try {
            nvobj::transaction::run(
                pop, [&] { r->v = nvobj::make_persistent<C2>(arr, arr + 3); });
            Throws::sThrows = true;
            ...
            nvobj::transaction::run(
                pop, [&] { nvobj::delete_persistent<C2>(r->v); });
        } catch (std::exception &e) {
            UT_FATALexc(e);
        }
    }
