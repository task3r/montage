...
int main(int argc, char *argv[]) {
    ...
    auto path = argv[1];
    auto pop =
        nvobj::pool<root>::create(path, "VectorTest: insert_iter_iter_iter",
...

    {
        try {
            ...
            C::iterator i = r->v->insert(
                r->v->cbegin() + 10, test_support::input_it<const int *>(a),
                test_support::input_it<const int *>(a + N));
            UT_ASSERT(r->v->size() == 100 + N);
            UT_ASSERT(i == r->v->begin() + 10);
            unsigned j;
            for (j = 0; j < 10; ++j) UT_ASSERT((*r->v)[j] == 0);
            for (std::size_t k = 0; k < N; ++j, ++k)
                UT_ASSERT((*r->v)[j] == a[k]);
            for (; j < 105; ++j) UT_ASSERT((*r->v)[j] == 0);
    ...
    }
    {
        try {
            ...
            const int N = sizeof(a) / sizeof(a[0]);
            C::iterator i = r->v->insert(
                r->v->cbegin() + 10, test_support::forward_it<const int *>(a),
    ...
    }
    {
        try {
            ...
            for (std::size_t k = 0; k < N; ++j, ++k)
                UT_ASSERT((*r->v)[j] == a[k]);
            for (; j < r->v->size(); ++j) UT_ASSERT((*r->v)[j] == 0);
            nvobj::transaction::run(pop,
                                    [&] { nvobj::delete_persistent<C>(r->v); });
        } catch (std::exception &e) {
            UT_FATALexc(e);
