...
int main(int argc, char *argv[]) {
    ...
    auto path = argv[1];
    auto pop = nvobj::pool<root>::create(path, "VectorTest: insert_iter_value",
...

    {
        try {
            ...
            UT_ASSERT((*r->v)[j] == 1);
            for (++j; j < 101; ++j) UT_ASSERT((*r->v)[j] == 0);
    ...
    }
    {
        try {
            ...
            for (j = 0; j < 10; ++j) UT_ASSERT((*r->v)[j] == 0);
            UT_ASSERT((*r->v)[j] == 1);
            for (++j; j < r->v->size(); ++j) UT_ASSERT((*r->v)[j] == 0);
            nvobj::transaction::run(pop,
                                    [&] { nvobj::delete_persistent<C>(r->v); });
        } catch (std::exception &e) {
            UT_FATALexc(e);
    ...
    }
    {
        try {
            nvobj::transaction::run(
                pop, [&] { r->v = nvobj::make_persistent<C>(100U); });
            while (r->v->size() < r->v->capacity()) r->v->push_back(0);
            r->v->pop_back();
