 ...
 */
template <class C>
void test(nvobj::pool<struct root> &pop, nvobj::persistent_ptr<C> &pptr,
          typename C::size_type n) {
    try {
        nvobj::transaction::run(pop,
                                [&] { pptr = nvobj::make_persistent<C>(n); });

        UT_ASSERTeq(pptr->size(), n);

        for (typename C::const_iterator i = pptr->begin(), e = pptr->end();
             i != e; ++i)
...
int main(int argc, char *argv[]) {
    ...
    auto path = argv[1];
    auto pop = nvobj::pool<root>::create(path, "VectorTest: construct_size",
