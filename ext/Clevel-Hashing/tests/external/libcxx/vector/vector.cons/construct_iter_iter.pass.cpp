...
void basic_test(nvobj::pool<struct root> &pop, Iterator first, Iterator last) {
    auto r = pop.root();

    /* construct */
    try {
        nvobj::transaction::run(
            pop, [&] { r->test1 = nvobj::make_persistent<C>(first, last); });
 ...
 * thrown.
 */
static void basic_test_cases(nvobj::pool<struct root> &pop) {
    ...
    basic_test<vector_type>(pop, test_support::bidirectional_it<const int *>(a),
                            test_support::bidirectional_it<const int *>(an));
    basic_test<vector_type>(pop, test_support::random_access_it<const int *>(a),
 ...
 */
static void emplaceable_concept_tests(nvobj::pool<struct root> &pop) {
    int arr1[] = {42};
    int arr2[] = {1, 101, 42};
    ...
    /* TEST_1 */
    {
        ...
        try {
            ...
            nvobj::transaction::run(
                pop, [&] { nvobj::delete_persistent<vector_type2>(r->test2); });
        } catch (std::exception &e) {
            UT_FATALexc(e);
        }
    ...
    /* TEST_2 */
    {
        ...
        try {
            ...
            nvobj::transaction::run(
                pop, [&] { nvobj::delete_persistent<vector_type3>(r->test3); });
        } catch (std::exception &e) {
            UT_FATALexc(e);
        }
...
int main(int argc, char *argv[]) {
    ...
    auto path = argv[1];
    auto pop =
        nvobj::pool<root>::create(path, "VectorTest: construct_iter_iter",
