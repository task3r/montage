...
template <class C>
void test_copy_ctor_01(nvobj::pool<struct root> &pop, const C &x) {
    ...
    try {
        nvobj::transaction::run(pop,
                                [&] { r->v2 = nvobj::make_persistent<C>(x); });
...
template <class C>
void test_copy_ctor_02(nvobj::pool<struct root> &pop) {
    auto r = pop.root();
    ...
    try {
        nvobj::transaction::run(pop, [&] {
            nvobj::persistent_ptr<C> v3 = nvobj::make_persistent<C>(*(r->v1));
            UT_ASSERT(*v3 == *(r->v1));
...
int main(int argc, char *argv[]) {
    ...
    auto r = pop.root();
    {
        ...
        try {
            nvobj::transaction::run(pop, [&] {
                r->v1 = nvobj::make_persistent<vector_type>(a, an);
            });
        } catch (std::exception &e) {
            UT_FATALexc(e);
        }
        ...
        try {
            nvobj::transaction::run(
                pop, [&] { nvobj::delete_persistent<vector_type>(r->v1); });
        } catch (std::exception &e) {
            UT_FATALexc(e);
        }
    }
