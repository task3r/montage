 ...
 */
template <class C>
void test(nvobj::pool<struct root> &pop, typename C::size_type n) {
    const int val = 3;
    ...
    try {
        nvobj::transaction::run(
            pop, [&] { r->pptr = nvobj::make_persistent<C>(n, val); });
...
int main(int argc, char *argv[]) {
    ...
    auto path = argv[1];
    auto pop =
        nvobj::pool<root>::create(path, "VectorTest: construct_size_value",
