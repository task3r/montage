...
void test_contiguous(const vector_type &c) {
    for (size_t i = 0; i < c.size(); ++i)
        UT_ASSERT(*(c.begin() +
                    static_cast<typename vector_type::difference_type>(i)) ==
...
int main(int argc, char *argv[]) {
    ...
    auto path = argv[1];
    auto pop = nvobj::pool<root>::create(path, "VectorTest: contiguous.pass",
