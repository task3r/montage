...
int main(int argc, char *argv[]) {
    ...
    auto path = argv[1];
    auto pop = nvobj::pool<root>::create(path, "VectorTest: resize_size_value",
    ...
    try {
        ...
        nvobj::transaction::run(pop, [&] {
            nvobj::persistent_ptr<C> v_tmp = nvobj::make_persistent<C>(50U);
            UT_ASSERT(*r->v == *v_tmp);
        ...
        for (unsigned i = 0; i < 50; ++i) UT_ASSERT((*r->v)[i] == 0);
        for (unsigned i = 50; i < 200; ++i) UT_ASSERT((*r->v)[i] == 1);
