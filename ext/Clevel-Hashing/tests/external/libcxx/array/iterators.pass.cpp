...
struct Testcase5 {
    ...
    void run() {
        ...
        UT_ASSERT(c.rend() == pmem_exp::rend(c));
        UT_ASSERT(c.crend() == pmem_exp::crend(c));
        UT_ASSERT(static_cast<const C &>(c).rend() == pmem_exp::crend(c));
        ...
        UT_ASSERT(*(pmem_exp::begin(static_cast<const C &>(c)) + 1) == 1);
        UT_ASSERT(*pmem_exp::rbegin(static_cast<const C &>(c)) == 4);
        UT_ASSERT(*(pmem_exp::rbegin(static_cast<const C &>(c)) + 1) == 3);
        UT_ASSERT(*(pmem_exp::end(static_cast<const C &>(c)) - 1) == 4);
        UT_ASSERT(*(pmem_exp::rend(static_cast<const C &>(c)) - 1) == 0);
    }
};
...
void run(pmem::obj::pool<root> &pop) {
    try {
        pmem::obj::transaction::run(pop, [&] {
            ...
            pop.root()->r3 = pmem::obj::make_persistent<Testcase3>();
            pop.root()->r4 = pmem::obj::make_persistent<Testcase4>();
            pop.root()->r5 = pmem::obj::make_persistent<Testcase5>();
        });
...
int main(int argc, char *argv[]) {
    ...
    try {
        pop = pmem::obj::pool<root>::create(
            path, "iterators.pass", PMEMOBJ_MIN_POOL, S_IWUSR | S_IRUSR);
    } catch (...) {
