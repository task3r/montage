...
struct Testcase3 {
    ...
    void run() {
        ...
        static_assert((std::is_same<decltype(cc.back()), const T &>::value),
                      "");
        static_assert((std::is_same<decltype(c.cfront()), const T &>::value),
                      "");
        static_assert((std::is_same<decltype(cc.cfront()), const T &>::value),
                      "");
        static_assert((std::is_same<decltype(c.cback()), const T &>::value),
                      "");
        static_assert((std::is_same<decltype(cc.cback()), const T &>::value),
                      "");
...
int main(int argc, char *argv[]) {
    ...
    try {
        pop = pmem::obj::pool<root>::create(
            path, "front_back.pass", PMEMOBJ_MIN_POOL, S_IWUSR | S_IRUSR);
    } catch (...) {
