...
void test(U &str, unsigned pos, pmem::obj::pool<root> &pop) {
    ...
    if (pos <= str.size()) {
        ...
        nvobj::transaction::run(
            pop, [&] { nvobj::delete_persistent<string_type>(r->s1); });
    } else {
        try {
            nvobj::transaction::run(pop, [&] {
                r->s1 = nvobj::make_persistent<string_type>(str, pos);
...
template <class S, class U>
void test(U &str, unsigned pos, unsigned n, pmem::obj::pool<root> &pop) {
    typedef typename S::traits_type T;
    ...
    if (pos <= str.size()) {
        nvobj::transaction::run(pop, [&] {
            r->s1 = nvobj::make_persistent<string_type>(str, pos, n);
        });
...
void run(pmem::obj::pool<root> &pop) {
    try {
        ...
        nvobj::transaction::run(
            pop, [&] { s_default = nvobj::make_persistent<string_type>(); });

        nvobj::transaction::run(
            pop, [&] { s1 = nvobj::make_persistent<string_type>("1"); });
...
void run_on_std_str(pmem::obj::pool<root> &pop) {
    try {
