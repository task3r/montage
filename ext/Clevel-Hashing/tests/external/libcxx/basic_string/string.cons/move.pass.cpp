...
template <class S>
void test(S &s0, pmem::obj::pool<root> &pop) {
    ...
    UT_ASSERT(T::compare(r->s2->c_str(), r->s1->c_str(), r->s1->size()) == 0);
...
void run(pmem::obj::pool<root> &pop) {
    try {
        ...
        nvobj::transaction::run(
            pop, [&] { s1 = nvobj::make_persistent<string_type>(); });

        nvobj::transaction::run(
            pop, [&] { s2 = nvobj::make_persistent<string_type>("1"); });
