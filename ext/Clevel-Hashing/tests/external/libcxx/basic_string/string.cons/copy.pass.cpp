...
void test(U &s1, pmem::obj::pool<root> &pop, nvobj::persistent_ptr<S> &ptr) {
    using T = typename S::traits_type;

    nvobj::transaction::run(pop, [&] { ptr = nvobj::make_persistent<S>(s1); });
...
void run_wstring(pmem::obj::pool<root> &pop) {
    auto r = pop.root();
    ...
    try {
        ...
        nvobj::transaction::run(pop, [&] {
            ...
            ws3 = nvobj::make_persistent<W>(L"12345678901234567890");
