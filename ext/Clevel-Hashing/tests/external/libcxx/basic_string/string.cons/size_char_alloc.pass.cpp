...
void test(unsigned n, charT c, pmem::obj::pool<root> &pop) {
    auto r = pop.root();

    nvobj::transaction::run(
        pop, [&] { r->s1 = nvobj::make_persistent<pmem_exp::string>(n, c); });

    auto &s2 = *r->s1;

    UT_ASSERT(s2.size() == n);
    nvobj::transaction::run(pop, [&] {
        for (unsigned i = 0; i < n; ++i) UT_ASSERT(s2[i] == c);
    });
    ...
    nvobj::transaction::run(
        pop, [&] { nvobj::delete_persistent<pmem_exp::string>(r->s1); });
}
