...
template <class S>
void test(nvobj::pool<struct root> &pop, const S &s) {
    UT_ASSERT(s.max_size() >= s.size());

    auto r = pop.root();

    nvobj::transaction::run(pop, [&] { r->s2 = nvobj::make_persistent<S>(s); });
    ...
    nvobj::transaction::run(pop, [&] { nvobj::delete_persistent<S>(r->s2); });
}
