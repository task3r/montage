...
void test(nvobj::pool<struct root> &pop, const S &s1, It first, It last,
          const S &expected) {
    auto r = pop.root();
...
template <class S, class It>
void test_exceptions(nvobj::pool<struct root> &pop, const S &s, It first,
                     It last) {
    auto r = pop.root();
        ...
        try {
            nvobj::transaction::run(pop, [&] {
                ...
                r->s_arr[16] = nvobj::make_persistent<S>(
                    "12345678901234567890ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijkl"
                    "mnopqrstuvwxyz"
                    "A");
            ...
            s_extra_long.append(s_extra_long.begin(), s_extra_long.end());
            ...
            {
                using It = test_support::forward_it<const char *>;
