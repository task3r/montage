...
int main(int argc, char *argv[]) {
    ...
    pop = nvobj::pool<root>::create(path, LAYOUT, PMEMOBJ_MIN_POOL * 1024,
                                    S_IWUSR | S_IRUSR);
    auto proot = pop.root();

    {
        ...
        proot->cons =
            nvobj::make_persistent<persistent_map_type>((uint64_t)N_LEVELS, 1);
    ...
    while (getline(&pbuf, &len, ycsb) != -1) {
        if (strncmp(buf, "INSERT", 6) == 0) {
            ...
            if (!ret.found) {
                ...
                if (loaded % 10000 == 0) {
                    std::cout
                        << "Load factor: " << (loaded - 1) * 1.0 / ret.capacity
                        << " inserted: " << loaded
