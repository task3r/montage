...
level_hash_op parse_level_hash_op(const char *str) {
...
void insert_item(nvobj::pool<root> &pop, int i) {
...
void search_item(nvobj::pool<root> &pop, int i) {
    ...
    if (r.found) {
        UT_OUT("[SUCCESS] found %d in levels[%ld] buckets[%ld] slots[%d]", i,
               r.level_idx, r.bucket_idx, r.slot_idx);
    } else {
...
void delete_item(nvobj::pool<root> &pop, int i) {
    ...
    if (r.found) {
        UT_OUT("[SUCCESS] delete %d in levels[%ld] buckets[%ld] slots[%d]", i,
               r.level_idx, r.bucket_idx, r.slot_idx);
    } else {
...
void print_usage(char *exe) {
    ...
    UT_OUT(
        "    cmd: a query for a key, including \"print\" (search), \"alloc\" "
        "(insert), and \"free\" (delete)");
...
int main(int argc, char *argv[]) {
    ...
    if (file_exists(path) != 0) {
        ...
        proot->cons =
            nvobj::make_persistent<persistent_map_type>((size_t)12, 1);
