...
int main(int argc, char *argv[]) {
    ...
    while (getline(&pbuf, &len, ycsb) != -1) {
        if (strncmp(buf, "INSERT", 6) == 0) {
            ...
            auto ret = map->insert(persistent_map_type::value_type(key, key),
                                   (size_t)0);
            if (!ret.found) {
                loaded++;
            } else {
    ...
    for (size_t t = 0; t < thread_num; t++) {
        run_queue[t] = (thread_queue *)calloc(READ_WRITE_NUM / thread_num + 1,
                                              sizeof(thread_queue));
        latency_queue[t] =
    ...
    while (getline(&pbuf, &len, ycsb_read) != -1) {
        if (strncmp(buf, "INSERT", 6) == 0) {
            run_queue[operation_num % thread_num]
                     [move[operation_num % thread_num]]
                         .key = string_t(buf + 7, KEY_LEN);
            run_queue[operation_num % thread_num]
                     [move[operation_num % thread_num]]
                         .operation = level_hash_op::INSERT;
            move[operation_num % thread_num]++;
        } else if (strncmp(buf, "READ", 4) == 0) {
            run_queue[operation_num % thread_num]
                     [move[operation_num % thread_num]]
                         .key = string_t(buf + 5, KEY_LEN);
            run_queue[operation_num % thread_num]
                     [move[operation_num % thread_num]]
                         .operation = level_hash_op::READ;
            move[operation_num % thread_num]++;
            ...
            run_queue[operation_num % thread_num]
                     [move[operation_num % thread_num]]
                         .operation = level_hash_op::DELETE;
            move[operation_num % thread_num]++;
            ...
            run_queue[operation_num % thread_num]
                     [move[operation_num % thread_num]]
                         .operation = level_hash_op::UPDATE;
            move[operation_num % thread_num]++;
        }
        operation_num++;
    }
    ...
    for (size_t i = 0; i < thread_num; i++) {
        threads.emplace_back(
            [&](size_t thread_id) {
                ...
                size_t offset =
                    loaded + READ_WRITE_NUM / thread_num * thread_id;
                for (size_t j = 0; j < READ_WRITE_NUM / thread_num; j++) {
                    if (THREADS[thread_id].run_queue[j].operation ==
                        ...
                        if (!ret.found) {
                            ...
                            THREADS[thread_id].ins_failure++;
                               ...
                               level_hash_op::READ) {
                        auto ret =
                            map->query(persistent_map_type::key_type(
                                           THREADS[thread_id].run_queue[j].key),
                                       thread_id);
                        if (ret.found) {
                        ...
                        auto ret =
                            map->erase(persistent_map_type::key_type(
                                           THREADS[thread_id].run_queue[j].key),
                                       thread_id);
                    ...
                    THREADS[thread_id].latency_queue[j] =
                        stop.tv_sec * 1000000000.0 + stop.tv_nsec;
    ...
    size_t elapsed =
        static_cast<size_t>((end.tv_sec - start.tv_sec) * 1000000000 +
    ...
    printf(
        "Delete operations: deleted existing %ld items via %ld delete "
        "operations in total\n",
        del_existing, deleted);
    printf(
        "Update operations: update existing %ld items via %ld update "
        "operations in total\n",
        upd_existing, updated);

    float elapsed_sec = elapsed / 1000000000.0;
    printf("%f seconds\n", elapsed_sec);
    printf("%f reqs per second (%ld threads)\n", READ_WRITE_NUM / elapsed_sec,
           thread_num);
    ...
    for (size_t t = 0; t < thread_num; ++t) {
        ...
        for (size_t i = 1; i < READ_WRITE_NUM / thread_num; i++) {
            latency =
                THREADS[t].latency_queue[i] - THREADS[t].latency_queue[i - 1];
            total_latency += latency;
            fprintf(fp_latency, "%f\n", latency);
        }
    }
