...
void memcached_thread_init() {
    ...
    if (is_restart) {
        ...
        lru_locks = (pthread_mutex_t *)pm_malloc(sizeof(pthread_mutex_t) *
                                                 POWER_LARGEST);
        item_locks = (pthread_mutex_t *)pm_calloc(item_lock_count,
 ...
 */
item *item_alloc(const char *key, size_t nkey, int flags, rel_time_t exptime,
                 int nbytes) {
    item *it;
...
item *item_get_locked(const char *key, const size_t nkey, const bool do_update,
                      uint32_t *hv) {
    *hv = tcd_hash(key, nkey);
    item_lock(*hv);
 ...
 */
enum delta_result_type add_delta(const char *key, const size_t nkey, bool incr,
