...
int bipbuf_unused(const bipbuf_t* me) {
    if (1 == me->b_inuse) /* distance between region B and region A */
...
void bipbuf_free(bipbuf_t* me) { pm_free(me); }
...
unsigned char* bipbuf_peek(const bipbuf_t* me, const unsigned int size) {
    ...
    if (me->size < me->a_start + size) return NULL;

    if (bipbuf_is_empty(me)) return NULL;
