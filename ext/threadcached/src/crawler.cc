...
typedef void (*crawler_eval_func)(crawler_module_t *cm, item *it, uint32_t hv,
                                  int slab_cls);
typedef int (*crawler_init_func)(crawler_module_t *cm,
                                 void *data);               // TODO: init args?
...
struct crawler_module_reg_t {
    ...
    crawler_doneclass_func
        doneclass; /* runs once per sub-crawler completion. */
    crawler_finalize_func
        finalize;    /* runs once when all sub-crawlers are done. */
    bool needs_lock; /* whether or not we need the LRU lock held when eval is
                        called */
};
...
static void crawler_expired_eval(crawler_module_t *cm, item *search,
                                 uint32_t hv, int i);
...
static void crawler_metadump_eval(crawler_module_t *cm, item *search,
                                  uint32_t hv, int i);
...
crawler_module_reg_t *crawler_mod_regs[3] = {
    &crawler_expired_mod, &crawler_expired_mod, &crawler_metadump_mod};
...
static void crawler_metadump_eval(crawler_module_t *cm, item *it, uint32_t hv,
                                  int i) {
    refcount_decr(it);
}
        ...
        while (crawler_count) {
            ...
            for (i = POWER_SMALLEST; i < LARGEST_ID; i++) {
                ...
                if (refcount_incr(search) != 2) {
                    refcount_decr(search);
                    if (hold_lock) item_trylock_unlock(hold_lock);
                    pthread_mutex_unlock(&lru_locks[i]);
                    continue;
                }
                ...
                active_crawler_mod.mod->eval(&active_crawler_mod, search, hv,
                                             i);
...
int lru_crawler_start(uint8_t *ids, uint32_t remaining,
                      const enum crawler_run_type type, void *data, void *c,
                      const int sfd) {
#if 0
  int starts = 0;
  bool is_running;
...
void lru_crawler_pause(void) { pthread_mutex_lock(&lru_crawler_lock); }

void lru_crawler_resume(void) { pthread_mutex_unlock(&lru_crawler_lock); }
