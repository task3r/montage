...
void assoc_init(const int hashtable_init) {
    ...
    if (!is_restart) {
        primary_hashtable =
            (pptr<item> *)pm_malloc(hashsize(hashpower) * sizeof(pptr<item>));
...
static pptr<item> *_hashitem_before(const char *key, const size_t nkey,
                                    const uint32_t hv) {
    pptr<item> *pos;
    unsigned int oldbucket;
        ...
        for (ii = 0; ii < hash_bulk_move && expanding; ++ii) {
            ...
            if ((item_lock = item_trylock(expand_bucket))) {
                for (it = old_hashtable[expand_bucket]; NULL != it; it = next) {
                    next = it->h_next;
                    bucket =
                        tcd_hash(ITEM_key(it), it->nkey) & hashmask(hashpower);
                ...
                if (expand_bucket == hashsize(hashpower - 1)) {
                    ...
                    STATS_LOCK();
                    __global_stats->hash_bytes -=
                        hashsize(hashpower - 1) * sizeof(void *);
