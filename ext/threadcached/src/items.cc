...
void items_init() {
    if (is_restart) {
        ...
        lru_maintainer_lock =
            pm_get_root<pthread_mutex_t>(RPMRoot::LRUMaintainerLock);
    } else {
        ...
        lru_maintainer_lock =
            (pthread_mutex_t *)pm_malloc(sizeof(pthread_mutex_t));
...
int item_is_flushed(item *it) {
    ...
    if (oldest_live == 0 || oldest_live > *current_time) return 0;
...
bool item_size_ok(const size_t nkey, const int flags, const int nbytes) {
    ...
    size_t ntotal = item_make_header(nkey + 2, flags, nbytes, prefix, &nsuffix);
...
int lru_pull_tail(const int orig_id, const int cur_lru,
    ...
    for (; tries > 0 && search != NULL; tries--, search = next_it) {
        ...
        if ((hold_lock = item_trylock(hv)) == NULL) continue;
        ...
        switch (cur_lru) {
            ...
            case WARM_LRU:
                if (limit == 0) limit = total_bytes * 40 / 100;
            ...
            case COLD_LRU:
                ...
                if (flags & LRU_PULL_EVICT) {
                    ...
                    if (search->exptime != 0) do_item_unlink_nolock(search, hv);
                    removed++;
                ...
                break;
            case TEMP_LRU:
                it = search; /* Kill the loop. Parent only interested in
                                reclaims */
                break;
        }
        if (it != NULL) break;
...
static int lru_maintainer_juggle(const int slabs_clsid) {
    ...
    for (i = 0; i < 500; i++) {
        int do_more = 0;
        if (lru_pull_tail(slabs_clsid, HOT_LRU, total_bytes,
...
static void *lru_maintainer_thread(void *arg) {
    ...
    crawler_expired_data *cdata = (crawler_expired_data *)pm_calloc(
        1, sizeof(struct crawler_expired_data));
    if (cdata == NULL) {
        fprintf(stderr,
    ...
    while (do_run_lru_maintainer_thread) {
        ...
        for (i = POWER_SMALLEST; i < MAX_NUMBER_OF_SLAB_CLASSES; i++) {
            ...
            if (next_juggles[i] > 0) {
                ...
                if (next_juggles[i] < to_sleep) to_sleep = next_juggles[i];
                continue;
            }
