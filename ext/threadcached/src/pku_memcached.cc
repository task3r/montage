...
memcached_return_t memcached_set(memcached_st *ptr, const char *key,
                                 size_t nkey, const char *data, size_t datan,
...
memcached_return_t memcached_flush(memcached_st *ptr, uint32_t exptime) {
...
memcached_result_st *memcached_fetch_result_internal(
    ...
    assert(run_once &&
           "You must run memcached_init before calling memcached_functions");
    if (nptrs == ptrcnt) {
        *error = MEMCACHED_END;
        return nullptr;
    }
    ...
    result->item_cas =
        0;  // it->data; // TODO is this right? maybe just default value it
...
HODOR_FUNC_ATTR
#endif
char *memcached_get_internal(const char *key, size_t key_length,
                             size_t *value_length, uint32_t *flags,
                             memcached_return_t *error) {
    assert(run_once &&
           "You must run memcached_init before calling memcached_functions");
...
HODOR_FUNC_ATTR
#endif
memcached_return_t memcached_mget_internal(const char *const *keys,
                                           const size_t *key_length,
                                           size_t number_of_keys) {
    assert(run_once &&
           "You must run memcached_init before calling memcached_functions");
    if (number_of_keys > 128) return MEMCACHED_FAILURE;
...
HODOR_FUNC_ATTR
#endif
memcached_return_t memcached_add_internal(const char *key, size_t nkey,
                                          const char *data, size_t datan,
                                          uint32_t exptime, uint32_t flags) {
    assert(run_once &&
           "You must run memcached_init before calling memcached_functions");
...
HODOR_FUNC_ATTR
#endif
memcached_return_t memcached_append_internal(const char *key, size_t nkey,
                                             const char *data, size_t datan,
                                             uint32_t exptime, uint32_t flags) {
    assert(run_once &&
           "You must run memcached_init before calling memcached_functions");
...
HODOR_FUNC_ATTR
#endif
memcached_return_t memcached_prepend_internal(const char *key, size_t nkey,
                                              const char *data, size_t datan,
                                              uint32_t exptime,
                                              uint32_t flags) {
    assert(run_once &&
...
HODOR_FUNC_ATTR
#endif
memcached_return_t memcached_increment_internal(const char *key, size_t nkey,
                                                uint64_t delta,
                                                uint64_t *value) {
    assert(run_once &&
           "You must run memcached_init before calling memcached_functions");
...
HODOR_FUNC_ATTR
#endif
memcached_return_t memcached_decrement_internal(const char *key, size_t nkey,
                                                uint64_t delta,
                                                uint64_t *value) {
    assert(run_once &&
           "You must run memcached_init before calling memcached_functions");
...
memcached_return_t memcached_increment_with_initial_internal(
    ...
    switch (add_delta(key, nkey, true, delta, value)) {
        ...
        case DELTA_ITEM_NOT_FOUND:
            ...
            *value = initial;
            return pku_memcached_insert(key, nkey, buff, NT - (&*buff),
                                        exptime);
        default:
            return MEMCACHED_FAILURE;
    }
