...
static rel_time_t realtime(const time_t exptime) {
    ...
    if (exptime > REALTIME_MAXDELTA) {
        ...
        if (exptime <= process_started) return (rel_time_t)1;
...
std::pair<store_item_type, size_t> do_store_item(item *it, int comm,
                                                 const uint32_t hv) {
    ...
    if (old_it != NULL && comm == NREAD_ADD) {
        ...
        if (comm == NREAD_APPEND || comm == NREAD_PREPEND) {
            ...
            if (stored == NOT_STORED) {
                ...
                new_it =
                    do_item_alloc(key, it->nkey, flags, old_it->exptime,
                                  it->nbytes + old_it->nbytes - 2 /* CRLF */);

                /* copy data from it and old_it to new_it */
                if (new_it == NULL ||
                    _store_item_copy_data(comm, old_it, new_it, it) == -1) {
                    failed_alloc = 1;
                    stored = NOT_STORED;
                    // failed data copy, free up.
                    if (new_it != NULL) item_remove(new_it);
                } else {
                    it = new_it;
                }
            }
...
static void init_thread_stats(thread_stats *ptr) {
    ...
    for (unsigned i = 0; i < POWER_LARGEST; ++i) ptr->lru_hits[i] = 0;
    ptr->total_items = 0;
}
...
static void init_global_stats(stats_state *ptr) {
    ...
    // std::atomic<bool>ptr->accepting_conns = 0;   whether we are currently
    // accepting
...
static void stats_init(void) {
    if (is_restart) {
        ...
        __thread_stats =
            (thread_stats *)pm_malloc(sizeof(thread_stats) * NUM_STATS);
...
void agnostic_init() {
    ...
    if (!is_restart) {
        ...
        assert(end_signal != nullptr && current_time != nullptr &&
               pause_sig != nullptr && num_lookers != nullptr);
...
static void dec_lookers(void) { --(*num_lookers); }
...
enum delta_result_type do_add_delta(const char *key, const size_t nkey,
                                    ...
                                    uint64_t *value_ptr, const uint32_t hv) {
    char *ptr;
    uint64_t value;
    int res;
    item *it;
    ...
    *value_ptr = value;
    ...
    if (res + 2 <= it->nbytes && it->refcount == 2) {
        ...
        new_it =
            do_item_alloc(ITEM_key(it), it->nkey, flags, it->exptime, res + 2);
        if (new_it == 0) {
            do_item_remove(it);
            return EOM;
        }
        ...
        if (it->refcount == 1) do_item_remove(it);
        if (incr) {
    ...
    if (incr) {
        __thread_stats[stats_id]
            .slab_stats[it->slabs_clsid]
            .incr_hits.fetch_add(1);
    } else {
        __thread_stats[stats_id]
            .slab_stats[it->slabs_clsid]
            .decr_hits.fetch_add(1);
    }
...
memcached_return_t pku_memcached_get(const char *key, size_t nkey,
    ...
    // Wentao: get doesn't need BEGIN_OP---it's OK to read things as soon as
    // they are visible Get the item using the protected key
    ...
    if (buffer == NULL) buffer = (char *)malloc(buffLen_prot);
...
memcached_return_t pku_memcached_mget(const char *const *keys,
...
memcached_return_t pku_memcached_insert(const char *key, size_t nkey,
                                        const char *data, size_t datan,
                                        uint32_t exptime) {
    // Reduce the number of allocations we have to do
    char *key_prot = (char *)pm_malloc(nkey + datan);
    if (key_prot == NULL) return MEMCACHED_MEMORY_ALLOCATION_FAILURE;
    ...
    if (it != NULL) {
        ...
        __thread_stats[stats_id].slab_stats[ITEM_clsid(it)].set_cmds.fetch_add(
            1);
...
memcached_return_t pku_memcached_set(const char *key, size_t nkey,
                                     const char *data, size_t datan,
                                     uint32_t exptime) {
    // increase # of set cmds in stats
    char *key_prot = (char *)pm_malloc(nkey + datan);
...
memcached_return_t pku_memcached_flush(uint32_t exptime) {
    pause_accesses();
...
memcached_return_t pku_memcached_delete(const char *key, size_t nkey,
                                        uint32_t exptime) {
    char *key_prot = (char *)pm_malloc(nkey);
    memcpy(key_prot, key, nkey);
    inc_lookers();
...
memcached_return_t pku_memcached_append(const char *key, size_t nkey,
                                        const char *data, size_t datan,
...
memcached_return_t pku_memcached_prepend(const char *key, size_t nkey,
                                         const char *data, size_t datan,
                                         uint32_t exptime, uint32_t flags) {
    char *key_prot = (char *)pm_malloc(nkey + datan);
    char *dat_prot = key_prot + nkey;
...
memcached_return_t pku_memcached_replace(const char *key, size_t nkey,
                                         const char *data, size_t datan,
                                         uint32_t exptime, uint32_t flags) {
    char *key_prot = (char *)pm_malloc(nkey + datan);
    char *dat_prot = key_prot + nkey;
