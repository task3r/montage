...
class DRAMHashTable : public RMap<K, V> {
   public:
    struct ListNode {
        ...
        inline K get_key() { return key; }
        inline V get_val() { return val; }
        inline void set_val(V v) { val = v; }
        ~ListNode() {}
    };
    struct Bucket {
        std::mutex lock;
...
class NVMHashTable : public RMap<std::string, std::string> {
   public:
    ...
    struct ListNode {
        ...
        inline std::string get_key() { return string(key); }
        inline std::string get_val() { return string(val); }
    ...
    ~NVMHashTable() { Persistent::finalize(); }
