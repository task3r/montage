...
class NVMSOFTHashTable : public RMap<K, V> {
    ...
    class PNode : public Persistent {
       public:
        ...
        PNode(const std::string &k, const std::string &val)
            : validStart(false), validEnd(false), deleted(false) {
    ...
    class Node {
       public:
        ...
        Node(PNode *pptr_, bool pValidity_, const std::string &k)
   ...
   public:
    ...
    NVMSOFTHashTable(GlobalTestConfig *gtc)
        ...
        for (size_t i = 0; i < idxSize; i++) {
            ...
            heads[i]->next.store(
                new Node(ptail, false, std::string(1, (char)127)),
                std::memory_order_release);
        }
    }

    ~NVMSOFTHashTable() { Persistent::finalize(); }
   ...
   private:
    ...
    optional<V> _get(Node *head, K key, int tid) {
        ...
        while (strncmp(curr->key.data(), key.data(), (size_t)TESTS_KEY_SIZE) <
               0) {
            curr = NVMsoftUtils::getRef<Node>(curr->next.load());
        }
        state currState = NVMsoftUtils::getState(curr->next.load());
        if ((strncmp(curr->key.data(), key.data(), TESTS_KEY_SIZE) == 0) &&
    ...
    retry:
        while (true) {
            ...
            if (strncmp(currRef->key.data(), key.data(), TESTS_KEY_SIZE) == 0) {
                ...
                newNode->next.store(static_cast<Node *>(NVMsoftUtils::createRef(
                                        currRef, state::INTEND_TO_INSERT)),
                                    std::memory_order_relaxed);
                if (!pred->next.compare_exchange_strong(
                        curr, static_cast<Node *>(NVMsoftUtils::createRef(
                                  newNode, predState)))) {
            ...
            while (NVMsoftUtils::getState(resultNode->next.load()) ==
                   state::INTEND_TO_INSERT)
                NVMsoftUtils::stateCAS<Node>(
    ...
    optional<V> _remove(Node *head, K key, int tid) {
        ...
        while (!casResult &&
               NVMsoftUtils::getState(currRef->next.load()) == state::INSERTED)
            casResult = NVMsoftUtils::stateCAS<Node>(
                currRef->next, state::INSERTED, state::INTEND_TO_DELETE);

        currRef->pptr->destroy(currRef->pValidity);

        while (NVMsoftUtils::getState(currRef->next.load()) ==
               state::INTEND_TO_DELETE) {
            ret = V(currRef->pptr->value.data(), TESTS_VAL_SIZE);
            NVMsoftUtils::stateCAS<Node>(currRef->next, state::INTEND_TO_DELETE,
                                         state::DELETED);
        }
        ...
        while (true) {
            ...
            if (LIKELY(cState != state::DELETED)) {
                if (UNLIKELY(strncmp(currRef->key.data(), key.data(),
                                     min((size_t)TESTS_KEY_SIZE, key.size())) >=
                             0)) {
                    break;
                }
                prev = currRef;
