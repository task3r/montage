...
inline bool NVT_CASB(ET volatile *ptr, ET oldv, ET newv) {
    bool ret;
    if (sizeof(ET) == 1) {
        ret = __sync_bool_compare_and_swap_1((bool *)ptr, *((bool *)&oldv),
                                             *((bool *)&newv));
    } else if (sizeof(ET) == 8) {
        ret = __sync_bool_compare_and_swap_8((long *)ptr, *((long *)&oldv),
                                             *((long *)&newv));
    } else if (sizeof(ET) == 4) {
        ret = __sync_bool_compare_and_swap_4((int *)ptr, *((int *)&oldv),
                                             *((int *)&newv));
    }
#if defined(MCX16)
    else if (sizeof(ET) == 16) {
        ret = __sync_bool_compare_and_swap_16(
...
inline ET NVT_CASV(ET volatile *ptr, ET oldv, ET newv) {
    ...
    if (sizeof(ET) == 1) {
        ...
        ret = (ET)__sync_val_compare_and_swap_8((long *)ptr, *((long *)&oldv),
                                                *((long *)&newv));
        ...
        ret = (ET)__sync_val_compare_and_swap_16(
            (__int128 *)ptr, *((__int128 *)&oldv), *((__int128 *)&newv));
    }
#endif
    else {
...
static long long
    flush_count[MAX_THREADS_POW2 * PMEM_STATS_PADDING];  // initialized to 0
...
#else
#error \
    "You must define what PWB is. Choose PWB_IS_CLFLUSH if you don't know what your CPU is capable of"
#endif
}
...
inline void NVT_FLUSH_STRUCT(const int tid, ET *ptr, size_t size) {
    ...
    for (uint64_t p = ((uint64_t)ptr) & NVT_CACHELINE_MASK;
         p < ((uint64_t)ptr) + size; p += 64ULL)
        // cout << p << endl;
...
#ifdef MARK_FLUSHED
    if (n->flushed) NVT_FLUSH(tid, ptr);
#else
    NVT_FLUSH(tid, ptr);
#endif
}
...
#ifdef MARK_FLUSHED
    if (ptr->flushed) NVT_FLUSH_STRUCT(tid, ptr);
#else
    NVT_FLUSH_STRUCT(tid, ptr);
#endif
}

inline void NVT_SFENCE() { asm volatile("sfence"); }
...
template <class ET, class NODE_T>
inline bool NVT_FCAS_node(const int tid, ET volatile *ptr, ET oldv, ET newv,
                          NODE_T *n) {
#ifdef MARK_FLUSHED
    __sync_fetch_and_add(&n->flushed, 1);
#endif
...
template <class ET, class NODE_T>
inline ET NVT_FCASV_node(const int tid, ET volatile *ptr, ET oldv, ET newv,
                         NODE_T *n) {
// if(!SAME_CACHELINE(ptr, n))
...
/*
    ...
    {
      if((unsigned long long) src & 15ull) cerr << src << " is not 16 byte
   aligned" << endl; unsigned __int128 result; asm volatile ("xor %%rax, %%rax;"
                    "xor %%rbx, %%rbx;"
                    "xor %%rcx, %%rcx;"
                    "xor %%rdx, %%rdx;"
                    "lock cmpxchg16b %1" : "=A"(result) : "m"(*src) : "rbx",
   "rcx"); return result;
    }*/
}  // namespace pmem_utils
