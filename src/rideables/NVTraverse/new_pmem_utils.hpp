...
template <class ET>
inline bool fence_casb_flush_fence(const int tid, ET volatile *ptr, ET oldv,
                                   ET newv) {
    bool ret;
    // implicit_fence(tid);
...
// That argument also assumes that the same value is not writen twice.
template <class ET>
inline bool fence_casb_flush(const int tid, ET volatile *ptr, ET oldv,
                             ET newv) {
    bool ret;
    // implicit_fence(tid);
    ret = NVT_CASB(ptr, oldv, newv);
...
// That argument also assumes that the same value is not writen twice.
template <class ET>
inline ET fence_casv_flush_fence(const int tid, ET volatile *ptr, ET oldv,
                                 ET newv) {
    ET ret;
    // implicit_fence(tid);
...
// link and persist makes the assumption that the same value is never written
// twice maybe having garbage collection fits the assumption as well.
template <class ET1, class ET2>
...
// link and persist makes the assumption that the same value is never written
// twice maybe having garbae colelction fits the assumption as well.
template <class ET1, class ET2>
...
inline void end_operation(const int tid) { NVT_FENCE(tid); }
}  // namespace izraelevitz_transformation

// I would recommend aliasing the namesapces before using them.
// Only use operations within the namespace. For example don't call FENCE or
// FLUSH, instead use traversal_datastructure_transformation::flush. Notice that
// izraelevitz does not have flush or fence because you should not have to use
// it. For end_operation(), it doesn't do anything special, just adds a fence,
// but you should still call it instead of putting your own fance because it
// makes the code more readable and also because we might implement some
// optimizations in the future where end_operation will become important.
// traversal_datastructure_transformation provides both a flush and flush_node
// operation. These operations are called between the travesal and the critical
// section. Since you are dealing with the linked list, you can use flush for
// everything because each node only has one field that needs to be flushed.
// When there are mutliple mutable fields per node, flush_node because more
// important. im_read doesn't do anything special, but it tells the reader that
// this is a immutable read. It should be inlined so there shouldn't be any
// function call overhead. It's up to you if you use it or not. Notice that some
// operations take pointers (ET*) while others take references (ET&) be careful
// about this when you use them.
...
/* Use this method for writes that are not part of the critical section
       ...
       For initialization, you can perform regular writes without going through
   this interface.
    */
template <class ET1, class ET2>
...
/* Use this method for writes that are not part of the critical section
   and also not part of the initialization section.
   For initialization, you can perform regular writes without going through this
   interface.
*/
template <class ET1, class ET2>
...
inline void flush_node(const int tid, NODE_T *n, size_t size) {
    ...
    for (uint64_t p = ((uint64_t)n) & NVT_CACHELINE_MASK;
         p < ((uint64_t)n) + size; p += 64ULL) {
        // std::cout << p << std::endl;
