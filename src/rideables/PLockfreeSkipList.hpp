#ifndef PLOCKFREESKIPLIST_H
#define PLOCKFREESKIPLIST_H
#include <algorithm>
#include <atomic>
#include <cstdio>
#include <cstdlib>
...
class PLockfreeSkipList : public RMap<K, V> {
   public:
    class Payload : public Persistent {
       public:
        V val;
        Payload() : Payload({}) {}
        Payload(V y) : val(y) { clwb_obj_nofence(this); }
   ...
   private:
    ...
    struct alignas(64) Node : public Persistent {
        ...
        Node(K _key, Payload *_payload, Node *_prev, Node *_next,
             unsigned long _level, bool _marker = false)
            : key(_key),
              payload(_payload),
              prev(_prev),
              next(_next),
    ...
    void dealloc_marker_node(Node *n) { delete n; }
   ...
   public:
    PLockfreeSkipList(int task_num) : tracker(task_num + 1, 100, 1000, true) {
        Persistent::init();
        int bg_tid = task_num;
        initialHeadNode = new Node(nullptr, nullptr, nullptr, 1);
        head.ptr.store(initialHeadNode);
        background_thread =
            std::move(std::thread(&PLockfreeSkipList::bg_loop, this, bg_tid));
    };
    ~PLockfreeSkipList() {
...
template <class T>
class PLockfreeSkipListFactory : public RideableFactory {
    Rideable *build(GlobalTestConfig *gtc) {
        return new PLockfreeSkipList<T, T>(gtc->task_num);
    }
};

template <class K, class V>
void PLockfreeSkipList<K, V>::bg_loop(int tid) {
...
template <class K, class V>
int PLockfreeSkipList<K, V>::bg_trav_nodes(int tid) {
    Node *prev, *node, *next;
...
template <class K, class V>
void PLockfreeSkipList<K, V>::get_index_above(Node *above_head,
                                              Node *&above_prev,
                                              Node *&above_next,
...
template <class K, class V>
int PLockfreeSkipList<K, V>::bg_raise_ilevel(int h, int tid) {
    int raised = 0;
    unsigned long zero = sl_zero.load();
...
template <class K, class V>
void PLockfreeSkipList<K, V>::bg_lower_ilevel(int tid) {
    unsigned long zero = sl_zero.load();
...
template <class K, class V>
void PLockfreeSkipList<K, V>::bg_help_remove(Node *prev, Node *node, int tid) {
    ...
    Payload *payload_ptr = node->payload.ptr.load();
    clwb_obj_nofence(node);
    if ((void *)payload_ptr != (void *)node || node->marker) return;
    ...
    clwb_obj_nofence(prev);
    if (prev_next != node || prev->marker) return;
...
template <class K, class V>
void PLockfreeSkipList<K, V>::bg_remove(Node *prev, Node *node, int tid) {
    assert(nullptr != node);
...
template <class K, class V>
int PLockfreeSkipList<K, V>::internal_finish_contains(const K &key, Node *node,
...
template <class K, class V>
int PLockfreeSkipList<K, V>::internal_finish_delete(const K &key, Node *node,
                                                    Payload *node_payload,
                                                    optional<V> &ret_value,
                                                    int tid) {
    int result = -1;
...
template <class K, class V>
int PLockfreeSkipList<K, V>::internal_finish_insert(const K &key, V &val,
                                                    Node *node,
                                                    Payload *node_payload,
                                                    Node *next,
                                                    Payload *&lazy_payload) {
...
template <class K, class V>
bool PLockfreeSkipList<K, V>::internal_do_operation(operation_type optype,
                                                    const K &key,
                                                    optional<V> &val,
                                                    optional<V> &ret_value,
                                                    int tid) {
...
template <class K, class V>
bool PLockfreeSkipList<K, V>::insert(K key, V val, int tid) {
    optional<V> unused = {};
    optional<V> val_opt = val;
    bool ret = internal_do_operation(operation_type::INSERT, key, val_opt,
                                     unused, tid);
    // sfence();
    return ret;
}
...
template <class K, class V>
optional<V> PLockfreeSkipList<K, V>::get(K key, int tid) {
    optional<V> res = {};
    optional<V> unused = {};
...
template <class K, class V>
optional<V> PLockfreeSkipList<K, V>::remove(K key, int tid) {
    optional<V> res;
    optional<V> unused = {};
...
template <class K, class V>
optional<V> PLockfreeSkipList<K, V>::put(K key, V val, int tid) {
    optional<V> res;
...
template <class K, class V>
optional<V> PLockfreeSkipList<K, V>::replace(K key, V val, int tid) {
    optional<V> res;
