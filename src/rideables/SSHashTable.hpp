...
class SSHashTable : public RMap<K, V> {
    ...
    struct Node {
        ...
        V val;
        MarkPtr next;
        Node(size_t so, K k, V v, Node *n)
            : so_k(so), key(k), val(v), next(n){};
...
bool SSHashTable<K, V>::list_find(MarkPtr *head, size_t so_k, K key, int tid) {
    while (true) {
        ...
        while (true) {
            ...
            if (prev[tid].ui->ptr.load() != curr[tid].ui) break;  // retry
            if (!cmark) {
                if (ckey > so_k) return false;
                if (ckey == so_k && curr[tid].ui->key == key) return true;
                prev[tid].ui = &(curr[tid].ui->next);
            } else {
                if (prev[tid].ui->ptr.compare_exchange_strong(curr[tid].ui,
                                                              next[tid].ui)) {
...
template <class K, class V>
optional<V> SSHashTable<K, V>::list_delete(MarkPtr *head, size_t so_k, K key,
                                           int tid) {
    optional<V> res;
    while (true) {
        ...
        res = curr[tid].ui->val;
        if (!curr[tid].ui->next.ptr.compare_exchange_strong(
                next[tid].ui, setMark(next[tid].ui))) {
            continue;
        }
        if (prev[tid].ui->ptr.compare_exchange_strong(curr[tid].ui,
                                                      next[tid].ui)) {
