...
class NatarajanTree : public RMap<K, V> {
   private:
    ...
    struct Node {
        ...
        Node(K k, V v, Node* l, Node* r, int lev)
            : level(lev), key(k), val(v), left(l), right(r){};
        Node(K k, V v, Node* l, Node* r)
            : level(-1), key(k), val(v), left(l), right(r){};
    };
    struct SeekRecord {
        Node* ancestor;
        Node* successor;
    ...
    inline bool getFlg(Node* mptr) { return (bool)((size_t)mptr & 1); }
    inline bool getTg(Node* mptr) { return (bool)((size_t)mptr & 2); }
    inline Node* mixPtrFlgTg(Node* ptr, bool flg, bool tg) {
        return (Node*)((size_t)ptr | flg | ((size_t)tg << 1));
    }
    // node comparison
    inline bool isInf(Node* n) { return getInfLevel(n) != -1; }
    ...
    inline bool nodeLessEqual(Node* n1, Node* n2) { return !nodeLess(n2, n1); }
   ...
   public:
    NatarajanTree(int task_num) : tracker(task_num, 100, 1000, true) {
        r.right.store(new Node(infK, defltV, nullptr, nullptr, 2),
                      std::memory_order_relaxed);
        r.left.store(&s, std::memory_order_relaxed);
        s.right.store(new Node(infK, defltV, nullptr, nullptr, 1),
                      std::memory_order_relaxed);
        s.left.store(new Node(infK, defltV, nullptr, nullptr, 0),
                     std::memory_order_relaxed);
...
bool NatarajanTree<K, V>::cleanup(K key, int tid) {
    ...
    res = successorAddr->compare_exchange_strong(
        successor, mixPtrFlgTg(getPtr(tmpSibling), getFlg(tmpSibling), false));
...
optional<V> NatarajanTree<K, V>::put(K key, V val, int tid) {
    ...
    Node* newLeaf =
        new Node(key, val, nullptr, nullptr);  // also to compare keys
    ...
    while (true) {
        ...
        if (!nodeEqual(newLeaf, leaf)) {  // key does not exist
            ...
            if (isInf(leaf)) {
                ...
                newInternal = new Node(std::max(key, leaf->key), defltV,
                                       newLeft, newRight);
...
bool NatarajanTree<K, V>::insert(K key, V val, int tid) {
    ...
    Node* newInternal = nullptr;
    Node* newLeaf =
        new Node(key, val, nullptr, nullptr);  // also for comparing keys
...
optional<V> NatarajanTree<K, V>::remove(K key, int tid) {
    ...
    while (true) {
        ...
        if (injecting) {
            ...
            Node* tmpExpected = getPtr(leaf);
            res = leaf->val;
            if (childAddr->compare_exchange_strong(
                    tmpExpected, mixPtrFlgTg(tmpExpected, true, false))) {
                injecting = false;
