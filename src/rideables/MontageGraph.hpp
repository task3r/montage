 ...
 */
template <int numVertices = 1024, int meanEdgesPerVertex = 20,
          int vertexLoad = 50>
class MontageGraph : public RGraph, public Recoverable {
   ...
   public:
    ...
    class Vertex : public BasePayload {
       public:
        ...
        Vertex(const Vertex& oth)
            : BasePayload(oth), m_id(oth.m_id), m_lbl(oth.m_lbl) {
            this->m_tag = 0;
        }
    ...
    class Relation : public BasePayload {
       public:
        ...
        Relation(int src, int dest, int weight)
            : m_weight(weight), m_src(src), m_dest(dest) {
            this->m_tag = 1;
        }
        Relation(const Relation& oth)
            : BasePayload(oth),
              m_weight(oth.m_weight),
              m_src(oth.m_src),
              m_dest(oth.m_dest) {
            this->m_tag = 1;
        }
        int src() const { return m_src; }
    ...
    struct RelationEqual {
        bool operator()(const pair<int, int>& r1,
                        const pair<int, int>& r2) const {
            return r1.first == r2.first && r1.second == r2.second;
        }
    };

    using Map = std::unordered_map<pair<int, int>, Relation*, RelationHash,
                                   RelationEqual>;
    ...
    class alignas(64) tVertex {
       public:
        ...
        Map dest_list;  // relations in this list is a duplication of those in
                        // some adjacency list
        ...
        tVertex(MontageGraph* ds_, Vertex* p) : ds(ds_) {
            // Use this method for recovery to avoid having to call PNEW when
        ...
        void set_lbl(int l) { payload = payload->set_lbl(ds, l); }
        int get_lbl() { return payload->get_lbl(ds); }
    ...
    struct alignas(64) VertexMeta {
        tVertex*
            idxToVertex;  // Transient set of transient vertices to index map
        std::mutex vertexLocks;  // Transient locks for transient vertices
        uint32_t vertexSeqs;     // Transient sequence numbers for transactional
    ...
    MontageGraph(GlobalTestConfig* gtc) : Recoverable(gtc), gtc(gtc) {
        ...
        MontageOpHolder _holder(
            this);  // clearing pending_allocs and persisting payloads
        if (gtc->verbose)
    ...
    std::tuple<int, int, double, int*, int> grab_stats() {
        ...
        return std::make_tuple(numV, numE, averageEdgeDegree, degrees,
                               numVertices);
    }
    ...
    void clear() {
        ...
        //     std::vector<Relation*> toDelete(source(i).size() +
        //     destination(i).size()); for (auto r : source(i))
        //     toDelete.push_back(r); for (auto r : destination(i))
        //     toDelete.push_back(r); source(i).clear(); destination(i).clear();
    ...
    int recover(bool simulated) {
        ...
        {
            MontageOpHolder _holder(this);
            for (auto itr = recovered->begin(); itr != recovered->end();
                 ++itr) {
                // iterate through all recovered blocks.  Sort the blocks into
                // vectors containing the different payloads to be iterated over
                // later.
                block_cnt++;
                ...
                switch (b->get_unsafe_tag(this)) {
                    ...
                    default: {
                        std::cerr << "Found bad tag " << b->get_unsafe_tag(this)
                                  << std::endl;
                    }
                }
            }
        ...
        dur_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(dur).count();
        std::cout << "Spent " << dur_ms << "ms gathering vertices("
                  << vertexVector.size() << ") and edges("
                  << relationVector.size() << ")..." << std::endl;
        begin = chrono::high_resolution_clock::now();
        ...
        for (int rec_tid = 0; rec_tid < rec_thd; rec_tid++) {
            workers.emplace_back(std::thread([&, rec_tid]() {
                ...
                // Recover vertexes:
                for (size_t i = rec_tid; i < vertexVector.size();
                     i += rec_thd) {
                ...
                if (rec_tid == 0) {
                    ...
                    dur_ms =
                        std::chrono::duration_cast<std::chrono::milliseconds>(
                            dur)
                            .count();
                    std::cout << "Spent " << dur_ms << "ms creating vertices..."
                              << std::endl;
                    begin = chrono::high_resolution_clock::now();
                }

                // Recover relationships:
                // Vertices are cyclically distributed across all threads, such
                // that a thread t_i owns vertices i, i + T, i + 2T, etc., where
                // T is the number of threads. The vector of edges is iterated
                // over in parallel and given an edge e=(v,v'), if v belongs to
                // thread t_i, t_i will add it to the source set of v; if v'
                // belongs to thread t_i, t_i will add v' to the destination set
                // of v'; if t_i does not own it, nothing happens
                ...
                for (size_t i = rec_tid; i < relationVector.size();
                    ...
                    buffers[rec_tid * rec_thd + (id1 % rec_thd)].push_back(
                        item);
                    if ((id2 % rec_thd) != (id1 % rec_thd)) {
                        buffers[rec_tid * rec_thd + (id2 % rec_thd)].push_back(
                            item);
                    }
                }
    ...
    startOver : {
            ...
            if (vertex(_vid) == nullptr && get_seq(vid) == seq) {
                for (auto r : source(vid)) {
                    if (r.second->dest() == _vid)
                        std::cout << "(" << r.second->src() << ","
                                  << r.second->dest() << ")" << std::endl;
                }
                for (auto r : destination(vid)) {
        ...
        if (get_seq(vid) != seq) {
            for (auto _vid = vertices.rbegin(); _vid != vertices.rend();
                 _vid++) {
                unlock(*_vid);
            }
            goto startOver;
        ...
        // vertices that relate to this vertex
        {
            ...
            for (int other : vertices) {
                ...
                if (!has_relation(source(other), src) &&
                    !has_relation(destination(other), dest)) {
                    std::cout
                        << "Observed pair (" << vid << "," << other
                        << ") that was originally there but no longer is..."
                        << std::endl;
                    for (auto r : source(vid)) {
                        if (r.second->dest() == other)
                            std::cout << "Us: (" << r.second->src() << ","
                                      << r.second->dest() << ")" << std::endl;
                    }
                    for (auto r : destination(other)) {
                        if (r.second->src() == vid) {
                            std::cout << "Them: (" << r.second->src() << ","
                                      << r.second->dest() << ")" << std::endl;
                        }
                    }
                    for (auto r : destination(vid)) {
                ...
                auto ret1 =
                    remove_relation(source(other), src);  // this may fail
                auto ret2 =
                    remove_relation(destination(other), dest);  // this may fail
                if (ret1 != nullptr) {
                    pdelete(
                        ret1);  // only deallocate relation removed from source
                }
                assert(!has_relation(source(other), src) &&
                       !has_relation(destination(other), dest));
            }
   ...
   private:
    tVertex*& vertex(size_t idx) { return vMeta[idx].idxToVertex; }

    void lock(size_t idx) { vMeta[idx].vertexLocks.lock(); }

    void unlock(size_t idx) { vMeta[idx].vertexLocks.unlock(); }

    // Lock must be owned for next operations...
    void inc_seq(size_t idx) { vMeta[idx].vertexSeqs++; }

    uint64_t get_seq(size_t idx) { return vMeta[idx].vertexSeqs; }
    ...
    // Incoming edges
    Map& source(int idx) { return vertex(idx)->adjacency_list; }

    // Outgoing edges
    Map& destination(int idx) { return vertex(idx)->dest_list; }
...
template <size_t numVertices = 1024, size_t meanEdgesPerVertex = 20,
          size_t vertexLoad = 50>
class MontageGraphFactory : public RideableFactory {
    Rideable* build(GlobalTestConfig* gtc) {
        return new MontageGraph<numVertices, meanEdgesPerVertex, vertexLoad>(
            gtc);
    }
};
