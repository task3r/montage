...
class PNatarajanTree : public RMap<std::string, std::string> {
   ...
   public:
    ...
    ~PNatarajanTree() { Persistent::finalize(); }
    ...
    seek_record_t* bst_seek(const int tid, const std::string& key,
        ...
        node_t* node_s =
            ADDRESS(iz::load(tid, node_r->left));  // root is immutable
        ...
        node_t* parent_field =
            (node_t*)iz::load_acq(tid, seek_record_l.parent->left);
        node_t* current_field =
            (node_t*)iz::load_acq(tid, seek_record_l.leaf->left);
    ...
    */
...
#endif
            node_t* result =
                iz::casv(tid, child_addr, ADDRESS(leaf), ADDRESS(new_internal));
    ...
    optional<std::string> remove(std::string key, const int tid) {
        ...
        while (1) {
            ...
            if (injecting == TRUE) {
                ...
                node_t* result =
                    iz::casv(tid, child_addr, lf, (node_t*)FLAG(lf));
                if (result == ADDRESS(leaf)) {
                    injecting = FALSE;
    ...
    bool_t bst_cleanup(const int tid, const std::string& key) {
        ...
        if (iz::casv(tid, succ_addr, ADDRESS(successor),
...
class PNatarajanTreeFactory : public RideableFactory {
    Rideable* build(GlobalTestConfig* gtc) { return new PNatarajanTree(gtc); }
};
