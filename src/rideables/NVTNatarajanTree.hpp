...
class NVTNatarajanTree : public RMap<std::string, std::string> {
   ...
   public:
    ...
    ~NVTNatarajanTree() { Persistent::finalize(); }
    ...
    void init_thread(GlobalTestConfig* gtc, LocalTestConfig* ltc) {
        ...
        // seek_record_tr = (seek_record_traverse*) malloc
        // (sizeof(seek_record_traverse));
        seek_record_tr = new seek_record_traverse();
        assert(seek_record_tr != NULL);
    }
    //===========================================

    node_t* create_node(const int tid, const std::string& k,
                        const std::string& value, int initializing) {
        node_t* new_node;
        // #if GC == 1
    ...
    node_t* create_node_initial(const int tid, const std::string& k,
                                const std::string& value, int initializing) {
        node_t* new_node;
        // new_node = (node_t*) malloc(sizeof(node_t));
        new_node = new node_t();
    ...
    seek_record_traverse* bst_seek(const int tid, const std::string& key,
                                   node_t* node_r) {
        // PARSE_TRY();
        ...
        while (current != NULL) {
            if (!GETTAG(parent_field)) {
                seek_record_l.ancestor_grandparent =
                    seek_record_l.ancestor_parent;
        ...
        seek_record_tr->ancestor_grandparent =
            seek_record_l.ancestor_grandparent;
    ...
    */
    void free_path(const int tid, const std::string& key, node_t* node_start,
                   node_t* node_stop) {
        ...
        while (current != node_stop) {
            if (current == NULL)
                return;  // this is a safety check, current should never be NULL
    ...
    optional<std::string> get(std::string key, int tid) {
        ...
        if (seek_record_tr->grandparent)
            tr::flush_node(tid, seek_record_tr->grandparent);
    ...
    /*
        ...
        traversal for flushing because all the nodes between successor
       (node_start) and parent (node_stop) have been marked meaning that they
       cannot change.

        flush_path flushes all nodes in the interval [node_start, node_stop)
    */
    void flush_path(const int tid, const std::string& key, node_t* node_start,
                    node_t* node_stop) {
        node_t* current = node_start;
        node_t* current_field = NULL;
    ...
    bool insert(std::string key, std::string val, int tid) {
        ...
        while (1) {
            ...
            if (seek_record_tr->ancestor_grandparent)
                tr::flush_node(tid, seek_record_tr->ancestor_grandparent);
            if (seek_record_tr->ancestor_parent)
                tr::flush_node(tid, seek_record_tr->ancestor_parent);

            // make_persistent
            tr::flush_node(tid, seek_record_tr->ancestor);
            flush_path(tid, key, seek_record_tr->successor,
                       seek_record_tr->parent);
            tr::flush_node(tid, seek_record_tr->parent);
            tr::flush_node(tid, seek_record_tr->leaf);
            ...
            if (/*likely*/ (created == 0)) {
                // new_internal=create_node(tid, max(key,tr::im_read(tid,
                // leaf->key)),0,0);
            ...
            MEM_BARRIER;
#endif
            node_t* result = tr::cr_casv(tid, child_addr, ADDRESS(leaf),
                                         ADDRESS(new_internal));
    ...
    optional<std::string> remove(std::string key, const int tid) {
        ...
        while (1) {
            ...
            if (injecting == TRUE) {
                ...
                node_t* result =
                    tr::cr_casv(tid, child_addr, lf, (node_t*)FLAG(lf));
                if (result == ADDRESS(leaf)) {
                    injecting = FALSE;
                    ...
                    node_t* chld = tr::cr_read(tid, *child_addr);
                    if ((ADDRESS(chld) == leaf) &&
                        (GETFLAG(chld) || GETTAG(chld))) {
                        bst_cleanup(tid, key);
                    }
                }
            } else {
    ...
    bool_t bst_cleanup(const int tid, const std::string& key) {
        ...
        if (tr::cr_casv(tid, succ_addr, ADDRESS(successor),
                        (node_t*)UNTAG(sibl)) == (node_t*)ADDRESS(successor)) {
            // #if GC == 1
    ...
    optional<std::string> replace(std::string key, std::string val,
                                  const int tid) {
...
class NVTNatarajanTreeFactory : public RideableFactory {
    Rideable* build(GlobalTestConfig* gtc) { return new NVTNatarajanTree(gtc); }
};
