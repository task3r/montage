...
class MontageNatarajanTree : public RMap<K, V>, public Recoverable {
    ...
    class Payload : public pds::PBlk {
       ...
       public:
        ...
        Payload(const Payload& oth)
            : PBlk(oth), m_key(oth.m_key), m_val(oth.m_val) {}
        void persist() {}
    };
   ...
   private:
    ...
    struct Node {
        ...
        Node(MontageNatarajanTree* ds_, K k, V val, Node* l = nullptr,
             Node* r = nullptr)
            : ds(ds_),
              level(finite),
              left(l),
              right(r),
              key(k),
              payload(ds_->pnew<Payload>(key, val)){};
        Node(MontageNatarajanTree* ds_, Level lev, Node* l = nullptr,
             Node* r = nullptr)
            : ds(ds_), level(lev), left(l), right(r), key(), payload(nullptr) {
            assert(
                lev != finite &&
                "use constructor with another signature for concrete nodes!");
        };
        ~Node() {
            if (payload != nullptr) {
                // this is a leaf
        ...
        V get_unsafe_val() { return (V)payload->get_unsafe_val(ds); }
        // not thread-safe
        ...
        // not thread-safe
        void set(Recoverable* ds, Level lev, Node* l = nullptr,
                 Node* r = nullptr) {
            level = lev;
            left.store(ds, l);
            right.store(ds, r);
    ...
    const size_t GET_POINTER_BITS =
        0xfffffffffffffffc;  // for machine 64-bit or less.
    ...
    inline bool getFlg(Node* mptr) { return (bool)((uint64_t)mptr & 1); }
    inline bool getTg(Node* mptr) { return (bool)((uint64_t)mptr & 2); }
    ...
    inline bool isInf(Node* n) { return getInfLevel(n) != finite; }
    ...
    // void doRangeQuery(Node& k1, Node& k2, int tid, Node* root, std::map<K,V>&
    // res);
   public:
...
optional<V> MontageNatarajanTree<K, V>::get(K key, int tid) {
    ...
    if (nodeEqual(key, leaf)) {
        ...
        res = leaf->get_unsafe_val();  // never old see new as we find node
                                       // before BEGIN_OP
    }
    ...
    while (true) {
        ...
        if (!nodeEqual(key, leaf)) {  // key does not exist
            ...
            if (isInf(leaf)) {
                ...
                newInternal->set(this, std::max(key, leaf->key), newLeft,
                                 newRight);
            ...
            if (childAddr->CAS_verify(this, tmpExpected, getPtr(newInternal))) {
                ...
                Node* tmpChild = childAddr->load(this);
                if (getPtr(tmpChild) == leaf &&
                    (getFlg(tmpChild) || getTg(tmpChild)))
                    cleanup(key, tid);
            }
            ...
            if (childAddr->CAS_verify(this, leaf, newLeaf)) {
                delete (newInternal);  // this is always local so no need to use
                                       // tracker
...
bool MontageNatarajanTree<K, V>::insert(K key, V val, int tid) {
    ...
    while (true) {
        ...
        if (!nodeEqual(key, leaf)) {  // key does not exist
            ...
            delete (
                newInternal);  // this is always local so no need to use tracker
    ...
    while (true) {
        ...
        if (injecting) {
            ...
            if (childAddr->CAS(this, tmpExpected,
                 ...
                 * before the leaf is found which is of course before that
                 * BEGIN_OP
                 */
                injecting = false;
...
optional<V> MontageNatarajanTree<K, V>::replace(K key, V val, int tid) {
    ...
    //         delete(newLeaf->payload);// payload is useless, directly
    //         reclaiming delete(newLeaf);// this is always local so no need to
    //         use tracker res={}; break;
    //     }
    //     else{//key exists, update and return old
    //         if(nodeLess(key,parent))
    //             childAddr=&(parent->left);
...
template <>
class MontageNatarajanTree<std::string, std::string>::Payload
    : public pds::PBlk {
