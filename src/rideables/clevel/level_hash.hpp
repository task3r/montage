...
#include "clevel/compound_pool_ptr.hpp"
#include "clevel/hash.hpp"
...
#if !LIBPMEMOBJ_CPP_USE_TBB_RW_MUTEX
using concurrent_hash_map_internal::shared_mutex_scoped_lock;
#endif
...
class level_hash {
   public:
    ...
    // protected:
    hv_type first_hash(const key_type &key) const {
#ifdef KEY_LEN
    ...
    hv_type second_hash(const key_type &key) const {
#ifdef KEY_LEN
    ...
    difference_type second_index(hv_type hv, size_type capacity) const {
    ...
    struct barrier {
        ...
        void cross(level_hash *ht, size_type thread_id) {
            pthread_mutex_lock(&mutex);
            crossing++;
            if (crossing < count) {
                std::cout << "[Thread-" << thread_id << "] adds crossing to "
                          << crossing << std::endl;
                pthread_cond_wait(&complete, &mutex);
                std::cout << "[Thread-" << thread_id << "] crosses barrier"
                          << std::endl;
            } else {
                std::cout << "[Thread-" << thread_id
                          << "] expands, crossing = " << crossing << std::endl;
                ht->expand();
                pthread_cond_broadcast(&complete);
                std::cout << "[Thread-" << thread_id << "] broadcasts"
                          << std::endl;
                crossing = 0;
            }
    ...
    struct level {
        ...
        void clear() {
            ...
            if (mutexes) {
                ...
                delete_persistent<persistent_ptr<mutex_t[assoc_num]>[]>(
                    mutexes, capacity);
            }
        }
    };
    ...
    level_hash(size_type level_power, size_type n_threads)
        ...
        std::cout << "level_hash constructor, level_size = " << level_power
                  << std::endl;
        ...
        for (size_t i = 0; i < 2; i++) {
            ...
            levels[i]->mutexes =
                make_persistent<persistent_ptr<mutex_t[assoc_num]>[]>(
                    levels[i]->capacity);
    ...
    // bool
    ret insert(const value_type &value, size_type thread_id) {
        return generic_insert(value.first, &value, allocate_KV_copy_construct,
                              thread_id);
    }

    ret insert(value_type &&value, size_type thread_id) {
    ...
    ret generic_insert(const key_type &key, const void *param,
        ...
        while (true) {
            if (need_resizing.load()) resize_barrier.cross(this, thread_id);

#ifdef DEBUG_RESIZING
            if (initial_capacity == 0) initial_capacity = capacity();
#endif
            ...
            if (level_expand_time > 0) {
                // f_idx and s_idx were calculated using the capacity of bottom
                // level
    ...
    // mapped_type
    ret query(const key_type &key, size_type thread_id) {
        if (need_resizing.load()) resize_barrier.cross(this, thread_id);
        ...
        for (size_type i = 0; i < 2; i++) {
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                ...
                if (levels[i]->buckets[f_idx].tokens[j] == 1 &&
                    ...
                    pop.persist(&levels[i]->buckets[f_idx].tokens[j],
                                sizeof(uint8_t));
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                ...
                if (levels[i]->buckets[s_idx].tokens[j] == 1 &&
                    ...
                    pop.persist(&levels[i]->buckets[s_idx].tokens[j],
                                sizeof(uint8_t));
        ...
        for (size_type i = 0; i < 2; i++) {
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                ...
                if (levels[i]->buckets[f_idx].tokens[j] == 1 &&
                    ...
                    pop.persist(&levels[i]->buckets[f_idx].slots[j].off,
                                sizeof(kv_ptr_t));
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                ...
                if (levels[i]->buckets[s_idx].tokens[j] == 1 &&
                    ...
                    pop.persist(&levels[i]->buckets[s_idx].slots[j].off,
                                sizeof(kv_ptr_t));
    ...
    void expand() {
...

        {
            ...
            interim_level->buckets =
                make_persistent<bucket[]>(interim_level->capacity);
        ...
        for (difference_type old_idx = 0;
             old_idx < static_cast<difference_type>(levels[1]->capacity);
             old_idx++) {
            for (size_type i = 0; i < assoc_num; i++) {
                if (levels[1]->buckets[old_idx].tokens[i] == 1) {
                    const key_type &key = levels[1]
                                              ->buckets[old_idx]
                                              .slots[i]
                                              .get_address(my_pool_uuid)
                                              ->first;
                    ...
                    difference_type f_idx =
                        first_index(first_hash(key), interim_level->capacity);
                    difference_type s_idx =
                        second_index(second_hash(key), interim_level->capacity);
                    ...
                    for (size_type j = 0; j < assoc_num; j++) {
                        if (interim_level->buckets[f_idx].tokens[j] == 0) {
                            insert_empty_slot(
                                pop, 2, f_idx, j,
                                levels[1]->buckets[old_idx].slots[i]);
                            succ = true;
                            break;
                        }
                    ...
                    pop.persist(&levels[1]->buckets[old_idx].tokens[i],
                                sizeof(uint8_t));
                }
            }
        }
    ...
    void clear() { std::cout << "level destroy!" << std::endl; }

    ~level_hash() { clear(); }

    void foo() { std::cout << "level_hash::foo()" << std::endl; }
    ...
    void insert_empty_slot(pool_base &pop, size_type level_idx,
        ...
        for (size_type i = 0; i < assoc_num; i++) {
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                ...
                if (levels[level_idx]->buckets[jdx].tokens[j] == 0) {
                    ...
                    pop.persist(
                        &levels[level_idx]->buckets[bucket_idx].tokens[i],
                        sizeof(uint8_t));
    ...
    bool b2t_movement(pool_base &pop, difference_type bucket_idx, kv_ptr_t e) {
        for (size_type i = 0; i < assoc_num; i++) {
            scoped_t src_guard(levels[1]->mutexes[bucket_idx][i], true);
            assert(levels[1]->buckets[bucket_idx].tokens[i] == 0);

            const key_type &m_key = levels[1]
                                        ->buckets[bucket_idx]
                                        .slots[i]
                                        .get_address(my_pool_uuid)
                                        ->first;
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                ...
                if (levels[0]->buckets[f_idx].tokens[j] == 0) {
                    ...
                    pop.persist(&levels[1]->buckets[bucket_idx].tokens[i],
                                sizeof(uint8_t));
    ...
    uint64_t capacity() {
        ...
        if (interim_level) n_buckets += interim_level->capacity;
