...
#include "clevel/concurrent_hash_map.hpp"
...
class clevel_hash {
   public:
    ...
    struct ret {
        ...
        ret(bool _found, KV_entry_ptr_t _ptr)
            : found(_found),
              ptr(_ptr),
              level_idx(0),
              bucket_idx(0),
    ...
    void set_thread_num(size_type num) {
        if (thread_num > 0) {
            ...
            delete_persistent<persistent_ptr<level_meta>[]>(tmp_meta,
                                                            thread_num);
            delete_persistent<persistent_ptr<level_bucket>[]>(tmp_level,
                                                              thread_num);
            delete_persistent<persistent_ptr<value_type>[]>(tmp_entry,
                                                            thread_num);
        }
    ...
    f_code_t find(pool_base &pop, const key_type &key, partial_t partial,
                  ...
                  KV_entry_ptr_t **e, uint64_t &level_num, difference_type &idx,
                  bool fix_dup, size_type thread_id, level_meta_ptr_t &m_copy);
        ...
        do {
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                if (f_b.slots[j].x.partial == partial &&
                    ...
                    if (key_equal{}(
                            f_b.slots[j].p.get_address(my_pool_uuid)->first,
                            key)) {
                        return ret(true, f_b.slots[j].p);
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                if (s_b.slots[j].x.partial == partial &&
                    ...
                    if (key_equal{}(
                            s_b.slots[j].p.get_address(my_pool_uuid)->first,
                            key)) {
                        return ret(true, s_b.slots[j].p);
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::generic_insert(
    ...
    RETRY_INSERT:
        ...
        f_code_t result;
        if (check_duplicate) {
            result = find(pop, key, partial, n_levels, old_e, &e, level_num,
                          idx, /*fix_dup=*/false, thread_id, m_copy);
        } else {
            result = find_empty_slot(pop, key, partial, n_levels, &e, level_num,
                                     m_copy);
        }
        ...
        if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT) {
            ...
            if (check_duplicate) {  // check_duplicate is set to false when
                                    // worker thread has inserted the KV
                                    // successfully
                return ret();
            } else {
                return ret(true);
        ...
        do {
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                ...
                if (tmp.x.partial == partial && tmp.p.get_offset() != 0) {
                    if (key_equal{}(tmp.p.get_address(my_pool_uuid)->first,
                                    key)) {
                        kv_ptr = tmp.p;
                        if (CAS(&(f_b.slots[j].p.off), tmp.p.off, 0)) {
                            pop.persist(&(f_b.slots[j].p.off),
                                        sizeof(uint64_t));
                            succ_deletion = true;
                            ...
                            // Instead of redoing the delete to guarantee the
                            // deletion is successful, we apply context checking
                            // to avoid unnecessary re-executions. The deletion
                            // fails only when the item to be deleted is copied
                            // by rehashing threads after checking and before
                            // deletion's CAS. Therefore, we can do context
                            // checking to avoid such failures.
                            if (m_copy != meta ||
                                (i == 0 && f_idx <= expand_bucket &&
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::generic_update(
    ...
    while (true) {
        ...
        f_code_t result =
            find(pop, key, partial, n_levels, old_e, &e, level_num, idx,
    ...
    if (cl->up == nullptr) {
        ...
        while (true) {
            ...
            if (CAS(&(meta.off), m_copy.off, tmp_meta[t_id].raw().off)) {
                ...
                // std::cout << "Thread-" << thread_id << " finishes expanding,
        ...
        if (meta == m_copy) {
            ...
            while (true) {
                ...
                if (cl->capacity >= new_capacity) {
                    ...
                    assert(cl->up != nullptr);
                    make_persistent_atomic<level_meta>(
                        pop, tmp_meta[t_id], cl->up, m->last_level, true);
                }

                if (CAS(&(meta.off), m_copy.off, tmp_meta[t_id].raw().off)) {
                    pop.persist(&(meta.off), sizeof(uint64_t));
                    // std::cout << "Thread-" << thread_id << " finishes
                    // expanding, capacity: " << capacity() << std::endl;
                    break;
                } else {
                    m_copy = level_meta_ptr_t(meta);
                    m = static_cast<level_meta *>(m_copy(my_pool_uuid));
            ...
            for (size_type slot_idx = 0; slot_idx < assoc_num; slot_idx++) {
                ...
                for (size_type j = 0; j < assoc_num; j++) {
                    ...
                    if (dst_tmp.get_offset() == 0) {
                        if (CAS(&(dst_b1.slots[j].p.off), dst_tmp.raw(),
                                src_tmp.raw())) {
                    ...
                    if (dst_tmp.get_offset() == 0) {
                        if (CAS(&(dst_b2.slots[j].p.off), dst_tmp.raw(),
                                src_tmp.raw())) {
