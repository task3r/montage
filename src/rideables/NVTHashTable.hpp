...
class NVTHashTable : public RMap<std::string, std::string> {
   ...
   public:
    ...
    ~NVTHashTable() { Persistent::finalize(); };
...
optional<std::string> NVTHashTable::put(std::string key, std::string val,
                                        int tid) {
        ...
        if (findNode(prev, curr, next, key, tid)) {
            ...
            if (prev->ptr.compare_exchange_strong(curr, tmpNode)) {
                ...
                while (!curr->next.ptr.compare_exchange_strong(next,
                                                               setMark(next))) {
                    clwb_fence(
                        &curr->next);  // flush after CAS, fence before CAS.
                }
                clwb_fence(&prev->ptr);  // flush after CAS, fence before CAS.
            ...
            tmpNode->next.ptr.store(curr);
            clwb_obj_fence(
                &tmpNode->next);  // flush after write, fence before CAS.
            if (prev->ptr.compare_exchange_strong(curr, tmpNode)) {
...
optional<std::string> NVTHashTable::replace(std::string key, std::string val,
                                            int tid) {
    Node* tmpNode = nullptr;
    MarkPtr* prev = nullptr;
    Node* curr = nullptr;
    ...
    while (true) {
        if (findNode(prev, curr, next, key, tid)) {
            ...
            if (prev->ptr.compare_exchange_strong(curr, tmpNode)) {
                ...
                // it's ok to have duplicated keys temporarily
                clwb_obj_fence(
                    &prev->ptr);  // flush after CAS, fence before CAS.
                while (!curr->next.ptr.compare_exchange_strong(next,
                                                               setMark(next)))
                    ;
                clwb_obj_fence(
...
bool NVTHashTable::findNode(MarkPtr*& prev, Node*& curr, Node*& next,
