...
class ProntoHashTable : public PersistentObject,
                        public RMap<std::string, std::string> {
    typedef char *T;
   ...
   public:
    struct ListNode {
        ...
        ListNode(std::string k = "", std::string v = "", ListNode *n = nullptr)
            : next(n) {
    ...
    // return true if key doesn't exist; false if it exists and val will not be
    // replaced
    ...
    static ProntoHashTable *Factory(uuid_t id) {
        ...
        manager.lock();
        ProntoHashTable *obj = (ProntoHashTable *)manager.findRecovered(id);
        if (obj == NULL) {
