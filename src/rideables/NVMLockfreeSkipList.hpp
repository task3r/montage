#ifndef NVM_LOCKFREESKIPLIST_H
#define NVM_LOCKFREESKIPLIST_H
#include <algorithm>
#include <atomic>
#include <cstdio>
#include <cstdlib>
...
class NVMLockfreeSkipList : public RMap<K, V> {
   public:
   ...
   private:
    ...
    void dealloc_marker_node(Node *n) {
        assert((void *)n->payload.ptr.load() == (void *)n);
        delete n;
    }
   ...
   public:
    NVMLockfreeSkipList(int task_num) : tracker(task_num + 1, 100, 1000, true) {
        Persistent::init();
        int bg_tid = task_num;
        initialHeadNode = new Node(nullptr, nullptr, nullptr, 1);
        head.ptr.store(initialHeadNode);
        background_thread =
            std::move(std::thread(&NVMLockfreeSkipList::bg_loop, this, bg_tid));
    };
    ~NVMLockfreeSkipList() {
...
template <class T>
class NVMLockfreeSkipListFactory : public RideableFactory {
    Rideable *build(GlobalTestConfig *gtc) {
        return new NVMLockfreeSkipList<T, T>(gtc->task_num);
    }
};

template <class K, class V>
void NVMLockfreeSkipList<K, V>::bg_loop(int tid) {
...
template <class K, class V>
int NVMLockfreeSkipList<K, V>::bg_trav_nodes(int tid) {
    Node *prev, *node, *next;
    ...
    while (nullptr != next) {
        bool expected = false;
        if (nullptr == node->payload.ptr.load()) {
            bg_remove(prev, node, tid);
            if (node->level >= 1) ++bg_tall_deleted;
            ++bg_deleted;
        } else if ((void *)node->payload.ptr.load() != (void *)node) {
        ...
        if (nullptr != node->payload.ptr.load() &&
            (void *)node != (void *)node->payload.ptr.load()) {
            ++bg_non_deleted;
        }
        prev = node;
        node = next;
...
template <class K, class V>
void NVMLockfreeSkipList<K, V>::get_index_above(Node *above_head,
                                                Node *&above_prev,
                                                Node *&above_next,
...
template <class K, class V>
int NVMLockfreeSkipList<K, V>::bg_raise_ilevel(int h, int tid) {
    int raised = 0;
    unsigned long zero = sl_zero.load();
    ...
    while (nullptr != (inext = index->succs[idx(h - 1, zero)].ptr.load())) {
        while ((void *)index->payload.ptr.load() == (void *)index) {
            // skip deleted nodes
        ...
        if (((((int)iprev->level <= h) && ((int)index->level == h)) &&
             ((int)inext->level <= h)) &&
            ((void *)index->payload.ptr.load() != (void *)index &&
             nullptr != index->payload.ptr.load())) {
            raised = 1;
...
template <class K, class V>
void NVMLockfreeSkipList<K, V>::bg_lower_ilevel(int tid) {
    unsigned long zero = sl_zero.load();
...
template <class K, class V>
void NVMLockfreeSkipList<K, V>::bg_help_remove(Node *prev, Node *node,
                                               int tid) {
    ...
    if ((void *)node->payload.ptr.load() != (void *)node || node->marker)
        return;
    ...
    if (prev->next.ptr.load() != node || prev->marker) return;
...
template <class K, class V>
void NVMLockfreeSkipList<K, V>::bg_remove(Node *prev, Node *node, int tid) {
    assert(nullptr != node);

    if (0 == node->level) {
        /* only remove short nodes */

        /* CAS expects a reference as the first argument, so we create a
         * temporary variable as a shim. */
        Payload *tmp = nullptr;
        node->payload.ptr.compare_exchange_strong(tmp, (Payload *)node);
        if ((void *)node->payload.ptr.load() == (void *)node)
            bg_help_remove(prev, node, tid);
    }
}

template <class K, class V>
int NVMLockfreeSkipList<K, V>::internal_finish_contains(
    const K &key, Node *node, Payload *node_payload, optional<V> &ret_value) {
    int result = 0;
...
template <class K, class V>
int NVMLockfreeSkipList<K, V>::internal_finish_delete(const K &key, Node *node,
    ...
    if (node->key != key)
        ...
        if (nullptr != node_payload) {
            ...
            while (1) {
                ...
                if (nullptr == node_payload ||
                    (void *)node == (void *)node_payload) {
                    result = 0;
                    break;
                } else {
                    if (node->payload.ptr.compare_exchange_strong(
                            node_payload, (Payload *)nullptr)) {
                        ret_value = V(node_payload->val);
...
template <class K, class V>
int NVMLockfreeSkipList<K, V>::internal_finish_insert(const K &key, V &val,
                                                      Node *node,
                                                      Payload *node_payload,
                                                      Node *next,
                                                      Payload *&lazy_payload) {
    ...
    if (lazy_payload == nullptr) {
        lazy_payload = new Payload(val);
    }
    if (node->key == key) {
        if (nullptr == node_payload) {
            if (node->payload.ptr.compare_exchange_strong(node_payload,
                                                          lazy_payload))
                result = 1;
        } else {
            result = 0;
...
template <class K, class V>
bool NVMLockfreeSkipList<K, V>::internal_do_operation(operation_type optype,
                                                      const K &key,
                                                      optional<V> &val,
                                                      optional<V> &ret_value,
                                                      int tid) {
...
template <class K, class V>
bool NVMLockfreeSkipList<K, V>::insert(K key, V val, int tid) {
    optional<V> unused = {};
...
template <class K, class V>
optional<V> NVMLockfreeSkipList<K, V>::get(K key, int tid) {
    optional<V> res = {};
    optional<V> unused = {};
...
template <class K, class V>
optional<V> NVMLockfreeSkipList<K, V>::remove(K key, int tid) {
    optional<V> res;
    optional<V> unused = {};
...
template <class K, class V>
optional<V> NVMLockfreeSkipList<K, V>::put(K key, V val, int tid) {
    optional<V> res;
...
template <class K, class V>
optional<V> NVMLockfreeSkipList<K, V>::replace(K key, V val, int tid) {
    optional<V> res;
...
template <>
class NVMLockfreeSkipList<std::string, std::string>::Payload
    : public Persistent {
   public:
