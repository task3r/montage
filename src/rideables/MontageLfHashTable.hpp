...
class MontageLfHashTable : public RMap<K, V>, public Recoverable {
   ...
   private:
    ...
    struct Node {
        ...
        Node(Payload* _payload)
            : payload(_payload),
              key(_payload->get_unsafe_key(ds)) {}  // for recovery
   ...
   public:
    MontageLfHashTable(GlobalTestConfig* gtc)
        : Recoverable(gtc), tracker(gtc->task_num, 100, 1000, true), gtc(gtc){};
    ~MontageLfHashTable(){};
...
optional<V> MontageLfHashTable<K, V, idxSize>::get(K key, int tid) {
    ...
    if (findNode(prev, curr, next, key, tid)) {
        ...
        res = curr->get_unsafe_val();  // never old see new as we find node
                                       // before BEGIN_OP
    }
    tracker.end_op(tid);
...
template <class K, class V, int idxSize>
bool MontageLfHashTable<K, V, idxSize>::findNode(MarkPtr*& prev, Node*& curr,
