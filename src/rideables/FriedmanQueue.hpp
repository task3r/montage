...
class FriedmanQueue : public RQueue<std::string> {
   ...
   public:
    ...
    ~FriedmanQueue() { Persistent::finalize(); };
    ...
    while (1) {
        ...
        if (first == head->load()) {
            if (first == last) {
                ...
                std::string value =
                    std::string(std::begin(next->value), std::end(next->value));
                int i = -1;
                if (next->deqTid.compare_exchange_strong(i, tid)) {
                    clwb_obj_fence(&(first->next.load()->deqTid));
                    std::copy(value.begin(), value.end(),
                              returnedVal[tid]->data());
                    clwb_range((void*)returnedVal[tid], sizeof(T));
                    Node* to_retire =
                        first;  // failed CAS will write new head into first
