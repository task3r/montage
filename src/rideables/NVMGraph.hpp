...
class NVMGraph : public RGraph {
   public:
    // We use smart pointers in the unordered_set, but we can only lookup by a
    // key allocated on the stack if and only if it is also wrapped into a smart
    // pointer. We create a custom 'deleter' function to control whether or not
    // it will try to delete the wrapped pointer below
    ...
    class alignas(64) tVertex {
       public:
        ...
        tVertex(const tVertex& oth) : id(oth.id) {
            payload = new Vertex(oth.id, oth.payload->get_lbl());
        }
        bool operator==(const tVertex& oth) const {
            return payload->id == oth.payload->id;
        }
    ...
    class Relation : public Persistent {
       public:
        ...
        Relation(const Relation& oth)
            : src(oth.src), dest(oth.dest), weight(oth.weight) {}
    ...
    struct alignas(64) VertexMeta {
        tVertex* idxToVertex =
            nullptr;  // Transient set of transient vertices to index map
        std::mutex vertexLocks;   // Transient locks for transient vertices
        uint32_t vertexSeqs = 0;  // Transient sequence numbers for
                                  // transactional operations on vertices
    };
    ...
    startOver : {
        ...
        for (int other : vertices) {
            ...
            if (!has_relation(source(other), src) &&
                ...
                std::cout << "Observed pair (" << vid << "," << other
