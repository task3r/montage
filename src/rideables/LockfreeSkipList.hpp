...
class LockfreeSkipList : public RMap<K, V> {
   private:
    ...
    int internal_finish_contains(const K &key, Node *node, V *node_val,
                                 optional<V> &ret_value);
    int internal_finish_delete(const K &key, Node *node, V *node_val,
                               optional<V> &ret_value, int tid);
    int internal_finish_insert(const K &key, V *val, Node *node, V *node_val,
                               Node *next);
    bool internal_do_operation(operation_type optype, const K &key, V *val,
                               optional<V> &ret_value, int tid);
   ...
   public:
    LockfreeSkipList(int task_num) : tracker(task_num + 1, 100, 1000, true) {
        ...
        background_thread =
            std::move(std::thread(&LockfreeSkipList::bg_loop, this, bg_tid));
    };
    ~LockfreeSkipList() {
...
int LockfreeSkipList<K, V>::bg_trav_nodes(int tid) {
    ...
    while (nullptr != next) {
        ...
        if (nullptr != node->val.ptr.load() &&
            (void *)node != (void *)node->val.ptr.load()) {
            ++bg_non_deleted;
        }
        prev = node;
        node = next;
...
void LockfreeSkipList<K, V>::get_index_above(Node *above_head,
                                             Node *&above_prev,
                                             Node *&above_next, unsigned long i,
                                             const K &key, unsigned long zero) {
...
void LockfreeSkipList<K, V>::bg_help_remove(Node *prev, Node *node, int tid) {
    ...
    if ((void *)node->val.ptr.load() != (void *)node || node->marker) return;
...
template <class K, class V>
int LockfreeSkipList<K, V>::internal_finish_contains(const K &key, Node *node,
                                                     V *node_val,
                                                     optional<V> &ret_value) {
    int result = 0;
...
template <class K, class V>
int LockfreeSkipList<K, V>::internal_finish_delete(const K &key, Node *node,
                                                   V *node_val,
                                                   optional<V> &ret_value,
                                                   int tid) {
    int result = -1;
        ...
        if (nullptr != node_val) {
            ...
            while (1) {
                ...
                if (nullptr == node_val || (void *)node == (void *)node_val) {
                    result = 0;
                    break;
                } else if (node->val.ptr.compare_exchange_strong(node_val,
                                                                 nullptr)) {
                    ret_value = V(*node_val);
...
template <class K, class V>
int LockfreeSkipList<K, V>::internal_finish_insert(const K &key, V *val,
                                                   Node *node, V *node_val,
                                                   Node *next) {
    int result = -1;
    Node *new_node, *temp;
...
template <class K, class V>
bool LockfreeSkipList<K, V>::internal_do_operation(operation_type optype,
                                                   const K &key, V *val,
                                                   optional<V> &ret_value,
                                                   int tid) {
    ...
    while (1) {
        ...
        if (nullptr == next || next->key > key) {
            if (CONTAINS == optype)
                result =
                    internal_finish_contains(key, node, node_val, ret_value);
            else if (DELETE == optype)
                result =
                    internal_finish_delete(key, node, node_val, ret_value, tid);
...
bool LockfreeSkipList<K, V>::insert(K key, V val, int tid) {
    ...
    optional<V> unused = {};
    bool success =
        internal_do_operation(operation_type::INSERT, key, s_ptr, unused, tid);
    if (!success) {
        delete s_ptr;
    }
    return success;
