...
void barrier() { pthread_barrier_wait(&pthread_barrier); }
...
void initTest(GlobalTestConfig* gtc) {
    ...
    gtc->test->init(gtc);
    for (size_t i = 0;
         i < gtc->allocatedRideables.size() && gtc->getEnv("report") == "1";
         i++) {
        if (Reportable* r =
                dynamic_cast<Reportable*>(gtc->allocatedRideables[i])) {
            r->introduce();
        }
    }
}
...
static void* thread_main(void* lp) {
    ...
    if (task_id == 0) {
        gtc->parInit_time =
            chrono::duration_cast<chrono::milliseconds>(
                chrono::high_resolution_clock::now() - gtc->start)
                .count();
    }
    ...
    if (task_id == 0) {
        ...
        gtc->interval =
            chrono::duration_cast<chrono::microseconds>(now - gtc->start)
                .count() /
            1000000.0;
        if (gtc->verbose) {
            std::cout << "actual interval:" << gtc->interval << std::endl;
        }
...
void parallelWork(GlobalTestConfig* gtc) {
    ...
    ctcs =
        (CombinedTestConfig*)malloc(sizeof(CombinedTestConfig) * gtc->task_num);
    ...
    if (gtc->timeOut) {
        ...
        alarm(gtc->interval +
              30);  // set an alarm for interval+30 seconds from now
    }
