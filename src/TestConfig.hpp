...
class GlobalTestConfig {
   public:
    ...
    void buildInterleavedPerCoreAffinity(std::vector<hwloc_obj_t>& aff,
                                         unsigned pu);
    void buildSingleSocketPerCoreAffinity(std::vector<hwloc_obj_t>& aff,
                                          unsigned pu);
   ...
   private:
    ...
    void buildDFSAffinity_helper(std::vector<hwloc_obj_t>& aff,
                                 hwloc_obj_t obj);
    void buildDFSAffinity(std::vector<hwloc_obj_t>& aff);
    int buildDefaultAffinity_findCoresInSocket(std::vector<hwloc_obj_t>& aff,
                                               hwloc_obj_t obj,
                                               std::vector<hwloc_obj_t>* cores);
    int buildDefaultAffinity_buildPUsInCores(std::vector<hwloc_obj_t>& aff,
                                             std::vector<hwloc_obj_t>* cores);
    int buildDefaultAffinity_findAndBuildSockets(std::vector<hwloc_obj_t>& aff,
                                                 hwloc_obj_t obj);
    void buildDefaultAffinity(std::vector<hwloc_obj_t>& aff);
    void buildSingleAffinity_helper(std::vector<hwloc_obj_t>& aff,
                                    hwloc_obj_t obj);
    void buildSingleAffinity(std::vector<hwloc_obj_t>& aff);
    void buildSingleSocketAffinity(std::vector<hwloc_obj_t>& aff);
    void buildPerCoreAffinity_helper(std::vector<hwloc_obj_t>& aff, unsigned pu,
                                     hwloc_obj_t obj);
    void buildInterleavedAffinity(std::vector<hwloc_obj_t>& aff);
    void buildInterleavedAffinity_traversePackages(
        std::vector<std::vector<hwloc_obj_t>>& thread_per_package,
        hwloc_obj_t obj);
    void buildInterleavedPerCoreAffinity_traversePackages(
