...
template <class K, class V>
void RecoverVerifyTest<K, V>::parInit(GlobalTestConfig* gtc,
                                      LocalTestConfig* ltc) {
...
template <>
inline std::string RecoverVerifyTest<std::string, std::string>::fromInt(
    uint64_t v) {
...
template <class K, class V>
int RecoverVerifyTest<K, V>::execute(GlobalTestConfig* gtc,
                                     LocalTestConfig* ltc) {
    std::string value_buffer;  // for string kv only
    if (!gtc->checkEnv("NoVerify")) {
        if (ltc->tid == 0) {  // FIXME: workaround when we can't change the
                              // total thread count of ralloc.
            ...
            std::cout << "insert finished. Spent " << dur_ms << "ms"
                      << std::endl;
            rec->flush();
            ...
            if (rec_cnt == (int)reference.size()) {
                ...
                std::cout << "recovered:" << rec_cnt
                          << " expecting:" << reference.size() << std::endl;
                exit(1);
            }

            for (auto itr = reference.begin(); itr != reference.end(); itr++) {
                if (!m->get(itr->first, tid)) {
                    std::cout << "key:" << itr->first << "not recovered."
                              << std::endl;
                    exit(1);
                }
            }
