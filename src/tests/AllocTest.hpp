...
enum AllocTestType { DO_JEMALLOC_ALLOC, DO_RALLOC_ALLOC, DO_MONTAGE_ALLOC };
...
class AllocTest : public Test {
   public:
    ...
    AllocTest(uint64_t ops, enum AllocTestType allocType)
        : total_ops(ops), allocType(allocType) {}
        ...
        for (size_t i = 0; i < thd_ops[tid]; i++) {
            switch (allocType) {
                ...
                case DO_RALLOC_ALLOC: {
                    objs.push_back(
                        (DummyObject *)RP_malloc(sizeof(DummyObject)));
