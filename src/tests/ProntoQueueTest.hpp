...
public:
    ...
    ProntoQueueTest(uint64_t o, unsigned int e = 50){
        //wl is a or b
    ...
    void parInit(GlobalTestConfig* gtc, LocalTestConfig* ltc){
        //we do prefill here because we want pronto pthrea create to run first
        if(ltc->tid==0)
            doPrefill(gtc,0);
    }
    ...
    void getRideable(GlobalTestConfig* gtc){
        ...
        q = dynamic_cast<ProntoQueue*>(ptr);
        if(!q){
            errexit("ProntoQueueTest must be run on ProntoQueue type object.");
        } 
    }
...
pthread_t ProntoQueueTest::snapshot_thread;
pthread_mutex_t ProntoQueueTest::snapshot_lock;
// template <>
// void ProntoQueueTest<std::string>::init(GlobalTestConfig* gtc){
//     cerr<<"ProntoQueueTest doesn't support string type!"<<std::endl;
//     exit(1);
// }
#endif
