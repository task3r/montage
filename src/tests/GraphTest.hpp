...
static void print_stats(int numV, int numE, double averageDegree,
                        int *vertexDegrees, int vertexDegreesLength) {
    int maxDegree = 0;
    for (auto i = 0; i < vertexDegreesLength; i++) {
        maxDegree = max(maxDegree, vertexDegrees[i]);
    }
    std::cout << "|V|=" << numV << ",|E|=" << numE
              << ",average degree = " << averageDegree
              << ",maximum degree = " << maxDegree << std::endl;
...
class GraphTest : public Test {
   public:
    ...
    void init(GlobalTestConfig *gtc) {
        ...
        addEdgeProb = std::max(1, (int)edge_op * desiredAvgDegree * 100 /
                                      (max_verts * vertexLoad));
        ...
        if (gtc->verbose)
            std::cout << "AddEdge:RemoveEdge:AddVertex:RemoveVertex="
                      << addEdgeProb << ":" << remEdgeProb << ":" << addVerProb
                      << ":" << remVerProb << std::endl;
    ...
    int execute(GlobalTestConfig *gtc, LocalTestConfig *ltc) {
        ...
        int tid = ltc->tid;
        if (tid == 0)
            if (gtc->verbose) std::cout << "Starting test now..." << std::endl;
        ...
        while (
            ...
            int rng = dist(gen_p);
            if (rng < addEdgeProb) {
                // std::cout << "rng(" << rng << ") is add_edge <= " <<
                // addEdgeProb << std::endl;
                if (g->add_edge(distv(gen_v), distv(gen_v), -1))
                    operations[tid].ui[0]++;
            } else if (rng < addEdgeProb + remEdgeProb) {
                // std::cout << "rng(" << rng << ") is remove_any_edge <= " <<
                // addEdgeProb + remEdgeProb << std::endl;
                if (g->remove_edge(distv(gen_v), distv(gen_v)))
                    operations[tid].ui[1]++;
            } else if (rng < addEdgeProb + remEdgeProb + addVerProb) {
                // std::cout << "rng(" << rng << ") is has_edge <= " <<
                // addEdgeProb + remEdgeProb + addVerProb << std::endl;
                if (g->add_vertex(distv(gen_v))) operations[tid].ui[2]++;
            } else {
                // std::cout << "rng(" << rng << ") is remove_vertex...";
                if (g->remove_vertex(distv(gen_v))) operations[tid].ui[3]++;
            }
            ops++;
            if (ops % 512 == 0) {
    ...
    void cleanup(GlobalTestConfig *gtc) {
        ...
        for (int i = 0; i < gtc->task_num; i++) {
            total += (operations[i].ui[0] + operations[i].ui[1] +
                      operations[i].ui[2] + operations[i].ui[3]);
        ...
        if (gtc->verbose)
            std::cout << "add_edge = " << add_edge << " (" << add_edge_prop
                      << "%)" << std::endl
                      << ", remove_edge = " << rem_edge << " (" << rem_edge_prop
                      << "%)" << std::endl
                      << ", add_vertex = " << add_ver << " (" << add_ver_prop
                      << "%)" << std::endl
                      << ", remove_vertex = " << rem_ver << " (" << rem_ver_prop
                      << "%)" << std::endl;
        delete g;
    }
