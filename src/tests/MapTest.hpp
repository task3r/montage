...
class MapTest : public Test {
   ...
   public:
    ...
    uint64_t total_ops;
    uint64_t* thd_ops;
    MapTest(int p_gets, int p_puts, int p_inserts, int p_removes, int range,
            int prefill = 0, int op = 10000000) {
        pg = p_gets;
        pp = p_puts;
        pi = p_inserts;
        pv = p_removes;
...
inline std::string MapTest<std::string, std::string>::fromInt(uint64_t v) {
    ...
    return "user" + std::string(key_size - _key.size() - 5, '0') +
           _key;  // 31 in total; last one left for terminating null
}

template <>
inline void MapTest<std::string, std::string>::doPrefill(
...
template <>
inline void MapTest<std::string, std::string>::operation(uint64_t key, int op,
                                                         int tid) {
