...
class TGraphConstructionTest : public Test {
   public:
    ...
    TGraphConstructionTest(string graphDir, string base_fname, int num_files,
                           int width)
        : graphDir(graphDir),
    ...
    void init(GlobalTestConfig *gtc) {
        ...
        if (!g) {
            errexit(
                "TGraphConstructionTest must be run on RGraph type object.");
        }
    ...
    int stream_edges_from_file(bool insert_edges, int num_threads, int tid) {
        ...
        if (!insert_edges)
            std::cout << "verify finished on thread " << tid << std::endl;
        return 0;
    }
    ...
    int execute(GlobalTestConfig *gtc, LocalTestConfig *ltc) {
        ...
        if (ltc->tid == 0) {
            std::cout << "Time to initialize graph: " << gtc->parInit_time
                      << " (ms)" << std::endl;
        }
        return 0;
    }
