 ...
 * In multi-threaded scenarios, each thread operates on a cyclical partition of
 * the key space. E.g. for n threads, thread 0 does operations on key #0, #n,
 * #2n etc. This wouldn't catch all data races, but is good enough.
 */
...
class MapVerify : public ChurnTest {
   public:
    ...
    void doPrefill(GlobalTestConfig* gtc) {
        // randomly prefill until specified amount of keys are successfully
        // inserted
        if (this->prefill > 0) {
...
inline void MapVerify<K, V>::operation(uint64_t key, int op, int tid) {
    ...
    if (op < this->prop_gets) {
        ...
        if (m_response != m_expected) {
            printf(
                "MapVerify response for insert %d, expected %d for key: %s\n",
                m_response, m_expected, k.c_str());
            exit(-1);
        }
        if (m_response) my_map[k] = getV(key);
        ...
        if (m_response.has_value() != ground_contains_bool) {
            printf(
                "MapVerfiy response for remove %d, expected %d for key: %s\n",
                m_response.has_value(), ground_contains_bool, k.c_str());
            exit(-1);
        }
        if (m_response.has_value() and *m_response != ite->second) {
            printf(
                "MapVerify response for remove %s, expected %s for key: %s\n",
                m_response->c_str(), ite->second.c_str(), k.c_str());
            exit(-1);
        }
