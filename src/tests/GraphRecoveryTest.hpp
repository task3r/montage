...
class GraphRecoveryTest : public Test {
   public:
    ...
    GraphRecoveryTest(string graphDir, string base_fname, int num_files,
                      int width, bool verify)
        : graphDir(graphDir),
    ...
    void init(GlobalTestConfig *gtc) {
        ...
        if (!rec) {
            errexit(
                "GraphRecoveryTest must be run on Recoverable type object.");
        }
    ...
    // Allocate an array of edge structs, read the bytes from the file into
    // this. Open the file, take the size, and divide by 8 to get the number of
    // edges in the file
    int stream_edges_from_file(bool insert_edges, int num_threads, int tid) {
        for (int i = tid; i < num_files; i += num_threads) {
            ...
            FILE *f =
                fopen((graphDir + base_fname + ss.str() + ".bin").c_str(), "r");
            if (f == nullptr) {
                std::cout << "Could not open file("
                          << graphDir + base_fname + ss.str() + ".bin"
                          << ")" << std::endl;
                std::cout << "i(" << i << "), num_files(" << num_files << ")"
                          << std::endl;
                exit(-1);
                return -1;
            }
    ...
    void parInit(GlobalTestConfig *gtc, LocalTestConfig *ltc) {
        ...
        // Allocate an array of edge structs, read the bytes from the file into
        // this. Open the file, take the size, and divide by 8 to get the number
        // of edges in the file
        ...
        if (tid == 0) {
            ...
            auto dur_ms =
                std::chrono::duration_cast<std::chrono::milliseconds>(dur)
                    .count();
            std::cout << "Parallel Initialization took " << dur_ms << "ms"
                      << std::endl;
        }
    }
    ...
    void cleanup(GlobalTestConfig *gtc) { delete g; }
};
#endif
