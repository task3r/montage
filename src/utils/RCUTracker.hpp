...
class RCUTracker : public BaseTracker {
   ...
   public:
    struct RCUInfo {
       public:
        ...
        RCUInfo(void* obj, uint64_t epoch, const std::function<void(void*)>& d)
            : obj(obj), epoch(epoch), destruct(d) {}
    };
   ...
   public:
    ~RCUTracker(){};
    RCUTracker(int task_num, int epochFreq, int emptyFreq, RCUType type,
               bool collect)
        : BaseTracker(task_num),
          task_num(task_num),
          freq(emptyFreq),
          epochFreq(epochFreq),
          collect(collect),
    ...
    RCUTracker(int task_num, int epochFreq, int emptyFreq)
        : RCUTracker(task_num, epochFreq, emptyFreq, type_RCU, true) {}
    ...
    void reserve(int tid) { start_op(tid); }
    void clear(int tid) { end_op(tid); }
    ...
    template <class T>
    void retire(T* obj, int tid) {
        retire(obj, tid, [](void* o) { delete (static_cast<T*>(o)); });
    }
    ...
    void empty(int tid) {
        ...
        for (auto iterator = myTrash->begin(), end = myTrash->end();
             iterator != end;) {
