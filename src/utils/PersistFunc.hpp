...
inline void clflush(void *p) { asm volatile("clflush (%0)" ::"r"(p)); }

inline void clflushopt(void *p) { asm volatile("clflushopt (%0)" ::"r"(p)); }
...
inline void clflush_range_nofence(void *p, size_t sz) {  // unit of sz is byte.
    for (char *curr = (char *)p;
         curr <= (char *)(((size_t)p + sz) | CACHE_LINE_MASK);
