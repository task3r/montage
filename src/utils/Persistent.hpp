...
class Persistent {
   public:
    ...
    static size_t get_malloc_size(void* ptr) { return RP_malloc_size(ptr); }
    ...
    static InuseRecovery::iterator recover() { return recover(1).at(0); }
    static void simulate_crash() { RP_simulate_crash(); }

    static void init_thread(GlobalTestConfig* gtc, LocalTestConfig* ltc) {
        init_thread(ltc->tid);
    }

    static void init_thread(int tid_) { Ralloc::set_tid(tid_); }
};
