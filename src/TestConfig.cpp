...
void GlobalTestConfig::printargdef() {
    uint64_t i;
    fprintf(stderr,
            "usage: %s [-m <test_mode_index>] [-M <test_mode_name>] [-r "
            "<rideable_test_object_index>] [-R <rideable_test_object_name>] "
            "[-a single|dfs|default] [-i <interval>] [-t <num_threads>] [-o "
            "<output_csv_file>] [-w <warm_up_MBs>] [-d <env_variable>=<value>] "
            "[-z] [-v] [-h]\n",
            argv0);
...
void GlobalTestConfig::parseCommandLine(int argc, char** argv) {
    ...
    if (tests.size() == 0) {
        errexit(
            "No test options provided.  Use GlobalTestConfig::addTestOption() "
            "to add.");
    }
    ...
    while ((c = getopt(argc, argv, "s:d:w:o:i:t:m:M:a:r:R:vhz")) != -1) {
        switch (c) {
            ...
            case 'R': {
                std::string n(optarg);
                auto idx =
                    std::find(rideableNames.begin(), rideableNames.end(), n);
    ...
    num_procs = hwloc_get_nbobjs_by_depth(
        topology, hwloc_get_type_depth(topology, HWLOC_OBJ_PU));
...
void GlobalTestConfig::buildDFSAffinity_helper(std::vector<hwloc_obj_t>& aff,
                                               hwloc_obj_t obj) {
...
// Default affinity: pin threads on PUs in the same core, then other cores in
// the socket, then different socket.

int GlobalTestConfig::buildDefaultAffinity_findCoresInSocket(
    std::vector<hwloc_obj_t>& aff, hwloc_obj_t obj,
    ...
    int ret = 1;
    for (unsigned i = 0; i < obj->arity; i++) {
        ret = ret && buildDefaultAffinity_findCoresInSocket(
                         aff, obj->children[i], cores);
    }
    return ret;
}

int GlobalTestConfig::buildDefaultAffinity_buildPUsInCores(
    std::vector<hwloc_obj_t>& aff, vector<hwloc_obj_t>* cores) {
...
int GlobalTestConfig::buildDefaultAffinity_findAndBuildSockets(
    std::vector<hwloc_obj_t>& aff, hwloc_obj_t obj) {
    ...
    for (unsigned i = 0; i < obj->arity; i++) {
        ret = ret &&
              buildDefaultAffinity_findAndBuildSockets(aff, obj->children[i]);
    }
    return ret;
}

void GlobalTestConfig::buildDefaultAffinity(std::vector<hwloc_obj_t>& aff) {
    if (!buildDefaultAffinity_findAndBuildSockets(
            aff, hwloc_get_root_obj(topology))) {
        fprintf(stderr,
                "Unsupported topology for default thread pinning (unable to "
                "detect sockets and cores).");
...
void GlobalTestConfig::buildSingleAffinity_helper(std::vector<hwloc_obj_t>& aff,
                                                  hwloc_obj_t obj) {
    if (obj->type == HWLOC_OBJ_PU) {
...
void GlobalTestConfig::buildSingleSocketAffinity(
    std::vector<hwloc_obj_t>& aff) {
...
void GlobalTestConfig::buildSingleSocketPerCoreAffinity(
    std::vector<hwloc_obj_t>& aff, unsigned pu) {
    hwloc_obj_t obj = hwloc_get_root_obj(topology);
    while (obj->type < HWLOC_OBJ_SOCKET) {
        obj = obj->children[0];
    }
...
// Per-core affinity: pin one thread to $pu$ pu (thread) of each core in the
// same socket, then go cross socket. Assuming cores have the same arity.
void GlobalTestConfig::buildPerCoreAffinity_helper(
    std::vector<hwloc_obj_t>& aff, unsigned pu, hwloc_obj_t obj) {
    if (obj->type == HWLOC_OBJ_CORE) {
        assert(obj->arity > pu);
...
void GlobalTestConfig::buildPerCoreAffinity(std::vector<hwloc_obj_t>& aff,
                                            unsigned pu) {
    buildPerCoreAffinity_helper(aff, pu, hwloc_get_root_obj(topology));
    extendAffinity(aff);
}

// Interleaved affinity: first thread of each socket, then the second thread,
// and so on. Assuming every socket has the same amount of threads.
void GlobalTestConfig::buildInterleavedAffinity_traversePackages(
    ...
    if (obj->type < HWLOC_OBJ_PACKAGE) {
        for (size_t i = 0; i < obj->arity; i++) {
            buildInterleavedAffinity_traversePackages(thread_per_package,
                                                      obj->children[i]);
        }
...
// Interleaved per-core affinity: the $pu$ thread of first core in each socket,
// then the $pu$ thread of second core in each socket, and so on.
void GlobalTestConfig::buildInterleavedPerCoreAffinity_traversePackages(
    std::vector<std::vector<hwloc_obj_t>>& thread_per_package, unsigned pu,
    hwloc_obj_t obj) {
    if (obj->type < HWLOC_OBJ_PACKAGE) {
        for (size_t i = 0; i < obj->arity; i++) {
            buildInterleavedPerCoreAffinity_traversePackages(
                thread_per_package, pu, obj->children[i]);
        }
    } else if (obj->type == HWLOC_OBJ_PACKAGE) {
        std::vector<hwloc_obj_t> curr_package;
...
void GlobalTestConfig::buildInterleavedPerCoreAffinity(
    std::vector<hwloc_obj_t>& aff, unsigned pu) {
    std::vector<std::vector<hwloc_obj_t>> thread_per_package;
    buildInterleavedPerCoreAffinity_traversePackages(
        thread_per_package, pu, hwloc_get_root_obj(topology));
    // check if all sockets have the same amount of threads: (debug only)
    assert(!thread_per_package.empty());
    for (size_t i = 0; i < thread_per_package.size(); i++) {
        assert(thread_per_package[i].size() == thread_per_package[0].size());
...
void GlobalTestConfig::addRideableOption(RideableFactory* h,
                                         const char name[]) {
...
void GlobalTestConfig::runTest() {
    ...
    if (outFile.size() != 0) {
        ...
        if (verbose) {
            std::cout << "Stored test results in: " << outFile << std::endl;
        }
    }
    if (verbose) {
        std::cout << recorder->getCSV() << std::endl;
    }
}