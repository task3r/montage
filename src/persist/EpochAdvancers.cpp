...
DedicatedEpochAdvancer::DedicatedEpochAdvancer(GlobalTestConfig* gtc,
    ...
    advancer_thread = std::move(
        std::thread(&DedicatedEpochAdvancer::advancer, this, gtc->task_num));
...
void DedicatedEpochAdvancer::advancer(int task_num) {
    if (advancer_affinity != nullptr) {
        hwloc_set_cpubind(gtc->topology, advancer_affinity->cpuset,
                          HWLOC_CPUBIND_THREAD);
    }
    EpochSys::init_thread(task_num);  // set tid to be the last
    uint64_t curr_epoch = INIT_EPOCH;
    int64_t next_sleep =
        epoch_length;  // unsigned to signed, but should be fine.
    ...
    while (advancer_state.load() == RUNNING) {
...

        {
            ...
            target_epoch.ui.compare_exchange_strong(tmp_curr_epoch,
                                                    tmp_curr_epoch + 1);
        ...
        // measure the time used for write-back and reclamation, and deduct it
        // from epoch_length.
        int64_t wb_length = chrono::duration_cast<chrono::microseconds>(
                                chrono::high_resolution_clock::now() - wb_start)
                                .count();
...
void DedicatedEpochAdvancer::sync(uint64_t c) {
    ...
    for (auto curr_epoch = esys->get_epoch(); curr_epoch < c + 2;
         curr_epoch++) {
        esys->on_epoch_end(curr_epoch);
        // Advance epoch number
        if (esys->epoch_CAS(curr_epoch, curr_epoch + 1)) {
...
DedicatedEpochAdvancerNbSync::DedicatedEpochAdvancerNbSync(
    GlobalTestConfig* gtc, EpochSys* es)
    : gtc(gtc), esys(es) {
    if (gtc->checkEnv("EpochLength")) {
        epoch_length = stoi(gtc->getEnv("EpochLength"));
    } else {
    ...
    if (epoch_length != 0) {
        ...
        started.store(false);
        advancer_thread = std::move(std::thread(
            &DedicatedEpochAdvancerNbSync::advancer, this, gtc->task_num));
        started.store(true);
    }
}
...
void DedicatedEpochAdvancerNbSync::advancer(int task_num) {
    ...
    while (started.load()) {
...

        {
            ...
            esys->on_epoch_end(curr_epoch);
            // Advance epoch number
            if (esys->epoch_CAS(curr_epoch, curr_epoch + 1)) curr_epoch++;
            // restart timer for a new epoch
            wb_start =
                chrono::high_resolution_clock::now();  // TODO: is this correct?
