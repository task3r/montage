...
class Mindicator : public PersistTracker {
    class MarkedVal {
       ...
       public:
        ...
        MarkedVal(uint64_t x, bool dirty) noexcept
            : v(x), mark(dirty ? 0x1ULL : 0) {}
        MarkedVal(const MarkedVal& oth) noexcept =
            default;  // trivially copyable.
        inline bool is_dirty() { return ((mark & 0x1ULL) == 0x1ULL); }
        inline void set_dirty() { mark |= 0x1ULL; }
        inline void set_clean() { mark &= ~0x1ULL; }
        inline uint64_t get_val() { return v; }
        inline void set_val(uint64_t x) { v = x; }
        inline uint64_t get_counter() { return (mark & ~0x1ULL); }
    ...
    bool summarize(uint64_t val, Node* n) {  // relaxed version.
        while (true) {
            ...
            uint64_t children_min =
                std::min(n->children[0]->marked_val.load().get_val(),
   ...
   public:
    ...
    void after_persist_epoch(uint64_t e, int tid) {
        ...
        int one = 1;
        has_write_op[e % EPOCH_WINDOW][tid].ui.compare_exchange_strong(
            one, 0);  // TODO: ABA problem.
    }
...
class IncreasingMindicator : public PersistTracker {
        ...
        while (true) {
            ...
            if (new_min > old_val) {
                if (n->val.compare_exchange_strong(old_val, new_min)) {
                    // either our value or our sibling changed the current
                    // value. continue propagation.
                    val = new_min;
                    return false;
                } else {
                ...
                // didn't change the current value, or the current value is
                // ahead of us. stop propogation.
                return true;
            }
        }
    }
   ...
   public:
    ...
    // }
    uint64_t next_epoch_to_persist(int tid) { return leaves[tid].val.load(); }
};
