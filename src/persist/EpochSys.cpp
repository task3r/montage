...
void EpochSys::parse_env() {
    ...
    if (gtc->checkEnv("TransTracker")) {
        ...
        if (env_transcounter == "AtomicCounter") {
            ...
            trans_tracker =
                new FenceBeginTransactionTracker(this->global_epoch, task_num);
        } else if (env_transcounter == "CurrEpoch") {
            trans_tracker =
                new PerEpochTransactionTracker(this->global_epoch, task_num);
        } else {
    ...
    //     if (gtc->getEnv("EpochAdvance") != "Dedicated" && env_epoch_advance >
    //     63){
...
// the same as end_readonly_transaction, but semantically different. Repeat to
// avoid confusion.
...
void EpochSys::register_update_pblk(PBlk* b, uint64_t c) {
    ...
    if (c == NULL_EPOCH) {
        // update before BEGIN_OP, return. This register will be done by
        // BEGIN_OP.
        return;
    }
...
void EpochSys::retire_pblk(PBlk* b, uint64_t c, PBlk* anti) {
    ...
    if (blk->retire != nullptr) {
        errexit(
            "double retire error, or this block was tentatively retired before "
            "recent crash.");
    }
    ...
    if (e > c) {
        ...
        // changing it directly to a DELETE node without putting it in
        // to_be_freed list.
...
//     // TODO: if we go with one bookkeeping thread, remove unecessary
//     synchronizations.
...
//         }
//     }
//     // persist_func::sfence(); // given the length of current epoch, we may
//     not need this.
...
//     persist_func::sfence();
//     // Actually advance the epoch
//     // global_epoch->compare_exchange_strong(c, c+1,
//     std::memory_order_seq_cst); global_epoch->store(c+1,
...
std::unordered_map<uint64_t, PBlk*>* EpochSys::recover(const int rec_thd) {
    ...
    if (itr_raw[0].is_dirty()) {
        ...
        clean_start = true;
        // clean restart, epoch system and app may still need iter to do
        // something
    }
...
void nbEpochSys::prepare_retire_pblk(
    PBlk* b, const uint64_t& c,
    std::vector<std::pair<PBlk*, PBlk*>>& pending_retires) {
    PBlk* blk = b;
    uint64_t e = blk->epoch;
...
void nbEpochSys::prepare_retire_pblk(std::pair<PBlk*, PBlk*>& pending_retire,
                                     const uint64_t& c) {
...
void nbEpochSys::local_free(uint64_t c) {
    ...
    for (uint64_t i = last_epoch - 1; i <= min(last_epoch + 1, c - 2); i++) {
...
void nbEpochSys::on_epoch_end(uint64_t c) {
    ...
    while (curr_thread >= 0) {
        // traverse mindicator to persist each leaf lagging behind, until the
        // top meets requirement
        local_descs[curr_thread]->try_abort(
            c - 1);  // lazily abort ongoing transactions
        to_be_persisted->persist_epoch_local(c - 1, curr_thread);
        persisted_epochs->after_persist_epoch(c - 1, curr_thread);
        curr_thread =
            persisted_epochs->next_thread_to_persist(c - 1, curr_thread);
    }
    // a lock-prefixed instruction (CAS) must have taken place inside
    // Mindicator, so no need to explicitly issue fence here.
    // persist_func::sfence();
