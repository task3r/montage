...
class UIDGenerator {
   ...
   public:
    UIDGenerator() {}
    UIDGenerator(uint64_t task_num) { init(task_num); }
    ~UIDGenerator() {
        if (curr_ids) {
            delete curr_ids;
        }
    ...
    uint64_t get_id(int tid) { return curr_ids[tid].ui++; }
};
...
class CircBuffer {
    ...
    // padding head/tail pointers: performs better when producer and consumer
    // are on different threads. head pointer, where things got popped off
    padded<Node*> head;
   ...
   public:
    ...
    void push(T x) {
        ...
        if (next_tail == head.ui) {
            ...
            // note that if we're in a chunk, it will be unfortunately broken
            // into half.
...
class PerThreadCircBuffer {
   ...
   public:
    ...
    void push(T x, int tid) { buffers[tid].ui->push(x); }
...
class FixedCircBuffer {
   ...
   public:
    FixedCircBuffer(size_t _cap) : cap(_cap) {
        payloads = new padded<T>[cap];
        pushed.ui = 0;
        popped.ui = 0;
    }
    ~FixedCircBuffer() { delete (payloads); }
    void push(T x, const std::function<void(T& x)>& func) {
        ...
        if (curr_pushed == curr_popped + cap) {
            ...
            popped.ui.compare_exchange_strong(curr_popped, curr_popped + 1,
    ...
    bool try_pop(const std::function<void(T& x)>& func) {
        ...
        while (!popped.ui.compare_exchange_strong(curr_popped, curr_popped + 1,
                                                  std::memory_order_acq_rel)) {
    ...
    void pop_all(const std::function<void(T& x)>& func) {
        while (true) {
            ...
            if (popped.ui.compare_exchange_strong(curr_popped, curr_pushed,
                                                  std::memory_order_acq_rel)) {
                return;
            }
        }
   ...
   public:
    ...
    void pop_all(const std::function<void(void*& x)>& func) {
        for (int i = 0; i < cap; i++) {
            ...
            func(exp);
            // we need to use CAS rather than a store here because we may get
            // preempted here and wake up several epochs later. but, there won't
            // be any correctness issues if we don't overwrite with 0; we're
            // only doing this to prevent redundant persisting.
...
// a group of per-thread circular buffer
// NOTE: this is designed for single-consumer pattern only. The container is NOT
// thread safe.
template <typename T>
class PerThreadVector {
   ...
   public:
    ...
    void push(T x, int tid) { buffers[tid].ui->push_back(x); }
    void pop_all(const std::function<void(T& x)>& func) {
        for (int i = 0; i < count; i++) {
