...
class PerThreadContainer {
   public:
    ...
    virtual bool try_pop_local(const std::function<void(T& x)>& func, int tid,
                               uint64_t c) = 0;
    virtual void pop_all_local(const std::function<void(T& x)>& func, int tid,
                               uint64_t c) = 0;
...
class FixedContainer {
   public:
    ...
    virtual void push(T x, const std::function<void(T& x)>& func, int tid,
                      uint64_t c) = 0;
...
class FixedHashSetContainer : public FixedContainer<void*> {
   ...
   public:
    ...
    void push(void* x, const std::function<void(void*& x)>& func, int tid,
              uint64_t c) {
        return containers[c % EPOCH_WINDOW][tid]->push(x, func);
    }
    ...
    void pop_all_local(const std::function<void(void*& x)>& func, int tid,
                       uint64_t c) {
        assert(tid != -1);
        containers[c % EPOCH_WINDOW][tid]->pop_all(func);
    }
    bool try_pop_local(const std::function<void(void*& x)>& func, int tid,
                       uint64_t c) {
        assert(tid != -1);
        return containers[c % EPOCH_WINDOW][tid]->try_pop(func);
    }
