...
struct EpochVerifyException : public std::exception {
    const char* what() const throw() {
        return "Epoch in which operation wants to linearize has passed; retry "
               "required.";
    }
};
}  // namespace pds
...
class Recoverable {
    ...
    padded<std::vector<pair<pds::PBlk*, pds::PBlk*>>>* pending_retires =
        nullptr;
   ...
   public:
    ...
    bool check_epoch(uint64_t c) { return _esys->check_epoch(c); }
    void begin_op() {
    ...
    void end_op() {
        ...
        if (!pending_retires[pds::EpochSys::tid].ui.empty()) {
            for (const auto& r : pending_retires[pds::EpochSys::tid].ui) {
                ...
                _esys->retire_pblk(r.first, epochs[pds::EpochSys::tid].ui,
                                   r.second);
            }
    ...
    void abort_op() {
        ...
        if (!pending_retires[pds::EpochSys::tid].ui.empty()) {
            for (const auto& r : pending_retires[pds::EpochSys::tid].ui) {
                _esys->withdraw_retire_pblk(r.second,
    ...
    class MontageOpHolder {
       ...
       public:
        MontageOpHolder(Recoverable* ds_) : ds(ds_) { ds->begin_op(); }
        ~MontageOpHolder() { ds->end_op(); }
    };
    class MontageOpHolderReadOnly {
        Recoverable* ds = nullptr;

       public:
        MontageOpHolderReadOnly(Recoverable* ds_) : ds(ds_) { ds->begin_op(); }
        ~MontageOpHolderReadOnly() { ds->end_readonly_op(); }
    };
        ...
        if (_esys->sys_mode == pds::ONLINE) {
            if (epochs[pds::EpochSys::tid].ui != NULL_EPOCH) {
                ...
                if (((pds::PBlk*)b)->get_epoch() == NULL_EPOCH) {
                    std::reverse_iterator pos = std::find(
                        pending_allocs[pds::EpochSys::tid].ui.rbegin(),
                        pending_allocs[pds::EpochSys::tid].ui.rend(), b);
                    assert(pos != pending_allocs[pds::EpochSys::tid].ui.rend());
                    pending_allocs[pds::EpochSys::tid].ui.erase(
                        (pos + 1).base());
                }
    ...
    void pretire(T* b) {
        if (epochs[pds::EpochSys::tid].ui == NULL_EPOCH) {
            ...
            _esys->prepare_retire_pblk(b, epochs[pds::EpochSys::tid].ui,
                                       pending_retires[pds::EpochSys::tid].ui);
        }
    }
    template <typename T>
    void preclaim(T* b) {
        if (_esys->sys_mode == pds::ONLINE) {
            ...
            if (epochs[pds::EpochSys::tid].ui == NULL_EPOCH) {
                ...
                epochs[pds::EpochSys::tid].ui =
                    _esys->begin_reclaim_transaction();
                if (!b->retired()) {
                    // WARNING!!!
    ...
    std::unordered_map<uint64_t, pds::PBlk*>* recover_pblks(
        const int rec_thd = 10) {
    ...
    void online_mode() { _esys->sys_mode = pds::ONLINE; }
    void flush() {
        for (int i = 0; i < 2; i++) {
            sync();
        }
    ...
    void simulate_crash() { _esys->simulate_crash(); }

    pds::sc_desc_t* get_dcss_desc() { return _esys->get_dcss_desc(); }
    uint64_t get_local_epoch() { return epochs[pds::EpochSys::tid].ui; }
};
...
#define GENERATE_FIELD(t, n, T)                                                \
   ...
   public:                                                                     \
    ...
    T* TOKEN_CONCAT(set_, n)(Recoverable * ds,                                 \
                             const in_type& TOKEN_CONCAT(tmp_, n)) {           \
     ...
     */                                                                        \
    template <class in_type>                                                   \
    void TOKEN_CONCAT(set_unsafe_, n)(Recoverable * ds,                        \
                                      const in_type& TOKEN_CONCAT(tmp_, n)) {  \
...
#define GENERATE_ARRAY(t, n, s, T)                                           \
   ...
   protected:                                                                \
    ...
    /* set method open a pblk for write. return a new copy when necessary */ \
    T* TOKEN_CONCAT(set_, n)(Recoverable * ds, int i,                        \
...
void atomic_lin_var<T>::store(Recoverable* ds, const T& desired) {
    ...
    while (true) {
        ...
        if (var.compare_exchange_strong(r, new_r)) break;
    }
}
...
lin_var atomic_lin_var<T>::load(Recoverable* ds) {
    ...
    while (true) {
        ...
        if (var.compare_exchange_strong(r, ret)) return ret;
    }
}
...
template <typename T>
bool atomic_lin_var<T>::CAS_verify(Recoverable* ds, lin_var expected,
                                   const T& desired) {
    bool not_in_operation = false;
...
template <typename T>
bool atomic_lin_var<T>::CAS_verify(Recoverable* ds, T expected,
                                   const T& desired) {
    bool not_in_operation = false;
    if (ds->get_local_epoch() == NULL_EPOCH) {
        ds->begin_op();
    ...
    ds->get_dcss_desc()->set_up_var(r.cnt + 1, reinterpret_cast<uint64_t>(this),
