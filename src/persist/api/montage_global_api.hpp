...
inline void init(GlobalTestConfig* gtc) {
    // here we assume that pds::init is called before pds::init_thread, hence
    // the assertion. if this assertion triggers, note that the order may be
    // reversed. Evaluation needed.
...
#define CHECK_CURR_OP_EPOCH() ({ global_recoverable->check_epoch(); })

#define CHECK_EPOCH(c) ({ global_recoverable->check_epoch(c); })

// TODO: get rid of arguments in rideables.
#define BEGIN_OP(...) ({ global_recoverable->begin_op(); })

// end current operation by reducing transaction count of our epoch.
// if our operation is already aborted, do nothing.
#define END_OP ({ global_recoverable->end_op(); })

// end current operation by reducing transaction count of our epoch.
// if our operation is already aborted, do nothing.
#define END_READONLY_OP ({ global_recoverable->end_readonly_op(); })

// end current epoch and not move towards next epoch in global_recoverable.
#define ABORT_OP ({ global_recoverable->abort_op(); })
...
#define PMALLOC(sz) ({ global_recoverable->pmalloc(sz); })

#define PNEW(t, ...) ({ global_recoverable->pnew<t>(__VA_ARGS__); })

#define PDELETE(b) ({ global_recoverable->pdelete(b); })

#define PRETIRE(b) ({ global_recoverable->pretire(b); })

#define PRECLAIM(b) ({ global_recoverable->preclaim(b); })

#define POPEN_READ(b) ({ global_recoverable->openread_pblk(b); })

#define POPEN_UNSAFE_READ(b) ({ global_recoverable->openread_pblk_usnafe(b); })

#define POPEN_WRITE(b) ({ global_recoverable->openwrite_pblk(b); })

#define REGISTER_PUPDATE(b) ({ global_recoverable->register_update_pblk(b); })
...
inline void flush() { global_recoverable->flush(); }

inline void recover_mode() { global_recoverable->recover_mode(); }

inline void online_mode() { global_recoverable->online_mode(); }
}  // namespace pds
