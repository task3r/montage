...
ThreadLocalFreedContainer::ThreadLocalFreedContainer(EpochSys* e,
...
ThreadLocalFreedContainer::~ThreadLocalFreedContainer() { delete container; }
...
void ThreadLocalFreedContainer::help_free_local(uint64_t c) {
    container->pop_all_local([&, this](PBlk*& x) { this->do_free(x, c); },
                             EpochSys::tid, c);
}
void ThreadLocalFreedContainer::clear() { container->clear(); }

void PerEpochFreedContainer::do_free(PBlk*& x, uint64_t c) {
    _esys->delete_pblk(x, c);
}
PerEpochFreedContainer::PerEpochFreedContainer(EpochSys* e,
                                               GlobalTestConfig* gtc) {
    container = new VectorContainer<PBlk*>(gtc->task_num);
    _esys = e;
    // container = new HashSetContainer<PBlk*>(gtc->task_num);
}
PerEpochFreedContainer::~PerEpochFreedContainer() { delete container; }
...
void PerEpochFreedContainer::clear() { container->clear(); }
