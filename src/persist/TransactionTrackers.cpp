...
bool PerEpochTransactionTracker::consistent_set(uint64_t target, uint64_t c) {
    ...
    curr_epochs[EpochSys::tid].ui.store(target,
                                        std::memory_order_seq_cst);  // fence
    if (c == global_epoch->load(std::memory_order_acquire)) {
        return true;
    } else {
        curr_epochs[EpochSys::tid].ui.store(
            NULL_EPOCH,
            std::memory_order_seq_cst);  // TODO: double-check this fence.
        return false;
    }
}
PerEpochTransactionTracker::PerEpochTransactionTracker(atomic<uint64_t>* ge,
                                                       int tn)
...
bool PerEpochTransactionTracker::consistent_register_active(uint64_t target,
                                                            uint64_t c) {
    return consistent_set(target, c);
}
bool PerEpochTransactionTracker::consistent_register_bookkeeping(
    uint64_t target, uint64_t c) {
    return consistent_set(target, c);
}
...
bool PerEpochTransactionTracker::no_active(uint64_t target) {
    for (int i = 0; i < task_num; i++) {
        ...
        if (target == curr_epoch && curr_epoch != NULL_EPOCH) {
            // std::cout<<"target:"<<target<<" curr_epoch:"<<curr_epoch<<"
            // i:"<<i<<std::endl;
            return false;
        }
    }
...
bool AtomicTransactionTracker::consistent_increment(
    std::atomic<uint64_t>& counter, const uint64_t c) {
...
AtomicTransactionTracker::AtomicTransactionTracker(atomic<uint64_t>* ge)
    : TransactionTracker(ge) {
    for (int i = 0; i < 4; i++) {
        active_transactions[i].ui.store(0, std::memory_order_relaxed);
        bookkeeping_transactions[i].ui.store(0, std::memory_order_relaxed);
    }
}
bool AtomicTransactionTracker::consistent_register_active(uint64_t target,
                                                          uint64_t c) {
    return consistent_increment(active_transactions[target % EPOCH_WINDOW].ui,
                                c);
}
bool AtomicTransactionTracker::consistent_register_bookkeeping(uint64_t target,
                                                               uint64_t c) {
    return consistent_increment(
        bookkeeping_transactions[target % EPOCH_WINDOW].ui, c);
}
void AtomicTransactionTracker::unregister_active(uint64_t target) {
    active_transactions[target % EPOCH_WINDOW].ui.fetch_sub(
        1, std::memory_order_seq_cst);
}
void AtomicTransactionTracker::unregister_bookkeeping(uint64_t target) {
    bookkeeping_transactions[target % EPOCH_WINDOW].ui.fetch_sub(
        1, std::memory_order_seq_cst);
}
bool AtomicTransactionTracker::no_active(uint64_t target) {
    return (active_transactions[target % EPOCH_WINDOW].ui.load(
                std::memory_order_seq_cst) == 0);
}
bool AtomicTransactionTracker::no_bookkeeping(uint64_t target) {
    return (bookkeeping_transactions[target % EPOCH_WINDOW].ui.load(
                std::memory_order_seq_cst) == 0);
}
...
bool NoFenceTransactionTracker::consistent_register(
    paddedAtomic<bool>* indicators, const uint64_t c) {
...
NoFenceTransactionTracker::NoFenceTransactionTracker(atomic<uint64_t>* ge,
                                                     int tn)
    : TransactionTracker(ge), task_num(tn) {
    for (int i = 0; i < 4; i++) {
...
bool NoFenceTransactionTracker::consistent_register_active(uint64_t target,
                                                           uint64_t c) {
    return consistent_register(active_transactions[target % EPOCH_WINDOW].ui,
                               c);
}
bool NoFenceTransactionTracker::consistent_register_bookkeeping(uint64_t target,
                                                                uint64_t c) {
    return consistent_register(
        bookkeeping_transactions[target % EPOCH_WINDOW].ui, c);
}
...
void FenceBeginTransactionTracker::set_register(
    paddedAtomic<bool>* indicators) {
    assert(EpochSys::tid != -1);
    indicators[EpochSys::tid].ui.store(true, std::memory_order_seq_cst);
}
FenceBeginTransactionTracker::FenceBeginTransactionTracker(atomic<uint64_t>* ge,
                                                           int task_num)
    : NoFenceTransactionTracker(ge, task_num) {}

void FenceEndTransactionTracker::set_unregister(
    paddedAtomic<bool>* indicators) {
    assert(EpochSys::tid != -1);
    indicators[EpochSys::tid].ui.store(false, std::memory_order_seq_cst);
}
FenceEndTransactionTracker::FenceEndTransactionTracker(atomic<uint64_t>* ge,
                                                       int task_num)
    : NoFenceTransactionTracker(ge, task_num) {}
