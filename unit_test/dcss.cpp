...
void increment(size_t tid) {
    ...
    while (true) {
        ...
        BEGIN_OP();
        if (d.CAS(x, x.get_val<uint64_t>() + 1)) real.fetch_add(1);
        END_OP;
    }
}
void increment_verify(size_t tid) {
    ...
    while (true) {
        try {
            ...
            BEGIN_OP_AUTOEND();
            if (d.CAS_verify(x, x.get_val<uint64_t>() + 1)) real.fetch_add(1);
...
int main() {
    ...
    cout << "d = " << dcas::d.load_val() << endl
         << "real = " << dcas::real.load() << endl;
    // cout<<"total cas: "<<pds::total_cnt.load()<<endl<<"abort cas:
    // "<<pds::abort_cnt.load()<<endl;
    return 0;
}